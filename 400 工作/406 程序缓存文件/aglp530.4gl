...
#該程式非freestyle程式!
{<section id="aglp530.description" type="s" >}
#應用 a00 樣板自動產生(Version:3)
#+ Standard Version.....: SD版次:0041(2014-09-25 16:56:16), PR版次:0041(2021-05-14 15:14:06)
#+ Customerized Version.: SD版次:0000(1900-01-01 00:00:00), PR版次:0000(1900-01-01 00:00:00)
#+ Build......: 000677
#+ Filename...: aglp530
#+ Description: 期末結轉作業
#+ Creator....: 01531(2014-07-03 08:55:02)
#+ Modifier...: 01531 -SD/PR- 14589
 
{</section>}
 
{<section id="aglp530.global" type="s" >}
...
#170316-00049#1   2017/03/16 By 02599   在根据当前据点抓取账套和检核当前用户是否有权限时，只报错，不return
#170322-00036#1   2017/03/22 By 01531   凭证里面没有核算项的栏位，没有赋值到空格，而是null。
#170320-00037#1   2017/03/23 By 01531   有设置共用科目结转的,才需要往下做栏位给值等动作，这样才不会占用一个凭证编号 
#170523-00046#1   2017/06/06 By 02599   1.从第二页签回到第一页签是，不用再将账套、年度、期别赋值预设值
#                                       2.在产生次月的应计凭证前，要先判断当月是否存在应计的凭证，若不存在，RETURN
#170621-00001#4   2017/06/21 By 09772   1.2401行的子SQL指定ent
#                                       2.2820行的子查询中glac表没有ent
#170617-00513#1   2017/07/27 By 02599   现行年度/期别=执行期末结转后的下一会计期
#170421-00009#1   2017/08/14 By 01531   按下執行後,整體完成進度條顯示100% 
#170727-00007#1   170911     By albireo RA傳票比照aglt310將現金科目產生glbc
#180308-00078#1   2018/04/10 By 09773   损益类的科目月结的时候要按照科目和汇率实际显示原币金额
#180507-00019#2   2018/05/08 By 04152   1.RA傳票的glap015要記錄來源的AC傳票單號
#                                       2.重新執行aglp530，現變碼glbc_t沒有刪除
#180808-00030#1   2018/08/10 by 08172   产生CE凭证时，抓取的科目由glch002改为glch004
#181107-00052#1   2018/11/16 By 05795   aglp530在产生glaq010原币金额时，通过借贷方交换后的借方金额glaq003是否大于0，没有考虑贷方金额是负数的情况，导致最后生成的数据本币与原币金额不相等
#190506-00054#1   2019/05/07 By 11384   glap002,glap004 年度期别取值时，调整为通过 s_get_accdate() 取值
#190428-00007#1   2019/05/13 By 12136   執行後不應該再抓關帳日期作為年度期別的預設值
#190630-00007#1   2019/07/17 By 01531   完善#190506-00054#1，畫面預設的是關賬日期對應的年期，調整一開始無關賬日期時，call s_get_accdate報錯的問題
#191118-00035#1   2019/11/19 By 05795   应计调整的凭证金额原币与本币金额不一致
#200212-00013#1   2020/02/21 By 10500   agli162未维护时 月结／年结时抓最近年度的资料结转
#200116-00002#12  2020/03/20 By 13656   取汇率改为调用s_fin_get_curr_rate(传入参数)
#190605-00032#1   2020/05/09 By 12290   在决定是否使用红字时，不仅要考虑单据别参数，也要判断科目参数
#200605-00087#1   2020/06/12 By 13589   子查询回传多笔问题修改
#200813-00024#1   2020/08/14 By 11234   效能优化，将in的写法改为exists写法
#210209-00005#1   2021/02/18 By 08734   修正181107-00052#1此单修改只改了收入科目结转本年利润部分，没有改费用科目结转本年利润部分问题
#210426-00027#1   2021/05/14 By 14589   修改rownum區分ora/pgs語法
#end add-point
...
DEFINE g_master_o type_master #190630-00007#1 add
...
DEFINE g_glaq RECORD  #傳票憑證單身檔
       ...
       #180308-00078#1 add--s
       #区分汇率不同时
       
       glaq010_j LIKE glaq_t.glaq010,  #借方金额 
       glaq010_d LIKE glaq_t.glaq010   #贷方金额
       #180308-00078#1 add--e
       END RECORD
...
#190506-00054#1 add_S
DEFINE g_flag1          LIKE type_t.chr1
DEFINE g_errno1          LIKE type_t.chr100
...
#190506-00054#1 add_E
DEFINE g_glap002        LIKE glap_t.glap002   #200212-00013#1 add
DEFINE g_postsql1       STRING   #PGS(D)-10500-add
DEFINE g_postsql2       STRING   #PGS(D)-10500-add
#end add-point
...
{<section id="aglp530.main" type="s" >}
MAIN
...
{<section id="aglp530.init" type="s" >}
#+ 初始化作業
...
#      RETURN #170316-00049#1 mark
      LET g_master.glapld = '' #170316-00049#1 add
   END IF
...
{<section id="aglp530.ui_dialog" type="s" >}
#+ 選單功能實際執行處
         ...
         AFTER FIELD glapld
            ...
            IF NOT cl_null(g_master.glapld) THEN
               ...
               SELECT glaa003,glaa010,glaa011,glaa013,glaa024,glaa112 
                ...
                WHERE glaaent = g_enterprise
                  AND glaald = g_master.glapld
#190506-00054#1  mark_S                
#               LET g_master.glap002 = g_glaa010
#               LET g_master.glap004 = g_glaa011
#190506-00054#1  mark_E  
#190506-00054#1  add_S
               IF NOT cl_null(g_glaa013) THEN #190630-00007#1 add  
                  CALL s_get_accdate(g_glaa003,g_glaa013,'','')
                     RETURNING g_flag1,g_errno1,g_glav002,g_glav005,g_sdate_s,g_sdate_e,
                        g_glav006,g_pdate_s,g_pdate_e,g_glav007,g_wdate_s,g_wdate_e
                  IF g_flag1='N' THEN
                     INITIALIZE g_errparam TO NULL
                     LET g_errparam.code = g_errno1
                     LET g_errparam.extend = ''
                     LET g_errparam.popup = FALSE
                     CALL cl_err()
                  END IF
               #190630-00007#1 add s--   
               ELSE
                  LET g_glav002 = NULL
                  LET g_glav006 = NULL
               END IF     
               #190630-00007#1 add e---
               #190428-00007#1  mark  2019/05/13 By 12136 ---(s)---
               #LET g_master.glap002 = g_glav002
               #LET g_master.glap004 = g_glav006
               #190428-00007#1  mark  2019/05/13 By 12136 ---(e)--- 
               #190630-00007#1 add s---
               IF g_master_o.glapld <> g_master.glapld OR cl_null(g_master_o.glapld) THEN    
                  LET g_master.glap002 = g_glav002
                  LET g_master.glap004 = g_glav006               
               END IF       
               #190630-00007#1 add e---               
#190506-00054#1  add_E
            ...
            END IF 
            CALL aglp530_glapld_desc() 
            LET g_master_o.* = g_master.*  #190630-00007#1 add
               ...
               IF NOT cl_null(g_glaa013) THEN #190630-00007#1 add
                  CALL s_get_accdate(g_glaa003,g_glaa013,'','')
                     ...
                     IF g_flag1='N' THEN
                        ...
                        CALL cl_err()
                     END IF 
                  IF g_master.glap002 < g_glav002 THEN              
#190506-00054#1  add_E   
#               IF g_master.glap002 < YEAR(g_glaa013) THEN  #190506-00054#1 mark               
                     INITIALIZE g_errparam TO NULL
             ...
             IF NOT cl_null(g_master.glap004) THEN
                ...
                SELECT glav003 INTO l_glav003 FROM glav_t
                 WHERE glavent = g_enterprise
                   ...
                   AND rownum  = 1#200605-00087#1 add
                #PGS(D)-10500 -E-                
                #1.月，2.周，3.445式   
...
#190506-00054#1  add_S
                IF NOT cl_null(g_glaa013) THEN #190630-00007#1 add
                   ...
                   IF g_flag1='N' THEN
                      ...
                      CALL cl_err()
                   END IF 
                   IF g_master.glap002 = g_glav002 AND  g_master.glap004 < g_glav006  THEN               
#190506-00054#1  add_E   
#                IF g_master.glap002 = YEAR(g_glaa013) AND  g_master.glap004 < MONTH(g_glaa013) THEN  #190506-00054#1 mark
                      INITIALIZE g_errparam TO NULL
                      LET g_errparam.code = 'agl-00163'
                      LET g_errparam.extend = g_master.glap004
                      LET g_errparam.popup = FALSE
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD glap002
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD glap004
...
{<section id="aglp530.transfer_argv" type="s" >}
...
{<section id="aglp530.process" type="s" >}
...
PRIVATE FUNCTION aglp530_process(ls_js)
  ...
  #END IF
   ...
   #170421-00009#1 add s---
   IF g_bgjob <> "Y" THEN
      CALL cl_progress_bar_no_window(1)
   END IF   
   #170421-00009#1 add e---   
...
#190506-00054#1  add_S
   IF NOT cl_null(g_glaa013) THEN #190630-00007#1 add
      ...
      IF g_flag1='N' THEN
         ...
         CALL cl_err()
      END IF
      IF (g_master.glap002 < g_glav002) OR (g_master.glap002 = g_glav002 AND
         (NOT cl_null(g_master.glap004) AND g_master.glap004 < g_glav006)) THEN                 
#190506-00054#1  add_E       
#      CALL cl_errmsg('glaa013',g_glaa013,'','agl-00163',1)
         ...
         DISPLAY '' ,0 TO stagenow,stagecomplete #170421-00009#1 add 
         RETURN    
      END IF 
   END IF #190630-00007#1 add  
   #判斷是否設置拋轉月結傳票單據別
...
#            OR cl_null(g_glaa010) OR cl_null(g_glaa011)
         IF g_master.glap004 < g_max_period
         #170617-00513#1--mod--end
         THEN
         #161018-00024#1--mod--end
            ...
            UPDATE glaa_t SET glaa010 = g_master.glap002,
             ...
             WHERE glaaent = g_enterprise
               AND glaald = g_master.glapld
         #170617-00513#1--add--str--
         ELSE
            #更新对应帐别的会计年度=年度+1，期别 = 1
            UPDATE glaa_t SET glaa010 = g_master.glap002+1,
                              glaa011 = 1
             WHERE glaaent = g_enterprise
               AND glaald = g_master.glapld
         #170617-00513#1--add--end
         END IF
...
#         CALL cl_errmsg('UPD_glaa',g_master.glapld,'',SQLCA.SQLCODE,1)
      ...
      IF g_success = 'Y' THEN 
         CALL s_transaction_end('Y','0')
         CALL cl_progress_no_window_ing("")   #170421-00009#1 add 
      ELSE
         CALL s_transaction_end('N','0')
         DISPLAY '' ,0 TO stagenow,stagecomplete #170421-00009#1 add
      END IF
...
{<section id="aglp530.get_buffer" type="s" >}
...
{<section id="aglp530.msgcentre_notify" type="s" >}
...
{<section id="aglp530.other_function" readonly="Y" type="s" >}
...
PRIVATE FUNCTION aglp530_glaa006_1()
   ...
   DEFINE l_glch004       LIKE glch_t.glch004     #180808-00030#1  add   
   ...
   DEFINE l_rate          LIKE type_t.num20_6     #200116-00002#12 add 
   ...
   #========单头glap_t======================
      ...
      IF l_flag='N' THEN
         INITIALIZE g_errparam TO NULL
#         LET g_errparam.code = 'l_errno'  #170523-00046#1 mark
         LET g_errparam.code = l_errno     #170523-00046#1 add
         LET g_errparam.extend = ''
         LET g_errparam.popup = TRUE
         CALL cl_err()
      ...
      #200116-00002#12 --s add
      IF g_glaa015='Y' OR g_glaa019='Y' THEN
         CALL s_fin_get_curr_rate(g_glap.glapcomp,g_glap.glapld,g_glap.glapdocdt,g_glaq.glaq005,'')
            RETURNING l_rate,g_glaq.glaq039,g_glaq.glaq042
      END IF
      #200116-00002#12 --e add
      
     
      #匯率(本位幣二)
      IF g_glaa015='Y' THEN
         #200116-00002#12 --s mark
#         CALL s_aooi160_get_exrate('2',g_glap.glapld,g_glap.glapdocdt,g_glaq.glaq005,g_glaa016,0,g_glaa018)
#         RETURNING  g_glaq.glaq039
         #200116-00002#12 --e mark
      ELSE
...
#                            AND glac006 = '1'              #账户科目
#                            AND glac007 = '6')            #损益类科目                             
         #200813-00024#1 mark(e)
         #200813-00024#1 add(s)
         AND EXISTS (SELECT 1 FROM glac_t
                      WHERE glarent = glacent
                        AND glar001 = glac002                       
                        AND glac001 = g_glaa004        #会计科目参照表号
                        AND glac003 IN ('2','3')       #獨立或明細科目
...
#                  "                      AND glac007 = '6')",            #损益类科目
                  ...
                  #200813-00024#1 add(e)
                  "   AND glar002 =", g_master.glap002,
                  "   AND glar003 =", g_master.glap004  #PGS(D)-10500-remove,
                  #PGS(D)-10500 -S-
                  LET l_sql = l_sql CLIPPED," GROUP BY ",l_sql2," ORDER BY ",l_sql2
                  #PGS(D)-10500 -E- 
...
#           IF l_sum1>0 OR l_red='Y' THEN
              ...
              #核算项如果没值,给个空格 
              IF l_glaq[l_i].glaq018 IS NULL THEN LET l_glaq[l_i].glaq018 = ' ' END IF
              IF l_glaq[l_i].glaq019 IS NULL THEN LET l_glaq[l_i].glaq019 = ' ' END IF
              IF l_glaq[l_i].glaq020 IS NULL THEN LET l_glaq[l_i].glaq020 = ' ' END IF
              IF l_glaq[l_i].glaq021 IS NULL THEN LET l_glaq[l_i].glaq021 = ' ' END IF
              IF l_glaq[l_i].glaq022 IS NULL THEN LET l_glaq[l_i].glaq022 = ' ' END IF
              IF l_glaq[l_i].glaq023 IS NULL THEN LET l_glaq[l_i].glaq023 = ' ' END IF
              IF l_glaq[l_i].glaq024 IS NULL THEN LET l_glaq[l_i].glaq024 = ' ' END IF
              IF l_glaq[l_i].glaq025 IS NULL THEN LET l_glaq[l_i].glaq025 = ' ' END IF
              IF l_glaq[l_i].glaq027 IS NULL THEN LET l_glaq[l_i].glaq027 = ' ' END IF
              IF l_glaq[l_i].glaq028 IS NULL THEN LET l_glaq[l_i].glaq028 = ' ' END IF
              IF l_glaq[l_i].glaq051 IS NULL THEN LET l_glaq[l_i].glaq051 = ' ' END IF
              IF l_glaq[l_i].glaq052 IS NULL THEN LET l_glaq[l_i].glaq052 = ' ' END IF
              IF l_glaq[l_i].glaq053 IS NULL THEN LET l_glaq[l_i].glaq053 = ' ' END IF
              IF l_glaq[l_i].glaq029 IS NULL THEN LET l_glaq[l_i].glaq029 = ' ' END IF
              IF l_glaq[l_i].glaq030 IS NULL THEN LET l_glaq[l_i].glaq030 = ' ' END IF
              IF l_glaq[l_i].glaq031 IS NULL THEN LET l_glaq[l_i].glaq031 = ' ' END IF
              IF l_glaq[l_i].glaq032 IS NULL THEN LET l_glaq[l_i].glaq032 = ' ' END IF
              IF l_glaq[l_i].glaq033 IS NULL THEN LET l_glaq[l_i].glaq033 = ' ' END IF
              IF l_glaq[l_i].glaq034 IS NULL THEN LET l_glaq[l_i].glaq034 = ' ' END IF
              IF l_glaq[l_i].glaq035 IS NULL THEN LET l_glaq[l_i].glaq035 = ' ' END IF
              IF l_glaq[l_i].glaq036 IS NULL THEN LET l_glaq[l_i].glaq036 = ' ' END IF
              IF l_glaq[l_i].glaq037 IS NULL THEN LET l_glaq[l_i].glaq037 = ' ' END IF
              IF l_glaq[l_i].glaq038 IS NULL THEN LET l_glaq[l_i].glaq038 = ' ' END IF
...
#           IF l_sum1>0 OR l_red='Y' THEN
        ...
        #200212-00013#1   add-S
        #agli162未维护时 月结／年结时抓最近年度的资料结转
        LET g_glap002 = 0
        SELECT UNIQUE glch001 INTO g_glap002 
          FROM glch_t
         WHERE glchent = g_enterprise AND glchld = g_master.glapld AND glch001 = g_master.glap002
        IF cl_null(g_glap002) OR g_glap002 = 0 THEN
           SELECT glch001 INTO g_glap002
             FROM ( SELECT UNIQUE rownum num,glch001
                     FROM glch_t
                    WHERE glchent = g_enterprise
                      AND glchld  = g_master.glapld
                      AND glch001 < g_master.glap002
                    ORDER BY glch001 DESC )
                    WHERE num = 1
        ELSE
           LET g_glap002 = g_master.glap002
        END IF
        #200212-00013#1   add-E
        #160628-00036#2 add s---
#        LET l_sql = " SELECT glch002,glch003 FROM glch_t ",   #180808-00030#1 180810 mark by 08172
        LET l_sql = " SELECT glch002,glch003,glch004 FROM glch_t ",   #180808-00030#1 180810 add by 08172
                    "  WHERE glchent = '",g_enterprise,"'",
                    "    AND glchld = '",g_master.glapld,"'",
                   #"    AND glch001 = '",g_master.glap002,"'",   #200212-00013#1 mark
                    "    AND glch001 = '",g_glap002,"'",          #200212-00013#1 add
#                    "    ORDER BY glch002,glch003 "       #180808-00030#1 180810 mark by 08172
                    "    ORDER BY glch002,glch003,glch004 "       #180808-00030#1 180810 add by 08172
...
#        FOREACH aglp530_glch_cs INTO l_glch002,l_glch003  #180808-00030#1 180810 mark by 08172
        FOREACH aglp530_glch_cs INTO l_glch002,l_glch003,l_glch004  #180808-00030#1 180810 add by 08172
           IF SQLCA.sqlcode THEN
#              CALL cl_errmsg('FOREACH','','',SQLCA.SQLCODE,1)
              INITIALIZE g_errparam TO NULL
   ...
   #              CALL cl_errmsg('FOREACH','','',SQLCA.SQLCODE,1)
              ...
              IF l_glac008 = '2' THEN
                 LET l_glaq[l_i].glaq010 = l_sum1
#                 LET l_glaq[l_i].glaq002 = l_glch002    #共用类 #180808-00030#1 180810 mark by 08172
                 LET l_glaq[l_i].glaq002 = l_glch004    #共用类 #180808-00030#1 180810 add by 08172
                 LET l_glaq[l_i].glaq003 = l_sum1               #借方金額(本位幣一)
                 IF g_glaa015='Y' THEN
                    LET l_glaq[l_i].glaq040 = l_sum2     #借方金額(本位幣二)
                 END IF
...
PRIVATE FUNCTION aglp530_glaa006_2(p_glap004_s,p_glap004_e)
   ...
   DEFINE l_glaa001        LIKE glaa_t.glaa001 #180308-00078#1 add
   DEFINE l_cnt1           LIKE type_t.num5    #180308-00078#1 add
   DEFINE l_rate           LIKE type_t.num20_6 #200116-00002#12 add
   #A.產生兩張期末結轉傳票(CE)
   ####################### A-1.費用科目結轉本年利潤 ###########################
   #  傳票單別=總帳參數設定檔.結轉用單別
...
#      CALL cl_errmsg('','','','agl-00210',1)
   ...
   LET g_glaq.glaqdocno = g_glap.glapdocno
   LET g_glaq.glaqcomp = g_glap.glapcomp
   #LET g_glaq.glaq005 = g_glaa001 #180308-00078#1 mark
   #LET g_glaq.glaq006 = 1         #180308-00078#1 mark
...
#      RETURNING  g_glaq.glaq042
   ...
   LET l_sql = " SELECT SUM(glaq003),SUM(glaq004),SUM(glaq040),SUM(glaq041),SUM(glaq043),SUM(glaq044),",
               " SUM (CASE WHEN glaq003 <> 0 THEN glaq010 ElSE 0 END ),",  #借方原币   #180308-00078#1 add
               " SUM (CASE WHEN glaq004 <> 0 THEN glaq010 ELSE 0 END ),",  #贷方原币   #180308-00078#1 add
               ...
               "        glaq035,glaq036,glaq037,glaq038,glaq051,glaq052,glaq053,glaq005,glaq006",   #180308-00078#1 add glaq005,glaq006
               "   FROM glaq_t,glap_t ",
...
#               "                       AND glaqent = ",g_enterprise," ",   #add by wanghaoz 20170621 #170621-00001#4    指定子SQL中glaq表的ent
#               "                       AND glaq002 = glac002",
...
#               "                       AND glac008 = '1') ",  #借余
               ...
               "             glaq005,glaq006 ",   #180308-00078#1 add glaq005,glaq006
   ...
   FOREACH aglp530_glaq_cs INTO g_glaq.glaq003,g_glaq.glaq004,g_glaq.glaq040,g_glaq.glaq041,g_glaq.glaq043,g_glaq.glaq044,
                                g_glaq.glaq010_j,g_glaq.glaq010_d,  #180308-00078#1  add
                                ...
                                g_glaq.glaq053,
                                g_glaq.glaq005,g_glaq.glaq006  #180308-00078#1  add  g_glaq.glaq005,g_glaq.glaq006
      IF SQLCA.sqlcode THEN
#         CALL cl_errmsg('',g_glaq.glaq002,'',SQLCA.SQLCODE,1)
      ...
      #LET g_glaq.glaq010 = g_glaq.glaq003 + g_glaq.glaq004  #180308-00078#1  mark
      #180308-00078#1  mod --s
      #科目冲销，借贷方相反
      #IF g_glaq.glaq003 > 0 THEN #210209-00005#1 mark
      IF g_glaq.glaq003 <> 0 THEN #210209-00005#1 add
         LET g_glaq.glaq010 = g_glaq.glaq010_d-g_glaq.glaq010_j
      ELSE
         LET g_glaq.glaq010 = g_glaq.glaq010_j-g_glaq.glaq010_d
      END IF
      #180308-00078#1 mod --e

      #170322-00036#1 add s--- 
      #核算项如果没值,给个空格 
      IF g_glaq.glaq018 IS NULL THEN LET g_glaq.glaq018 = ' ' END IF
      IF g_glaq.glaq019 IS NULL THEN LET g_glaq.glaq019 = ' ' END IF
      IF g_glaq.glaq020 IS NULL THEN LET g_glaq.glaq020 = ' ' END IF
      IF g_glaq.glaq021 IS NULL THEN LET g_glaq.glaq021 = ' ' END IF
      IF g_glaq.glaq022 IS NULL THEN LET g_glaq.glaq022 = ' ' END IF
      IF g_glaq.glaq023 IS NULL THEN LET g_glaq.glaq023 = ' ' END IF
      IF g_glaq.glaq024 IS NULL THEN LET g_glaq.glaq024 = ' ' END IF
      IF g_glaq.glaq025 IS NULL THEN LET g_glaq.glaq025 = ' ' END IF
      IF g_glaq.glaq027 IS NULL THEN LET g_glaq.glaq027 = ' ' END IF
      IF g_glaq.glaq028 IS NULL THEN LET g_glaq.glaq028 = ' ' END IF
      IF g_glaq.glaq051 IS NULL THEN LET g_glaq.glaq051 = ' ' END IF
      IF g_glaq.glaq052 IS NULL THEN LET g_glaq.glaq052 = ' ' END IF
      IF g_glaq.glaq053 IS NULL THEN LET g_glaq.glaq053 = ' ' END IF
      IF g_glaq.glaq029 IS NULL THEN LET g_glaq.glaq029 = ' ' END IF
      IF g_glaq.glaq030 IS NULL THEN LET g_glaq.glaq030 = ' ' END IF
      IF g_glaq.glaq031 IS NULL THEN LET g_glaq.glaq031 = ' ' END IF
      IF g_glaq.glaq032 IS NULL THEN LET g_glaq.glaq032 = ' ' END IF
      IF g_glaq.glaq033 IS NULL THEN LET g_glaq.glaq033 = ' ' END IF
      IF g_glaq.glaq034 IS NULL THEN LET g_glaq.glaq034 = ' ' END IF
      IF g_glaq.glaq035 IS NULL THEN LET g_glaq.glaq035 = ' ' END IF
      IF g_glaq.glaq036 IS NULL THEN LET g_glaq.glaq036 = ' ' END IF
      IF g_glaq.glaq037 IS NULL THEN LET g_glaq.glaq037 = ' ' END IF
      IF g_glaq.glaq038 IS NULL THEN LET g_glaq.glaq038 = ' ' END IF
      #170322-00036#1 add e--- 
...
#   LET g_glaq.glaq002 = '2298'    #本年利润
   ...
   LET g_glaq.glaq006 = 1   #180308-00078#1 add
   LET g_glaq.glaq005 = g_glaa001 #180308-00078#1 add   
   LET l_sql="SELECT SUM(glaq003),SUM(glaq004),SUM(glaq040),SUM(glaq041),SUM(glaq043),SUM(glaq044),",l_sql1,
             "  FROM p530_tmp "#PGS(D)-10500-remove,
             #PGS(D)-10500 -S-
             LET l_sql = l_sql CLIPPED," GROUP BY ",l_sql1," ORDER BY ",l_sql1
             #PGS(D)-10500 -E-
...
#            CALL cl_errmsg('ins_tmp_2',g_glaq.glaq002,'',SQLCA.SQLCODE,1)
   ...
   IF l_cnt>0 THEN
      ...
      SELECT COUNT(1) INTO l_cnt1 FROM p530_tmp
      WHERE glaqdocno=g_glaq.glaqdocno
        AND glaq005 <> l_glaa001
      IF l_cnt1 >0 THEN
         LET g_glap.glap014 = 'Y'
      END IF
      #180308-00078#1  mod --e
      #INSERT INTO glap_t VALUES (g_glap.*)
      INSERT INTO glap_t (glapent,glapld,glapcomp,glapdocno,glapdocdt,glap001,glap002,glap003,glap004,glap005,
...
#               "                     WHERE glaqent = glacent",
#               "                       AND glaq002 = glac002",
#               "                       AND glacent = ",g_enterprise," ",    #add by wanghaoz 20170621 #170621-00001#4     子查询glac的ent
#               "                       AND glac001 = '",g_glaa004,"'",
#               "                       AND glac007 = '6'",
#               "                       AND glac008 = '2') ",
               #200813-00024#1 mark(e)
   ...
   FOREACH aglp530_glaq_cs2 INTO g_glaq.glaq003,g_glaq.glaq004,g_glaq.glaq040,g_glaq.glaq041,g_glaq.glaq043,g_glaq.glaq044,
                                 ...
                                 g_glaq.glaq031,g_glaq.glaq032,g_glaq.glaq033,g_glaq.glaq034,g_glaq.glaq035,
                                 g_glaq.glaq036,g_glaq.glaq037,g_glaq.glaq038,g_glaq.glaq051,g_glaq.glaq052,
                                 g_glaq.glaq053,g_glaq.glaq005,g_glaq.glaq006  #180308-00078#1 add glaq005,glaq006
      IF SQLCA.sqlcode THEN
#         CALL cl_errmsg('',g_glaq.glaq002,'',SQLCA.SQLCODE,1)
         INITIALIZE g_errparam TO NULL
      ...
      #180308-00078#1  mod --s
      #科目冲销，借贷方相反
#      IF g_glaq.glaq003 > 0 THEN  #181107-00052#1---mark
      IF g_glaq.glaq003 <> 0 THEN #181107-00052#1---add
         LET g_glaq.glaq010 = g_glaq.glaq010_d-g_glaq.glaq010_j
      ELSE
         LET g_glaq.glaq010 = g_glaq.glaq010_j-g_glaq.glaq010_d
      END IF
...
#          CALL cl_errmsg('INS_glaq_a2',g_glap.glapdocno,'',SQLCA.SQLCODE,1)
   ...
   #170320-00037#1 add s---
   LET l_cnt=0
   SELECT COUNT(glch003) INTO l_cnt FROM glch_t
    WHERE glchent = g_enterprise AND glchld = g_master.glapld
     #AND glch001 = g_master.glap002   #200212-00013#1 mark
      AND glch001 = g_glap002          #200212-00013#1 add
      AND glchstus = 'Y'
...
#               "                        AND glchstus = 'Y')",
               #200813-00024#1 mark(e)
               #200813-00024#1 add(s)
               "    AND EXISTS (SELECT 1 FROM glch_t ",
               "                 WHERE glchent = glaqent ",
               "                   AND glch002 = glaq002 ",
               "                   AND glchld = '",g_master.glapld,"' ",
               "                   AND glch001 = ",g_glap002,
               "                   AND glchstus = 'Y') ",
               #200813-00024#1 add(e)
               "    AND glap002 = ",g_master.glap002,
               "    AND glap004 BETWEEN ",p_glap004_s," AND ",p_glap004_e,
...
#         CALL cl_errmsg('',g_glaq.glaq002,'',SQLCA.SQLCODE,1)
      ...
      LET l_sql="SELECT SUM(glaq003),SUM(glaq004),SUM(glaq040),SUM(glaq041),SUM(glaq043),SUM(glaq044),",l_sql1,
                ...
                "   AND glaq038 = '",g_glaq.glaq038,"' AND glaq023 = '",g_glaq.glaq023,"'"#PGS(D)-10500-remove,
                #PGS(D)-10500 -S-
                LET l_sql = l_sql CLIPPED," GROUP BY ",l_sql1," ORDER BY ",l_sql1
                #PGS(D)-10500 -E-
   ...
   #180808-00030#1 -s 180810 add by 08172
   #glch002->glch004
   UPDATE p530_tmp SET glaq002 = (SELECT glch004 
                                    FROM glch_t 
                                   WHERE glchent = g_enterprise
                                     AND glchld = g_master.glapld
                                    #AND glch001 = g_master.glap002   #200212-00013#1 mark
                                     ...
                                     AND glchstus = 'Y')
    #200813-00024#1 mark(s)
#    WHERE glaq002 IN (SELECT glch002
#                        FROM glch_t 
#                       WHERE glchent = g_enterprise
#                         AND glchld = g_master.glapld
#                        #AND glch001 = g_master.glap002   #200212-00013#1 mark
...
#                         AND glchstus = 'Y')
   #200813-00024#1 mark(e)
   #200813-00024#1 add(s)
    WHERE EXISTS (SELECT 1 FROM glch_t
                   WHERE glchent = g_enterprise
                     AND glch002 = glaq002
                     AND glchld = g_master.glapld
                     AND glch001 = g_glap002
   ...
   IF SQLCA.SQLCODE THEN
      INITIALIZE g_errparam TO NULL
      LET g_errparam.extend = 'UPDATE p530_tmp'
      LET g_errparam.code = SQLCA.SQLCODE
      LET g_errparam.popup = TRUE
      CALL cl_err()
      LET g_success = 'N' 
   END IF 
   #180808-00030#1 -e 180810 add by 08172
...
PRIVATE FUNCTION aglp530_ra_gen()
   ...
   #170727-00007#1-----e
   DEFINE l_glad002   LIKE glad_t.glad002 #190605-00032#1 add
...
#→更新指定帳別的帳別基本資料檔(glaa_t).現行會計期別(累加一期)
     ...
     SELECT COUNT(1) INTO l_cnt FROM glap_t
      WHERE glapent=g_enterprise AND glapld=g_master.glapld
        AND glap002=g_master.glap002 AND glap004=g_master.glap004
        AND glap007='AC' AND glapstus = 'S'
...
#        CALL cl_errmsg('l_sql','ra_gen_glaq','',SQLCA.SQLCODE,1)
     ...
     #170727-00007#1-----s
     #直接將AC傳票的glbc複製過來借貸別顛倒          
     LET l_sql = "INSERT INTO glbc_t(",
                                     "glbcent,glbcld,glbccomp,glbcdocno,glbcseq,",
                                     "glbcseq1,glbc001,glbc002,glbc003,glbc004, ",
                                     "glbc005,glbc006,glbc007,glbc008,glbc009,  ",
                                     "glbc010,glbc011,glbc012,glbc013,glbc014,  ",
                                     "glbcud001,glbcud002,glbcud003,glbcud004,  ",
                                     "glbcud005,glbcud006,glbcud007,glbcud008,  ",
                                     "glbcud009,glbcud010,glbcud011,glbcud012,  ",
                                     "glbcud013,glbcud014,glbcud015,glbcud016,  ",
                                     "glbcud017,glbcud018,glbcud019,glbcud020,  ",
                                     "glbcud021,glbcud022,glbcud023,glbcud024,  ",
                                     "glbcud025,glbcud026,glbcud027,glbcud028,  ",
                                     "glbcud029,glbcud030,glbc015               ",
                                     ") ",
                                     "   SELECT glbcent,glbcld,glbccomp,?,glbcseq,",
                                     "glbcseq1,?,?,CASE WHEN glbc003='1'THEN '2' ELSE '1' END,glbc004, ",
                                     "glbc005,glbc006,glbc007,glbc008,glbc009,  ",
                                     "glbc010,glbc011,glbc012,glbc013,glbc014,  ",
                                     "glbcud001,glbcud002,glbcud003,glbcud004,  ",
                                     "glbcud005,glbcud006,glbcud007,glbcud008,  ",
                                     ...
                                     "glbcud021,glbcud022,glbcud023,glbcud024,  ",
                                     "glbcud025,glbcud026,glbcud027,glbcud028,  ",
                                     "glbcud029,glbcud030,'Y'               ",
                                     " FROM glbc_t ",
                                     " WHERE glbcent = ? ",
                                     "   AND glbcdocno = ? "
     PREPARE ins_glbcp1 FROM l_sql
     #170727-00007#1-----e
...
#        END IF #170208-00016#1 add #170214-00033#1 mark
        ...
        LET g_glap.glapcnfdt = cl_get_current()
        LET ld_date = cl_get_current()
        LET g_glap.glap015 = l_glapdocno  #原傳票編號 #180507-00019#2
        #INS_glap_t==========
        #161111-00028#8----modify----begin---------
        #INSERT INTO glap_t VALUES(g_glap.*)
        INSERT INTO glap_t (glapent,glapld,glapcomp,glapdocno,glapdocdt,glap001,glap002,glap003,glap004,glap005,
...
#           CALL cl_errmsg('UPD date',g_glap.glapdocno,'',SQLCA.SQLCODE,1)
        ...
        #170727-00007#1-----s
        #直接將AC傳票的glbc複製過來借貸別顛倒          
        EXECUTE ins_glbcp1 USING g_glap.glapdocno,g_glap.glap002,g_glap.glap004,g_enterprise,l_glapdocno
        IF SQLCA.SQLCODE THEN
           INITIALIZE g_errparam TO NULL
           LET g_errparam.extend = 'INSERT INTO glbc_t'
           LET g_errparam.code = SQLCA.SQLCODE
           LET g_errparam.popup = TRUE
           CALL cl_err()
           LET g_success = 'N' 
...
#              CALL cl_errmsg('RA_gen_glaq','g_glap.glapdocno','',SQLCA.SQLCODE,1)
           ...
           #190605-00032#1 add-S  #根据总账单据别参数D-FIN-1002和科目参数(agli031)共同判断，是否产生负数凭证
           LET l_glad002 = 'N'
           ...
           #190605-00032#1 add-E
           
#           IF l_red = 'Y' THEN   #190605-00032#1 mark
           IF l_red = 'Y' AND l_glad002 = 'Y' THEN   #190605-00032#1 add
              LET g_glaq.glaq003 = g_glaq.glaq003 * (-1)
              LET g_glaq.glaq004 = g_glaq.glaq004 * (-1)
              LET g_glaq.glaq010 = g_glaq.glaq010 * (-1)  #191118-00035#1--add
...
PRIVATE FUNCTION aglp530_glaz()
 ...
 #161111-00028#8----modify----end---------
   ...
   LET l_sql="SELECT glazent,glazld,glazcomp,glazdocno,glazseq,glazdocdt,",
             ...
             " ORDER BY glaz_t.glazdocno,glaz_t.glazseq,glaz_t.glaz003" #ODI-ORA#
...
#         CALL cl_errmsg('INSERT glaz_t',g_glap.glapdocno,'',SQLCA.SQLCODE,1)
   ...
   LET l_sql="SELECT glaxcomp,glaxdocno,glaxseq,glaxdocdt,",
             ...
             " ORDER BY glax_t.glaxdocno,glax_t.glaxseq,glax_t.glax002" #ODI-ORA#
...
PRIVATE FUNCTION aglp530_fix_acc_sql(p_glaq002)
   ...
   LET r_sql="glaq017"
   LET r_sql1="glar012"
   LET g_postsql1 = r_sql      #PGS(D)-10500-add
   LET g_postsql2 = r_sql1     #PGS(D)-10500-add   
   IF l_glad007='Y' THEN
      LET r_sql=r_sql,",glaq018"
      LET r_sql1=r_sql1,",glar013"
      LET g_postsql1 = g_postsql1,",glaq018"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar013"    #PGS(D)-10500-add
   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad008='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq019"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar014"    #PGS(D)-10500-add   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad009='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq020"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar015"    #PGS(D)-10500-add   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad010='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq021"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar016"    #PGS(D)-10500-add
   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad027='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq022"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar017"    #PGS(D)-10500-add
   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad011='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq023"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar018"    #PGS(D)-10500-add
   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad012='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq024"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar019"    #PGS(D)-10500-add
   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad031='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq051"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar051"    #PGS(D)-10500-add
   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad032='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq052"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar052"    #PGS(D)-10500-add
   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad033='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq053"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar053"    #PGS(D)-10500-add   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   #人員
   IF l_glad013='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq025"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar020"    #PGS(D)-10500-add   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
...
#   END IF
   ...
   IF l_glad015='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq027"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar022"    #PGS(D)-10500-add   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad016='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq028"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar023"    #PGS(D)-10500-add   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad017='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq029"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar024"    #PGS(D)-10500-add
   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad018='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq030"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar025"    #PGS(D)-10500-add   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad019='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq031"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar026"    #PGS(D)-10500-add   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad020='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq032"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar027"    #PGS(D)-10500-add   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad021='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq033"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar028"    #PGS(D)-10500-add   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad022='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq034"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar029"    #PGS(D)-10500-add   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad023='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq035"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar030"    #PGS(D)-10500-add   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad024='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq036"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar031"    #PGS(D)-10500-add   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad025='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq037"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar032"    #PGS(D)-10500-add   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
   ...
   IF l_glad026='Y' THEN
      ...
      LET g_postsql1 = g_postsql1,",glaq038"    #PGS(D)-10500-add
      LET g_postsql2 = g_postsql2,",glar033"    #PGS(D)-10500-add   ELSE
      LET r_sql=r_sql,",''"
      LET r_sql1=r_sql1,",''"
   END IF
