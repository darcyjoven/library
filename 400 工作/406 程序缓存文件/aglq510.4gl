...
#+ Standard Version.....: SD版次:102(2020-04-22 16:41:40), PR版次:0102(2021-05-25 21:33:11)
#+ Customerized Version.: SD版次:(), PR版次:0000(1900-01-01 00:00:00)
#+ Build......: 000372
#+ Filename...: aglq510
#+ Description: 總帳與子系統自動勾稽
#+ Creator....: 02599(2015-10-29 15:07:19)
#+ Modifier...: 08172 -SD/PR- 14586
...
#200115-00006#1  2020/01/15  By 05795   aglq510第一页签都显示核对ok无差异，但是差异页签还是有资料
#200203-00006#1  2020/02/04  By 08729   修正成本模組檢核時，因開帳時不會有底稿資料，沒有將開帳的總帳金額給底稿的問題
#190930-00038#1  2020/02/18  By 13656   产生axct705的条件:差异转出金额xccd304<>'4' 已修改为xccd304<>'0'， xcch304<>'4' 已修改为xcch304<>'0'
#200310-00027#1  2020/03/10  By 13067   差异页签，原币金额由SUM(glat113-glat114)修改为SUM(glat103-glat104)
#200310-00062#1  2020/03/11  By 00537   从axci200抓单别绑定科目逻辑段改写，原来的逻辑无法把不同单别对应的科目区分开
#200316-00017#1  2020/03/16  By 07804   修正應付模組-計算總帳金額正常餘額類型判斷錯誤，導致[錯誤明細]頁簽總帳負數跟子系統正數，造成差異金額翻倍
#200323-00010#1  2020/03/23  By 05795   S-FIN-1003=4,就不判断axct706
#200331-00022#1  2020/03/31  By 00537   aglq510参考其他Q作业，加上“含审计调整凭证”的勾选项，当勾选项=Y时，总账包含AD类凭证金额，当勾选项=N时，总账不包含AD类凭证金额。默认不勾选
#                                       勾选“含审计调整凭证”时，总账自行录入检核页签增加显示当期已过帐的审计调整凭证，增加来源码栏位，区分GL和AD类凭证
#200407-00011#1  2020/04/22  by 08172   1.画面增加'总账是否包含未审核及未过账凭证',为Y时总账来源增加未审核和已审核的aglt310单身glaq
#                                       2.子系统取值前，还未月结的的账款区间，若总账关账日期小于查询期别最后一天，自动执行查询模组月结作业
#200509-00026#1  2020/05/19  By 05795   子系统和总账差异页签数据会重复
#200605-00032#1  2020/06/12  By 13132   1.修改CREATE TEMP TABLE aglq510_tmp01 : flag 型態為chr2
#                                       2.#190416-00021#1 ,mark掉原有的SQL ,造成 -201錯誤 , 應補回條件 : " ON apcb002=pmdtdocno AND apcb003=pmdtseq ",
#200725-00001#1  2020/07/27  By 11234   aglq510错误明细页签中排除付讫产生账务分录为N的单据
#200924-00032#1  2020/09/27  By 13656   修正rownum写法
#201104-00027#1  2020/11/06  By 11234   底稿aglp590產生的統計額,針對成本的時候,aglq510抓取也應該比照總帳抓取本幣
#200722-00027#1  2020/11/09  By 05795   去掉AND xccd002=xcce002条件。
#201117-00029#1  2020/11/18  By 11234   修正子系统与总账差异明细页签串查错误问题
#210118-00024#1  2021/01/19  By 05795   没有入库就结案,但aglq510_5pre_chk6_3会捞出这两张委外采购单，需要排除. 最后插入实体表的时候是记录的201不是2011,所以2011换成201
#210204-00013#1  2021/02/05  By 11234   在调用资金月结作业前，要先判断年度期别最后一天大于资金模组关账日
#210217-00014#1  2021/02/18  By 14586   檢查axct703是否產生帳務資料卻未產生的條件,原本只排除來源為4的資料,改為排除來源為3跟4的資料
#210303-00038#1  2021/03/04  By 11234   总账来源往来账款时，不管是否启用客商核算项，都会将客商写入xrea，但写入glar时要判断核算项是否开启
#210316-00036#1  2021/03/17  By 11234   参照#191111-00027#1解法，错误明细中AP/AR子系统与总账分录依账款单性质判断正负
#210318-00005#1  2021/03/18  By 11234   AP检查的sql也加上正负值的处理
#210318-00043#1  2021/03/19  By 11234   修正agli161存货类科目维护料号，在计算在制科目时取不到在制金额的问题
#210510-00040#1  2021/05/17  By 14929   报错信息内容不正确
#201201-00064#2  2021/05/25  By 14586   aglq* 有帳套條件則科目開窗應多串glad
#end add-point
...
PRIVATE TYPE type_g_glar_d RECORD
   ...
   soamt LIKE type_t.num25_6, 
   doamt LIKE type_t.num25_6, 
   zoamt LIKE type_t.num25_6, 
   samt LIKE type_t.num25_6, 
   damt LIKE type_t.num25_6, 
   zamt LIKE type_t.num25_6, 
...
#171212-00013#2 add s---
 TYPE type_g_glaq5_d RECORD
   ...
   glap007_desc LIKE gzcbl_t.gzcbl004,    #200331-00022#1 add
...
DEFINE tm            RECORD 
       ...
       chk_fm        LIKE type_t.chr1,  #200331-00022#1 add ,
       chk_ad        LIKE type_t.chr1   #200331-00022#1 add
      ,chk_unconf    LIKE type_t.chr1   #200407-00011#1 add 
       END RECORD
...
MAIN
   ...
   DROP TABLE aglq510_tmp #ODI-ORA#


   DROP TABLE aglq510_tmp01 #180716-00006#1 add #ODI-ORA#
...
PRIVATE FUNCTION aglq510_init()
   ...
   #200331-00022#1 --begin
   CALL cl_set_comp_visible("glap007_desc",FALSE)   #200331-00022#1
#200331-00022#1 --end
   #end add-point
...
#                        END IF 
#                     END IF
                     #201117-00029#1 mark(e)
                     #201117-00029#1 add(s)
                     IF NOT cl_null(g_glar4_d[l_ac].glapdocno) THEN
                        CALL s_aooi200_fin_get_slip(g_glar4_d[l_ac].glapdocno) RETURNING l_success,l_slip
                        SELECT oobx004 INTO l_gzza001 FROM oobx_t
                         WHERE oobxent = g_enterprise AND oobx001 = l_slip
                        IF l_gzza001 <> 'MULTI' THEN
                           LET la_param.prog = l_gzza001
                           LET la_param.param[1] = tm.glaald
                           LET la_param.param[2] = g_glar4_d[l_ac].glapdocno
                           LET ls_js = util.JSON.stringify(la_param)
                           CALL cl_cmdrun_wait(ls_js)
                        ELSE
                           INITIALIZE g_errparam TO NULL 
...
#                     END IF
                     #201117-00029#1 mark(e)
                     #201117-00029#1 add(s)
                     IF NOT cl_null(g_glar4_d[l_ac].xrea005) THEN
                        CALL s_aooi200_fin_get_slip(g_glar4_d[l_ac].xrea005) RETURNING l_success,l_slip
                        SELECT oobx004 INTO l_gzza001 FROM oobx_t 
                         WHERE oobxent = g_enterprise AND oobx001 = l_slip
                        IF l_gzza001 <> 'MULTI' THEN
                           LET la_param.prog = l_gzza001
                           IF l_gzza001 = 'axrq342' OR l_gzza001 = 'axrq343' THEN
                              LET la_param.param[2] = tm.glaald
                              LET la_param.param[3] = g_glar4_d[l_ac].xrea005
                           ELSE
                              LET la_param.param[1] = tm.glaald
                              LET la_param.param[2] = g_glar4_d[l_ac].xrea005
                           END IF
                           LET ls_js = util.JSON.stringify(la_param)
                           CALL cl_cmdrun_wait(ls_js)
                        ELSE
      ...
      CONSTRUCT g_wc_table ON glar001,glar009
       ...
       #單身一般欄位開窗相關處理
         ...
         ON ACTION controlp INFIELD b_glar001
            ...
            #201201-00064#2 add--s
            LET g_qryparam.where = "glac001 = '",g_glaa004,"' AND  glac003 <>'1' AND glac006 = '1'",
                                   " AND glac002 IN (SELECT glad001 FROM glad_t,glac_t WHERE glad001= glac002 AND gladld='",tm.glaald,"' ",
                                   "                    AND gladent=",g_enterprise," AND gladent = glacent AND gladstus = 'Y' )" 
            #201201-00064#2 add--e
...
#                    tm.chk_fa,tm.chk_nm,tm.chk_xc,tm.chk_fm   #190221-00009#1 mark
                    tm.chk_fa,tm.chk_nm,tm.chk_xc,tm.chk_fm,tm.chk_show,tm.chk_ad   #190221-00009#1 add    #200331-00022#1 add tm.chk_ad
                   ,tm.chk_unconf  #200407-00011#1 add
         ...
         BEFORE INPUT
            ...
            IF cl_null(tm.chk_ad) THEN LET tm.chk_ad = 'N' END IF          #200331-00022#1 add 
            IF cl_null(tm.chk_unconf) THEN LET tm.chk_unconf = 'N' END IF  #200407-00011#1 add            
...
#200331-00022#1 --begin
        ON CHANGE chk_ad
            IF tm.chk_ad MATCHES '[yY]' THEN
               CALL cl_set_comp_visible("glap007_desc",TRUE)
            ELSE
               CALL cl_set_comp_visible("glap007_desc",FALSE)
            END IF 
#200331-00022#1 --end
...
#  LET ls_wc = g_wc, " AND ", g_wc2, " AND ", g_wc_filter, cl_sql_auth_filter()   #(ver:40) add cl_sql_auth_filter()   #171117-00042 mark
  ...
  #LET ls_wc = g_wc, " AND ", g_wc2, " ", cl_sql_auth_filter()   #(ver:40) add cl_sql_auth_filter()   #171117-00042 modify   #200917-00047 mark
   ...
   LET ls_sql_rank = "SELECT '','',glar001,'',glar009,'','','','','','','',''  ,DENSE_RANK() OVER( ORDER BY glarld,glar001,glar002,glar003,glar004) AS RANK FROM glar_t",
   ...
   LET ls_sql_rank = ls_sql_rank, cl_sql_add_filter("glar_t"),
                     " ORDER BY glarld,glar001,glar002,glar003,glar004"
...
#  LET g_sql = "SELECT COUNT(1) FROM (",ls_sql_rank,")"   #200804-00006 mark
   ...
   FOREACH b_fill_curs INTO g_glar_d[l_ac].sel,g_glar_d[l_ac].sys,g_glar_d[l_ac].glar001,g_glar_d[l_ac].glar001_desc,g_glar_d[l_ac].glar009,g_glar_d[l_ac].soamt,g_glar_d[l_ac].doamt,g_glar_d[l_ac].zoamt,g_glar_d[l_ac].samt,g_glar_d[l_ac].damt,g_glar_d[l_ac].zamt,g_glar_d[l_ac].diff,g_glar_d[l_ac].result 
...
PRIVATE FUNCTION aglq510_filter()
   ...
   DIALOG ATTRIBUTES(UNBUFFERED,FIELD ORDER FORM)
      ...
      CONSTRUCT g_wc_filter ON glar001,glar009
         ...
         AFTER FIELD b_glar001
            #add-point:AFTER FIELD b_glar001 name="construct.c.filter.af.pages.b_glar001"
         ...
         AFTER FIELD b_glar009
            #add-point:AFTER FIELD b_glar009 name="construct.c.filter.af.pages.b_glar009"
...
PRIVATE FUNCTION aglq510_create_temp_table()
   ...
   DROP TABLE aglq510_tmp01 #ODI-ORA#
...
#      flag          LIKE type_t.chr1 )      #标记错误类型（用于排序）  #200605-00032#1 mark
      flag          LIKE type_t.chr2 )      #200605-00032#1 add
   #180716-00006#1 add-e
   #190516-00019#1 add -s
   #中间层暂存档

   DROP TABLE aglq510_tmp02 #ODI-ORA#
...
PRIVATE FUNCTION aglq510_get_data()
   ...
   #190516-00019#1 add -e
   DEFINE l_glad027           LIKE glad_t.glad027   #200115-00006#1---add
   #200407-00011#1 add -s
   DEFINE la_param            RECORD
          prog                STRING,
          actionid            STRING,
          background          LIKE type_t.chr1,
   ...
   DEFINE la_param1           RECORD
          ...
          xrea001_1           LIKE xrea_t.xrea001,   
          xrea002_1           LIKE xrea_t.xrea002,
          wc                  STRING
                              END RECORD
   #aapp910
   ...
   DEFINE l_lastday           LIKE glav_t.glav004
   DEFINE ls_js               STRING
   #200407-00011#1 add -e
   DEFINE l_sfin4007          LIKE nmba_t.nmbadocdt  #210204-00013#1 add  
   #210303-00038#1 add(s)
   DEFINE l_glar016           LIKE glar_t.glar016
   DEFINE l_glad010           LIKE glad_t.glad010
   #210303-00038#1 add(e)
   ...
   #年度期别最后一天
   CALL s_fin_date_get_lastday(tm.glaald,'',tm.glar002,tm.glar003,'') RETURNING g_sub_success,l_lastday
   #執行前先判斷總帳關帳日glaa013<查詢期別最後一天  自動執行試結後查詢
   #AFA不适合试结，暂不处理
   #AXC太复杂不处理，且AXC是先算成本再关账
   IF l_glaa013 < l_lastday THEN
      #AXR-axrp910
      IF tm.chk_ar='Y' THEN
         #axrp910
         LET la_param.prog = "axrp910" 
         LET la_param.background = "Y"
         LET la_param1.xreasite  = tm.glaacomp
         LET la_param1.xreald    = tm.glaald
         LET la_param1.xrea001   = tm.glar002
         LET la_param1.xrea002   = tm.glar003
         LET la_param1.wc        = " 1=1 "
      ...
      IF tm.chk_ap='Y' THEN
         #axrp910
         LET la_param.prog = "aapp910" 
         LET la_param.background = "Y"
         LET la_param2.ooef001   = tm.glaacomp
         LET la_param2.xreald    = tm.glaald
         LET la_param2.xrea001   = tm.glar002
         LET la_param2.xrea002   = tm.glar003
         LET la_param2.wc        = " 1=1 "
         LET la_param.param[1] = util.JSON.stringify(la_param2)
         LET ls_js = util.JSON.stringify(la_param)
         CALL cl_cmdrun_wait(ls_js)
      END IF
      #ANM-anmp830,anmp850
      IF tm.chk_nm='Y' THEN
         #210204-00013#1 add(s)
         CALL cl_get_para(g_enterprise,tm.glaacomp,'S-FIN-4007') RETURNING l_sfin4007
         IF l_sfin4007 < l_lastday THEN
         #210204-00013#1 add(e)
         #anmp830
         LET la_param.prog = "anmp830" 
         LET la_param.background = "Y"
         LET la_param3.nmbxcomp  = tm.glaacomp
         LET la_param3.nmbx001   = tm.glar002
         LET la_param3.nmbx002   = tm.glar003
         LET la_param3.wc        = " 1=1 "
         LET la_param.param[1] = util.JSON.stringify(la_param3)
         LET ls_js = util.JSON.stringify(la_param)
         CALL cl_cmdrun_wait(ls_js)
         #anmp850
         LET la_param.prog = "anmp850" 
         LET la_param.background = "Y"
         LET la_param4.nmcysite  = tm.glaacomp
         LET la_param4.nmcyld    = tm.glaald
         LET la_param4.nmcy001   = tm.glar002
         LET la_param4.nmcy002   = tm.glar003
         LET la_param4.wc        = " 1=1 "
         LET la_param.param[1] = util.JSON.stringify(la_param4)
         LET ls_js = util.JSON.stringify(la_param)
         CALL cl_cmdrun_wait(ls_js)
         END IF  #210204-00013#1 add
      END IF
   END IF
   #200407-00011#1 add -e
   #抓取底稿余额
   ...
   #170324-00097#1-----e
   #170324-00097#1-----s
   #LET l_sql="SELECT SUM(glat113-glat114),SUM(glat113-glat114) ",  #200310-00027#1 mark
   LET l_sql="SELECT SUM(glat103-glat104),SUM(glat113-glat114) ",  #200310-00027#1 add
             "  FROM glat_t ",
             " WHERE glatent=",g_enterprise," AND glatld='",tm.glaald,"'",
             "   AND glat001=",tm.glar002," AND glat002<=",tm.glar003,
             "   AND glat007=? "
   ...
   #200203-00006#1 add(s)
   LET l_sql = " SELECT MIN(glat001) FROM glat_t ",
               "  WHERE glatent = ",g_enterprise," ",
               "    AND glatld = ? ", 
               "    AND glat003 = ? "
   PREPARE sel_glatp5 FROM l_sql
   #200203-00006#1 add(e)
   ...
   #200115-00006#1-- add --S--判断一下这个科目是否开启账款对象管理
   #从agli031取科目设置
   LET l_sql = " SELECT glad027 ",
               "       ,glad010 ",  #210303-00038#1 add
               "   FROM glad_t ",
               "  WHERE gladent = ",g_enterprise,
               "    AND gladld  = '",tm.glaald CLIPPED,"'",
               "    AND glad001 = ? "
   PREPARE aglq510_sel_glad027_pb FROM l_sql
   DECLARE aglq510_sel_glad027_cs CURSOR FOR aglq510_sel_glad027_pb
   #200115-00006#1 add --E--   
   
   #抓取总账余额
   #200407-00011#1 mark -s
...
#             "   AND glar001=? AND glar009=? "
   #200407-00011#1 mark -e
   #200407-00011#1 add -s
   LET l_sql="SELECT SUM(glar010-glar011) oamt,SUM(glar005-glar006) amt ",
             "  FROM glar_t ",
             " WHERE glarent=",g_enterprise," AND glarld='",tm.glaald,"'",
             "   AND glar002=",tm.glar002," AND glar003<=",tm.glar003,
             "   AND glar001=? AND glar009=? "
   #总账是否包含未审核及未过账凭证为Y时总账来源增加未审核及未过账凭证
   IF tm.chk_unconf = 'Y' THEN
      LET l_sql = l_sql CLIPPED," UNION ",
                                "SELECT SUM(CASE WHEN glaq003=0 THEN glaq010*-1 ELSE glaq010 END) oamt,SUM(glaq003-glaq004) amt ",
                                "  FROM glaq_t,glap_t ",
      ...
      LET l_sql = " SELECT SUM(oamt),SUM(amt) ",
                  "   FROM (",l_sql,") "
   END IF
   #200407-00011#1 add -e
   ...
   #190516-00019#1 add -s
   #差异页签用
#   LET l_sql = l_sql CLIPPED," AND glar017 = ?"  #200407-00011#1 mark
   #200407-00011#1 add -s
   LET l_sql="SELECT SUM(glar010-glar011) oamt,SUM(glar005-glar006) amt ",
             "  FROM glar_t ",
             " WHERE glarent=",g_enterprise," AND glarld='",tm.glaald,"'",
   ...
   IF tm.chk_unconf = 'Y' THEN
      ...
      LET l_sql = " SELECT SUM(oamt),SUM(amt) ",
                  "   FROM (",l_sql,") "
   END IF
   PREPARE aglq510_sel_glar_pr12 FROM l_sql
   ...
   IF tm.chk_unconf = 'Y' THEN
      LET l_sql = l_sql CLIPPED,"  UNION ",
                                ...
                                "    AND glap002 = ",tm.glar002," AND glap004 <= ",tm.glar003,
                                "    AND glaq002 = ? AND glaq005 = ? ",
                                "    AND glaq021 = ? AND glaq022 = ? "
      LET l_sql = " SELECT SUM(oamt),SUM(amt) ",
                  "   FROM (",l_sql,") "
   END IF
   PREPARE aglq510_sel_glar_pr13 FROM l_sql
   #210303-00038#1 add(e)
   
   #180428-00012#1 add--s 总账扣除审计部分
   #200407-00011#1 add -s 
   IF tm.chk_unconf = 'Y' THEN  #总账是否包含未审核及未过账凭证为Y时审计也要扣除未审核和审核的审计
   ...
   #200407-00011#1 add -e
      ...
      LET l_sql="SELECT SUM(CASE WHEN glaq003=0 THEN glaq010*-1 ELSE glaq010 END),SUM(glaq003-glaq004) ",
                ...
                " WHERE glapent=glaqent AND glapld=glaqld AND glapdocno=glaqdocno ",
                "   AND glapent=",g_enterprise," AND glapld='",tm.glaald,"'",
                "   AND glapstus = 'S' AND glap007 = 'AD' ",              #200331-00022#1    glapstus <> 'X' --> glapstus = 'S'
                "   AND (glap002 < ",tm.glar002," OR (glap002=",tm.glar002," AND glap004<=",tm.glar003,") ) ",
                "   AND glaq002=? AND glaq005=? "  #会计科目，交易币种
   END IF  #200407-00011#1 add
...
#             "   AND glar001=? "
   #200407-00011#1 mark -e
   #200407-00011#1 add -s
   LET l_sql="SELECT SUM(glar005-glar006) oamt,SUM(glar005-glar006) amt ",
             "  FROM glar_t ",
             " WHERE glarent=",g_enterprise," AND glarld='",tm.glaald,"'",
             "   AND glar002=",tm.glar002," AND glar003<=",tm.glar003,
             "   AND glar001=? "
...
#                "   AND xrea001=",tm.glar002," AND xrea002<=",tm.glar003, #160728-00003#1 mark
      ...
      FOREACH aglq510_sel_xrea_cs1 INTO l_xrea003,l_glar001,l_glar009,l_xrea103,l_xrea113
         ...
         #200407-00011#1 add -s
         IF tm.chk_unconf = 'Y' THEN
            EXECUTE aglq510_sel_glar_pr1 USING l_glar001,l_glar009,l_glar001,l_glar009
         ...
         #180716-00006#2 mark-s
          ...
          IF tm.chk_ad = 'N' THEN      #200331-00022#1 add
          #抓取总账需扣掉的部分--审计调整
...
#160728-00003#1--mark--end
      ...
      FOREACH aglq510_sel_xrea_cs2 INTO l_xrea003,l_glar001,l_glar009,l_glar017,l_xrea103,l_xrea113
         ...
         #抓取总账余额
         #200115-00006#1 add --S--
         EXECUTE aglq510_sel_glad027_cs USING l_glar001 into l_glad027
                                                            ,l_glad010  #210303-00038#1 add
         #210303-00038#1 mark(s)
#         IF l_glad027 = 'Y' THEN
#         #200115-00006#1 add --E--
#            #200407-00011#1 add -s
#            IF tm.chk_unconf = 'Y' THEN
#               EXECUTE aglq510_sel_glar_pr11 USING l_glar001,l_glar009,l_glar017,l_glar001,l_glar009,l_glar017
#                                              INTO l_glar010,l_glar005
#            ELSE
#            #200407-00011#1 add -e
...
#            END IF  #200407-00011#1 add
#         END IF
         #210303-00038#1 mark(e)
         #210303-00038#1 add(s)
         #此段就是在往来账的检核中，子系统xrea009，xrea010一定是有写入客商的
         #但总账glar016，glar017是根据agli031设定的客商核算项是否开启来判断是否写入glar
         IF l_glad010 = 'N' AND l_glad027 = 'Y' THEN  #启用账款客商管理
            IF tm.chk_unconf = 'Y' THEN
               EXECUTE aglq510_sel_glar_pr11 USING l_glar001,l_glar009,l_glar017,l_glar001,l_glar009,l_glar017
                                              INTO l_glar010,l_glar005
            ELSE
         ...
         IF l_glad010 = 'Y' AND l_glad027 = 'N' THEN  #启用收付款客商管理
            IF tm.chk_unconf = 'Y' THEN
               EXECUTE aglq510_sel_glar_pr12 USING l_glar001,l_glar009,l_glar016,l_glar001,l_glar009,l_glar016
                                              INTO l_glar010,l_glar005
            ELSE
               EXECUTE aglq510_sel_glar_pr12 USING l_glar001,l_glar009,l_glar016  
                                              INTO l_glar010,l_glar005
            END IF                              
         END IF
         
         IF l_glad010 = 'Y' AND l_glad027 = 'Y' THEN  #两者都启用
            IF tm.chk_unconf = 'Y' THEN
               EXECUTE aglq510_sel_glar_pr13 USING l_glar001,l_glar009,l_glar016,l_glar017,l_glar001,l_glar009,l_glar016,l_glar017
                                              INTO l_glar010,l_glar005
            ELSE
               EXECUTE aglq510_sel_glar_pr13 USING l_glar001,l_glar009,l_glar016,l_glar017 
                                              INTO l_glar010,l_glar005
            END IF                              
         END IF
         
         IF l_glad010 = 'N' AND l_glad027 = 'N' THEN  #未启用客商管理
            IF tm.chk_unconf = 'Y' THEN
               EXECUTE aglq510_sel_glar_pr1 USING l_glar001,l_glar009,l_glar001,l_glar009
                                             INTO l_glar010,l_glar005
            ELSE
               EXECUTE aglq510_sel_glar_pr1 USING l_glar001,l_glar009  
                                             INTO l_glar010,l_glar005
            END IF
            #xrea中一定是有客商的，此处要重新对xrea103赋值，不group by核算项
            LET l_sql = "  SELECT SUM(CASE WHEN xrea004 LIKE '2%' OR xrea004 = '02' OR xrea004 = '04' OR xrea004 = '06' THEN (xrea103-xrea107)*-1 ELSE (xrea103-xrea107) END), ", 
                        "         SUM(CASE WHEN xrea004 LIKE '2%' OR xrea004 = '02' OR xrea004 = '04' OR xrea004 = '06' THEN (xrea113-xrea117)*-1 ELSE (xrea113-xrea117) END) ",
                        "    FROM xrea_t ",
                        "   WHERE xreaent = ",g_enterprise," AND xreald = '",tm.glaald,"' ",
                        "     AND xrea001 = ",tm.glar002," AND xrea002 =",tm.glar003,  
                        ...
                        "     AND xrea019 = ? ",
                        "     AND xrea100 = ? "
            PREPARE aglq510_sel_xrea_pr3 FROM l_sql
            EXECUTE aglq510_sel_xrea_pr3 USING l_glar001,l_glar009 INTO l_xrea103,l_xrea113      
         END IF
         #210303-00038#1 add(e)
         #200115-00006#1 add --S--
         IF SQLCA.sqlcode THEN
   ...
   IF tm.chk_fa='Y' THEN
      ...
      LET l_sql="SELECT acc,faan010,SUM(amt) ",
                "  FROM (",
                #                财产科目    /币别   /本币余额
                "         SELECT faaj023 acc,faan010,SUM(faan014) amt ", #PGS(D)-10043-mod 移除左括號
                "           FROM faan_t,faaj_t ",
                ...
                "            AND ",l_wc,
                "          GROUP BY faaj023,faan010 ", #PGS(D)-10043-mod 移除右括號
                "          UNION ",
                #                累折科目    /币别   /本币余额
#                "        (SELECT faaj024 acc,faan010,SUM(faan018) amt ", #170210-00059#1 mark
                "        SELECT faaj024 acc,faan010,SUM(CASE WHEN faaj023=faaj024 THEN faan018 *-1 ELSE faan018 END) amt ", #170210-00059#1 add #PGS(D)-10043-mod 移除左括號
                "           FROM faan_t,faaj_t ",
                "          WHERE faanent=faajent AND faanld=faajld ",
                "            AND faan003=faaj037 AND faan004=faaj001 AND faan005=faaj002",
                "            AND faanent=",g_enterprise," AND faanld='",tm.glaald,"'",
                ...
                "            AND faaj048 <> '1' ",   #170208-00010#1 add lujh
                 "           AND faan007 NOT IN ('5','6') ",#170927-00026#2 xul add 
                "            AND ",l_wc1,
                "          GROUP BY faaj024,faan010 ", #PGS(D)-10043-mod 移除右括號
      ...
      FOREACH aglq510_sel_faan_cs1 INTO l_glar001,l_glar009,l_xrea113
         ...
         IF g_glaa121 = 'Y' THEN
            ...
            IF cl_null(l_glat103) THEN LET l_glat103 = 0 END IF
            IF cl_null(l_glat113) THEN LET l_glat113 = 0 END IF
            LET l_glat103 = l_glat113       #200331-00022#1 add
...
#            LET l_glar010 = l_glar010 * -1 #161125-00026#1 mark
         ...
         EXECUTE aglq510_sel_glap_pr0 USING l_glar001,l_glar009
          ...
          END IF
          LET l_glaq010 = l_glaq003     #200331-00022#1 add
          LET l_glar010 = l_glar010 - l_glaq010
          LET l_glar005 = l_glar005 - l_glaq003
          END IF                  #200331-00022#1 add 
...
#      LET l_wc1 = cl_replace_str(l_wc1,'glar009','glat100') #180716-00006#3 add  #200407-00011#1 mark
      LET l_wc1 = cl_replace_str(l_wc1,'glar009','nmcy100') #200407-00011#1 add
      #nmcy(票据期统计档)，还没有写，故暂时不处理,暂时按照固资的写法预留抓取nmcy_t
      LET l_sql="SELECT acc,curr,SUM(amt1),SUM(amt2) ",
                "  FROM (",
                #              账户对应科目   /币别        /原币余额                 /本币余额
                "         SELECT glab005 acc,nmbx100 curr,SUM(nmbx103-nmbx104) amt1,SUM(nmbx113-nmbx114) amt2 ", #PGS(D)-10043-mod 移除左括號
                "           FROM nmbx_t,glab_t ",
                ...
                "            AND ",l_wc,
                "          GROUP BY glab005,nmbx100 ", #PGS(D)-10043-mod 移除右括號
...
#                           " AND ",l_wc1,
#                          " GROUP BY glab005,nmcy100) tsed",
                #181009-00011#1 mark-e                          
                #170918-00041#1-----e 
               #181009-00011#1 add-s
                "         UNION ",
                       "  SELECT nmcy040 acc,nmcy100 curr,SUM(nmcy103) amt1,SUM(nmcy113) amt2 ", #PGS(D)-10043-mod 移除左括號
                          " FROM nmcy_t  ", 
                        "  WHERE nmcyent = ",g_enterprise," ",
                           ...
                           " AND nmcycomp ='",tm.glaacomp,"' ",
                           " AND ",l_wc1,
                          " GROUP BY nmcy040,nmcy100 ", #PGS(D)-10043-mod 移除右括號
                "         UNION ",
                       "  SELECT nmcy040 acc,nmcy100 curr,SUM(nmcy103) amt1,SUM(nmcy113) amt2 ", #PGS(D)-10043-mod 移除左括號
                          " FROM nmcy_t  ", 
                        "  WHERE nmcyent = ",g_enterprise," ",
...
#190313-00030#1 --end
      ...
      #210318-00043#1 add(s)
      LET l_glcc001 = NULL
      LET l_glcc011 = NULL
      LET l_glar001 = NULL
      LET l_glcc016 = NULL
      #210318-00043#1 add(e)
...
#                      "             AND xccdent=xcceent AND xccdld=xcceld AND xccd001=xcce001 AND xccd002=xcce002 AND xccd003=xcce003 ", #200722-00027#1--mark
                      "             AND xccdent=xcceent AND xccdld=xcceld AND xccd001=xcce001 AND xccd003=xcce003 ", #200722-00027#1--add
...
#            END IF   #170711-00051#1 add lujh       #190613-00046#1 mark             
      ...
      #180815-00022#1 add-s
      #1）抓出axci200该账套下在制科目 
#200310-00062#1   xcbu的处理逻辑为xcbu001单别对应xcbu004,xcbu006,xcbu007,xcbu008,xcbu009,xcbu010,xcbu011,xcbu012这一组科目组合，最后记录到aglq510_tmp时，用xcbu001单据别分组，l_glcc011记录xcbu001
#原逻辑没有按xcbu001分组金额，导致所有的科目抓出的都是同一个总金额，现在改为按xcbu001单别分组，每一组都是同一个单别下一组材料，人工制费科目的金额
      LET l_sql="SELECT DISTINCT xcbu001,xcbu004,xcbu006,xcbu007,xcbu008,xcbu009,xcbu010,xcbu011,xcbu012 FROM xcbu_t ",     #200310-00062#1  '' --> xcbu001
...
#         #181009-00011#1 mark-e
         ...
         LET l_sql="SELECT curr,SUM(amt2),SUM(amt3),SUM(amt4),SUM(amt5),SUM(amt6),SUM(amt7),SUM(amt8) ",   
                   ...
                   "         (SELECT xcce010 curr,SUM(xcce902b) amt2,SUM(xcce902c) amt3,SUM(xcce902d) amt4,SUM(xcce902e) amt5,SUM(xcce902f) amt6,SUM(xcce902g) amt7,SUM(xcce902h) amt8",
                   "            FROM xccd_t,xcce_t ",
#                   "           WHERE xccdent=xcceent AND xccdld=xcceld AND xccd001=xcce001 AND xccd002=xcce002 AND xccd003=xcce003 ",  #200722-00027#1--mark
                   "           WHERE xccdent=xcceent AND xccdld=xcceld AND xccd001=xcce001 AND xccd003=xcce003 ",  #200722-00027#1--add
                   "             AND xccd004=xcce004 AND xccd005=xcce005 AND xccd006=xcce006 ",
                   "             AND xccdent=",g_enterprise," AND xccdld='",tm.glaald,"' ",
                   "             AND xccd001='1' AND xccd004=",tm.glar002," AND xccd005=",tm.glar003,
                   "             AND xccdcomp='",tm.glaacomp,"' ",
                   "             AND ",l_wc2,
                   "             AND xcce007 = 'DL+OH+SUB'",
#200310-00062#1 --begin   axci200里可以用单别对应不同科目组合，不需要特别排除委外，只要委外单别设的科目与一般在制能区分开就可以
#190716-00012#1 --begin    #这里只抓一般在制的拆件
#                  "             AND xccd006 IN (SELECT sfaadocno FROM sfaa_t WHERE sfaaent = ",g_enterprise,
#                  "                                 AND sfaa057 <> '2') ",
#190716-00012#1 --end
#200310-00062#1 --end
                   "             AND  EXISTS (SELECT 1 FROM xcbu_t WHERE xcbuent=xccdent AND xcbuld=xccdld AND xcbu001=substr(xccd006,",g_str_pos,',',g_slip_len,") AND xcbu001 ='",l_glcc011,"' ) ",      #200310-00062#1 add xcbu001 = l_glcc011
                   "           GROUP BY xcce010 ",
                   "         )",
                   "         UNION ",
                   "         (SELECT xcci010 curr,SUM(xcci902b) amt2,SUM(xcci902c) amt3,SUM(xcci902d) amt4,SUM(xcci902e) amt5,SUM(xcci902f) amt6,SUM(xcci902g) amt7,SUM(xcci902h) amt8",
...
#190716-00012#1 --end
#200310-00062#1 --end
                   "             AND  EXISTS (SELECT 1 FROM xcbu_t WHERE xcbuent=xcchent AND xcbuld=xcchld AND xcbu001=substr(xcch006,",g_str_pos,',',g_slip_len,") AND xcbu001 ='",l_glcc011,"' ) ",      #200310-00062#1 add xcbu001 = l_glcc011 
                   "           GROUP BY xcci010 ",
                   "         )",
                   "       ) ",
                   " GROUP BY curr ",
...
#190613-00046#1 --begin  #agli161里，委外加工费设在在制页签，所以这里glcc004是null，要从在制页签按同成本分群抓在制的加工费科目
         LET l_sql="SELECT DISTINCT glcc004 FROM glcc_t ",  
                   " WHERE glccent=",g_enterprise," AND glccld='",tm.glaald,"'",
                   "   AND glcc001 IN ('3')  ",
                   "  AND rownum = 1 ", #200605-00032#1 add
...
#190902-00053#1 --begin
            IF SQLCA.sqlcode = 100 THEN   #如果用委外的成本分群抓不到在制页签的，就再判断在制里是否有成本分群是*的
               ...
               IF SQLCA.sqlcode THEN
                  INITIALIZE g_errparam TO NULL
#                 LET g_errparam.code = SQLCA.sqlcode        #210510-00040#1---mark
                  LET g_errparam.code = "asf-00517"          #210510-00040#1---add
...
#                      "             AND xccdent=xcceent AND xccdld=xcceld AND xccd001=xcce001 AND xccd002=xcce002 AND xccd003=xcce003 ",   #200722-00027#1--mark
                      ...
                      "             AND xccd006 IN (SELECT sfaadocno FROM sfaa_t WHERE sfaaent = ",g_enterprise,
                      "                                 AND sfaa057 = '2') ",
#                      "             AND NOT EXISTS (SELECT 1 FROM xcbu_t WHERE xcbuent=xccdent AND xcbuld=xccdld AND xcbu001=substr(xccd006,",g_str_pos,',',g_slip_len,") ) ",    #200310-00062#1 mark
                      "           GROUP BY xcce010 ",
                      "         )",
                      "         UNION ",
                      "         (SELECT xcci010 curr,SUM(xcci902b) amt2,SUM(xcci902c) amt3,SUM(xcci902d) amt4,SUM(xcci902e) amt5,SUM(xcci902f) amt6,SUM(xcci902g) amt7,SUM(xcci902h) amt8",   
                      ...
                      "             AND xcci006 IN (SELECT sfaadocno FROM sfaa_t WHERE sfaaent = ",g_enterprise,
                      "                                 AND sfaa057 = '2') ",
#                      "             AND NOT EXISTS (SELECT 1 FROM xcbu_t WHERE xcbuent=xcchent AND xcbuld=xcchld AND xcbu001=substr(xcch006,",g_str_pos,',',g_slip_len,") ) ",  #180815-00022#1 add   #200310-00062#1 mark
                      "           GROUP BY xcci010 ",
                      "         )",
                      "       ) ",
                      " GROUP BY curr ",
...
#161209-00052#1 add e---      
      ...
      FOREACH aglq510_sel_tmp_cs1 INTO l_glar001,l_glar009,l_xrea113
         ...
         IF g_glaa121 = 'Y' THEN
            ...
            #200203-00006#1 add(s)
            #將190709-00037 mark段移到此處再做修改
            
            #經詢問SA秋玲，因為成本開帳不會有底稿
            #EX 再2019年1月開帳，所以第一期的glat會是 2019年1月
            #所以GL傳票會開再2018/12月 之後做年結，故 2019年第0期glar 會有資料
            #所以底稿資料需再將2018/12的總帳資料加回去
            
            LET l_glar003 = NULL
            LET l_glar003 = NULL
            #所以抓取底稿月結中最小的年度
            EXECUTE sel_glatp5 USING tm.glaald,'XC'
                               INTO l_glar002
            #再用該年度找到最小的期別                   
            EXECUTE sel_glatp4 USING tm.glaald,l_glar002,'XC'
                               INTO l_glar003
            #因要取開帳的GL傳票資料，所以要-1                   
            LET l_glar003 = l_glar003 - 1
            LET l_glar00522 = NULL
            EXECUTE sel_glarp3 USING g_enterprise,tm.glaald,l_glar001,l_glar002,l_glar003
               INTO l_glar00522
            IF cl_null(l_glar00522)THEN LET l_glar00522 = 0 END IF
            
            #LET l_glat103 = l_glat103 + l_glar00522  #201104-00027#1 mark
            LET l_glat103 = l_glat113 + l_glar00522   #201104-00027#1 add
            LET l_glat113 = l_glat113 + l_glar00522
            #200203-00006#1 add(e)
         ...
         #200407-00011#1 add -s
         IF tm.chk_unconf = 'Y' THEN
            EXECUTE aglq510_sel_glar_pr2 USING l_glar001,l_glar001  
                                          INTO l_glar010,l_glar005
         ELSE
         #200407-00011#1 add -e
...
#200331-00022#1 --begin
         IF tm.chk_ad = 'N' THEN      
            ...
            EXECUTE aglq510_sel_glap_pr0 USING l_glar001,l_glar009
             ...
             IF cl_null(l_glaq010) THEN LET l_glaq010 = 0 END IF
             IF cl_null(l_glaq003) THEN LET l_glaq003 = 0 END IF
             LET l_glaq010 = l_glaq003    #成本没有原币，原币的栏位也取本币显示
             IF l_glac008 = '2' THEN
                LET l_glaq010 = l_glaq010 * -1
                LET l_glaq003 = l_glaq003 * -1
             END IF
...
PRIVATE FUNCTION aglq510_b_fill4()
   ...
   IF tm.chk_ar = 'Y' OR tm.chk_ap = 'Y' THEN
      ...
      #200509-00026#1---add---str
      CASE
         WHEN tm.chk_ar = 'Y' AND tm.chk_ap = 'Y' 
            LET g_wc3 ="((xrea003 = 'AR' AND sys = '1') OR ( xrea003 = 'AP' AND sys = '2'))"
         WHEN tm.chk_ar = 'Y' AND tm.chk_ap <> 'Y' 
            LET g_wc3 ="(xrea003 = 'AR' AND sys = '1')"
         WHEN tm.chk_ar <> 'Y' AND tm.chk_ap = 'Y' 
            LET g_wc3 = "(xrea003 = 'AP' AND sys = '2')"   
      END CASE
      #200509-00026#1---add---end
...
#                "   AND xrea001=",tm.glar002," AND xrea002<=",tm.glar003, #160728-00003#1 mark
                "   AND xrea001=",tm.glar002," AND xrea002 =",tm.glar003,  #160728-00003#1 add
                "   AND ",g_wc3,
#                "   AND sys IN ('1','2')",  #200509-00026#1---mark
                "   AND diff IN ('2','3','4','6')",
   ...
   IF tm.chk_fa='Y' THEN
      LET l_sql="SELECT faan004,acc,faan010,SUM(amt) ",
                "  FROM (",
                #               财产编号/资产科目    /币别    /本币余额
                "         SELECT faan004,faaj023 acc,faan010,SUM(faan014) amt ", #PGS(D)-10043-mod 移除左括號
                ...
                "            AND diff IN ('2','3','4','6')",
                "          GROUP BY faan004,faaj023,faan010  ", #PGS(D)-10043-mod 移除右括號
                "          UNION ",
                #                财产编号/累折科目    /币别   /本币余额
#                "        (SELECT faan004,faaj024 acc,faan010,SUM(faan018) amt ", #170210-00059#1 mark
                "         SELECT faan004,faaj024 acc,faan010,SUM(CASE WHEN faaj023=faaj024 THEN faan018 *-1 ELSE faan018 END) amt ", #170210-00059#1 add #PGS(D)-10043-mod 移除左括號
                "           FROM faan_t,faaj_t,aglq510_tmp ",
                "          WHERE faanent=faajent AND faanld=faajld ",
                "            AND faan003=faaj037 AND faan004=faaj001 AND faan005=faaj002",
                ...
                "            AND faan007 NOT IN ('5','6')  AND faaj048 <> '1' ",#170927-00026#2 xul add 
                "            AND diff IN ('2','3','4','6')",
                "          GROUP BY faan004,faaj024,faan010 ", #PGS(D)-10043-mod 移除右括號
                "       ) t51 ", #PGS(D)-10043-add t51 
   ...
   IF tm.chk_nm='Y' THEN
      LET l_sql="SELECT nmbx003,acc,curr,SUM(amt1),SUM(amt2) ",
                ...
                "         SELECT nmbx003,glab005 acc,nmbx100 curr,SUM(nmbx103-nmbx104) amt1,SUM(nmbx113-nmbx114) amt2 ", #PGS(D)-10043-mod 移除左括號
                ...
                "            AND sys ='4'",
                "            AND diff IN ('2','3','4','6')",
                "          GROUP BY nmbx003,glab005,nmbx100  ", #PGS(D)-10043-mod 移除右括號
             #170918-00041#1 mark-----s                
             #   #170322-00040#1 add s---
             #   "         UNION ",
             #   "        (SELECT '',glab005 acc,glat100 curr,SUM(glat103-glat104) amt1,SUM(glat113-glat114) amt2 ",
             ...
             #170918-00041#1 mark-----e
                #170918-00041#1-----s
                "         UNION ",
                "         SELECT '',glab005 acc,nmcy100 curr,SUM(nmcy103) amt1,SUM(nmcy113) amt2 ", #PGS(D)-10043-mod 移除左括號
                ...
                "            AND diff IN ('2','3','4','6')",
                #PGS(D)-02160 memo -S-
                "          GROUP BY glab005,nmcy100  ",  #PGS(D)-10043-mod 移除右括號
                #"          GROUP BY '',glab005,nmcy100  ",  #PGS(D)-10043-mod 移除右括號
                #PGS(D)-02160 memo -E-
                #170918-00041#1-----e                
                "       ) t51 ",  #PGS(D)-10043-add t51                
...
DEFINE l_i        LIKE type_t.num5
   ...
   LET l_sql = " SELECT glaq002,'',glaq005,(SELECT gzcbl004 FROM gzcbl_t WHERE gzcbl001 = '8007' AND gzcbl002 = glap007 AND gzcbl003 = '",g_lang,"') glap007_desc,glaqdocno,glapdocdt,SUM(CASE WHEN glaq003=0 THEN glaq010*-1 ELSE glaq010 END),SUM(glaq003-glaq004) ",        #200331-00022#1 add glap007_desc
               "   FROM glaq_t,glap_t,glad_t ",
               "  WHERE glaqent = glapent AND glaqdocno = glapdocno AND glaqld = glapld "    #200331-00022#1 remove ,
#200331-00022#1 --begin
   IF tm.chk_ad = 'N' THEN
      LET l_sql = l_sql,
                  "    AND gladent = glaqent AND gladld = glaqld AND glad001 = glaq002  ",
                  "    AND glap007 = 'GL' AND glad037 = 'Y' AND glap002 = ",tm.glar002," AND glap004 = ",tm.glar003
   ELSE
      LET l_sql = l_sql,                  
                  "    AND gladent = glaqent AND gladld = glaqld AND glad001 = glaq002  ",
                  "    AND ((glap007 = 'GL' AND glad037 = 'Y') OR glap007 = 'AD') AND glap002 = ",tm.glar002," AND glap004 = ",tm.glar003
   END IF
   #200407-00011#1 add -s
   IF tm.chk_unconf = 'Y' THEN
      LET l_sql = l_sql,
   ...
   #200407-00011#1 add -e
   LET l_sql = l_sql,
#               "    AND glapstus = 'S' ",   #200407-00011#1 mark
...
#200331-00022#1 --end
               ...
               "  GROUP BY glaq002,glaq005,glap007,glaqdocno,glapdocdt",              #200331-00022#1 add glap007
               "  ORDER BY glap007_desc "                                                  #200331-00022#1 add


   PREPARE aglq510_b_fill5_pr FROM l_sql
   DECLARE aglq510_b_fill5_cs CURSOR FOR aglq510_b_fill5_pr
   FOREACH aglq510_b_fill5_cs INTO g_glaq5_d[l_i].glaq002,g_glaq5_d[l_i].glaq002_desc,g_glaq5_d[l_i].glaq005,g_glaq5_d[l_i].glap007_desc,    #200331-00022#1 add glap007_desc
...
#   #180716-00006#2 mark-e
   ...
   LET l_sql=" SELECT sys,glaq002,'',glaq005,glaq022,'',glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003),0),'','','','',0,'','','','','','',SUM(glaq004) ",
             ...
             #190408-00030#1---mark---end--
              ...
              "            AND EXISTS (SELECT 1 FROM aglq510_tmp WHERE glar001=glaq002 AND glar009=glaq005",l_wc3," AND diff NOT IN('0','5') )) t52 ",  #180716-00006#2 add #PGS(D)-10043-add t52
   ...
   LET l_sql=" SELECT sys,glaq002,'',glaq005,glaq022,'',glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003),0),'','','','',0,'','','','','','',SUM(glaq004),glap007 ", #180716-00006#2 add
             ...
             #190408-00030#1---mark---end--
              ...
              #"            AND EXISTS (SELECT 1 FROM aglq510_tmp WHERE glar001=glaq002 AND glar009=glaq005 AND diff NOT IN('0','5') ))",  #180716-00006#2 mark
              "            AND EXISTS (SELECT 1 FROM aglq510_tmp WHERE glar001=glaq002 AND glar009=glaq005",l_wc3," AND diff NOT IN('0','5') )) t51 ",  #180716-00006#2 add #PGS(D)-10043-add t51 
...
#190221-00009#1  add_S      
      IF l_tmp.sys5='2' THEN  #AP
         LET l_tmp.advise  = cl_getmsg('agl-44596',g_dlang)    
      END IF 
      IF l_tmp.sys5='1' THEN  #AR
         LET l_tmp.advise  = cl_getmsg('agl-44618',g_dlang)     
      END IF       
...
#190221-00009#1  add_S      
      IF l_tmp.sys5='2' THEN  #AP
         LET l_tmp.advise  = cl_getmsg('agl-44597',g_dlang)    
      END IF 
      IF l_tmp.sys5='1' THEN  #AR
         LET l_tmp.advise  = cl_getmsg('agl-44620',g_dlang)     
      END IF       
#190221-00009#1 add_E      
   ...
   LET l_sql=" SELECT glgbdocno,NVL(SUM(glgb010),0),NVL(SUM(glgb003-glgb004),0) glgb003 ",
             ...
             "          AND glgbdocno=? AND glgb002=? ) t51 ", #PGS(D)-10043-add t51
   ...
   LET l_sql=" SELECT glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003-glaq004),0) glgb003 ",
             ...
             "          AND glaqdocno=? AND glaq002=? ) t51 ", #PGS(D)-10043-add t51
...
#                  "        CASE WHEN xrca001='21' OR xrca001='25' OR xrca001='26' THEN xrce113  ELSE xrce119 END xrce1191  ",
                  ...
                  "                        AND apdastus = 'Y' GROUP BY apce003,apce004,apce005 ) t51 ",   #180716-00006#2 add #PGS(D)-10043-add t51
...
#                  "                        AND xrcfdocno = xrcbdocno GROUP BY xrcf008,xrcf009,xrcf010 ) tsed",  #180716-00006#2 add    #191125-00033#1---mark
                  "                        AND xrcfdocno = xrcadocno GROUP BY xrcf008,xrcf009,xrcf010 ) t52 ",  #191125-00033#1---add #PGS(D)-10043-add t52
...
#                  "        CASE WHEN xrca001='21' OR xrca001='25' OR xrca001='26' THEN xrce113  ELSE xrce119 END xrce1191  ",
                  ...
                  "                        AND apcedocno = apdadocno ",
                  #"                        AND apdastus = 'Y' )",   #180716-00006#2 mark
                  "                        AND apdastus = 'Y' GROUP BY apce003,apce004,apce005 ) t53 ",   #180716-00006#2 add #PGS(D)-10043-add t53
                  "               ON xrccdocno=apce003  AND xrccseq=apce004 AND xrcc001=apce005 ",
#                  #190514-00012#1---mark----str
#                  "  LEFT OUTER JOIN( ",
#                  #"                     SELECT xrcf008,xrcf009,xrcf010,xrcf103,xrcf113 ",   #180716-00006#2 mark
...
#                  "                        AND xrcfld  = xrcbld ",
#                  #"                        AND xrcfdocno = xrcbdocno )",  #180716-00006#2 mark
#                  "                        AND xrcfdocno = xrcbdocno GROUP BY xrcf008,xrcf009,xrcf010 ) tsed",  #180716-00006#2 add
#                  #190514-00012#1---mark----end
                  #190514-00012#1---add----str
                  "  LEFT OUTER JOIN( ",
                  ...
                  "                        AND xrcfdocno = xrcadocno GROUP BY xrcf008,xrcf009,xrcf010 ) t54 ",  #PGS(D)-10043-add t54
                  #190514-00012#1---add----end
                  "               ON xrccdocno=xrcf008 AND xrccseq=xrcf009 AND xrcc001=xrcf010 ",
#                  #190514-00012#1---mark----str
#                  " WHERE xrcaent=xrcbent AND xrcald=xrcbld AND xrcadocno=xrcbdocno ",
...
#                  #190514-00012#1---mark----end
   ...
   IF tm.chk_ap='Y' THEN
      ...
      LET l_sql = " SELECT '2',apcb029,'',apca100,'','',apca038,'','','','','',apcbdocno,0,'','','','','','', ",
                  ...
                  "                        AND xrdastus = 'Y' GROUP BY xrce003,xrce004,xrce005  ) t55 ",  #180716-00006#2 add #PGS(D)-10043-add t55
                  ...
                  "                        AND apcfdocno = apcadocno GROUP BY apcf008,apcf009,apcf010  ) t56 ", #180716-00006#2 add #PGS(D)-10043-add t56
...
#                  #190514-00012#1---mark---end
                  ...
                  "                        AND xrceent=",g_enterprise ,
                  #"                        AND xrdastus = 'Y' )", #180716-00006#2 mark
                  "                        AND xrdastus = 'Y' GROUP BY xrce003,xrce004,xrce005 ) t57 ", #180716-00006#2 add #PGS(D)-10043-add t57
                  "               ON apccdocno=xrce003  AND apccseq=xrce004 AND apcc001=xrce005 ",
                  "  LEFT OUTER JOIN( ",
                  #"                     SELECT apcf008,apcf009,apcf010,apcf103,apcf113 ", #180716-00006#2 mark
                  "                     SELECT apcf008,apcf009,apcf010,SUM(apcf103) apcf103,SUM(apcf113) apcf113 ", #180716-00006#2 add
                  ...
                  "                        AND apcfent=",g_enterprise ,
                  #"                        AND apcfdocno = apcadocno )", #180716-00006#2 mark
                  "                        AND apcfdocno = apcadocno GROUP BY apcf008,apcf009,apcf010 ) t58 ", #180716-00006#2 add #PGS(D)-10043-add t58
                  "               ON apccdocno=apcf008 AND apccseq=apcf009 AND apcc001=apcf010 ",
#                  #190514-00012#1---mark---str
#                  " WHERE apcaent=apcbent AND apcald=apcbld AND apcadocno=apcbdocno ",
#                  "   AND apcbent=",g_enterprise," AND apcbld='",tm.glaald,"'",
#                  #190514-00012#1---mark---end
      ...
      IF g_glaa121 = 'Y'   THEN  #启用分录
         IF l_sfin2002='3' THEN  #冲账参数3
           ...
           FOREACH aglq510_cur_chk5_4  INTO l_tmp.*,l_xrcc.*
              ...
              #190408-00030#1---mark---end--
              #191111-00027#1-----s
             #IF l_glac008='1' THEN   #200316-00017#1 mark
              IF l_glac008='2' THEN   #200316-00017#1 add
                 LET l_tmp.glgb010=l_tmp.glgb010*-1
                 LET l_tmp.glgb003=l_tmp.glgb003*-1 
              END IF 
         ...
         ELSE  #冲账参数为1、2时
            ...
            FOREACH aglq510_cur_chk5_5 INTO l_tmp.*,l_xrcc.*
              ...
              #190408-00030#1---mark---end--  
              #191111-00027#1-----s
             #IF l_glac008='1' THEN   #albireo 191113   #200316-00017#1 mark
              IF l_glac008='2' THEN   #albireo 191113   #200316-00017#1 add
                 LET l_tmp.glgb010=l_tmp.glgb010*-1
                 LET l_tmp.glgb003=l_tmp.glgb003*-1 
              END IF      
              IF l_xrca.xrca001 MATCHES '2*' THEN
   ...
   LET l_sql=" SELECT glgbdocno,NVL(SUM(glgb010),0),NVL(SUM(glgb003-glgb004),0) glgb003 ",
             ...
             "              glgb003,glgb004",
             "         FROM glgb_t WHERE glgbent=",g_enterprise," AND glgbld='",tm.glaald,"' ",
             "          AND glgbdocno=? AND glgb002=? AND glgb022=? ) t51 ", #PGS(D)-10043-add t51
             " GROUP BY glgbdocno "
   ...
   LET l_sql=" SELECT glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003-glaq004),0) glgb003 ",
             ...
             "              glaq003,glaq004",
             "         FROM glaq_t WHERE glaqent=",g_enterprise," AND glaqld='",tm.glaald,"' ",
             "          AND glaqdocno=? AND glaq002=? AND glaq022=?) t51 ", #PGS(D)-10043-add t51
             " GROUP BY glaqdocno  "
...
#                  "        CASE WHEN xrca001='21' OR xrca001='25' OR xrca001='26' THEN xrce113  ELSE xrce119 END xrce1191  ",
                  ...
                  "                        AND apceent=",g_enterprise ,
                  #"                        AND apdastus = 'Y' )",  #180716-00006#2 mark
                  "                        AND apdastus = 'Y' GROUP BY apce003,apce004,apce005) t59 ",  #180716-00006#2 add #PGS(D)-10043-add t59
                  "               ON xrccdocno=apce003  AND xrccseq=apce004 AND xrcc001=apce005 ",
                  "  LEFT OUTER JOIN( ",
                  #"                     SELECT xrcf008,xrcf009,xrcf010,xrcf103,xrcf113 ", #180716-00006#2 mark
                  "                     SELECT xrcf008,xrcf009,xrcf010,SUM(xrcf103) xrcf103,SUM(xrcf113) xrcf113 ", #180716-00006#2 add
                  ...
                  "                        AND xrcfent=",g_enterprise ,
                  #"                        AND xrcfdocno = xrcbdocno )",  #180716-00006#2 mark
                  "                        AND xrcfdocno = xrcbdocno GROUP BY xrcf008,xrcf009,xrcf010 ) t60 ",  #180716-00006#2 add #PGS(D)-10043-add t60
                  "               ON  xrccdocno=xrcf008 AND xrccseq=xrcf009 AND xrcc001=xrcf010 ",
                  " WHERE xrcaent=xrcbent AND xrcald=xrcbld AND xrcadocno=xrcbdocno ",
                  "   AND xrcbent=",g_enterprise," AND xrcbld='",tm.glaald,"'",
                  "   AND (xrcadocdt BETWEEN '",l_date_s,"' AND '",l_date_e,"' OR ",
...
#                  "        CASE WHEN xrca001='21' OR xrca001='25' OR xrca001='26' THEN xrce113  ELSE xrce119 END xrce1191  ",
                  ...
                  "                        AND apcedocno = apdadocno ",
                  #"                        AND apdastus = 'Y' )", #180716-00006#2 mark
                  "                        AND apdastus = 'Y' GROUP BY apce003,apce004,apce005 ) t61 ", #180716-00006#2 add #PGS(D)-10043-add t61
                  "               ON xrccdocno=apce003  AND xrccseq=apce004 AND xrcc001=apce005 ",
                  "  LEFT OUTER JOIN( ",
                  #"                     SELECT xrcf008,xrcf009,xrcf010,xrcf103,xrcf113 ",  #180716-00006#2 mark
#                  #190514-00012#1---mark---str
...
#                  #190514-00012#1---mark---end
                  ...
                  "                        AND xrcfent=",g_enterprise ,
                  "                        AND xrcfld  = xrcald ",
                  "                        AND xrcfdocno = xrcadocno GROUP BY xrcf008,xrcf009,xrcf010 ) t62 ",  #PGS(D)-10043-add t62
                  #190514-00012#1---add---end
                  "               ON xrccdocno=xrcf008 AND xrccseq=xrcf009 AND xrcc001=xrcf010 ",
#                  #190514-00012#1---mark---str
#                  " WHERE xrcaent=xrcbent AND xrcald=xrcbld AND xrcadocno=xrcbdocno ",
...
#                  #190514-00012#1---mark---end
      ...
      IF g_glaa121 = 'Y'   THEN
         ...
         ELSE  #冲账参数为1/2时
            ...
            FOREACH aglq510_cur_chk7_3 INTO l_tmp.*,l_xrcc.*
                                           ,l_xrca.xrca001  #210316-00036#1 add
               IF SQLCA.sqlcode THEN
                  INITIALIZE g_errparam TO NULL
                  LET g_errparam.code = SQLCA.sqlcode
              ...
              IF l_glac008='2' THEN
                 ...
                 LET l_tmp.glgb003=l_tmp.glgb003*-1 #180716-00006#2 add
              END IF                      
              #210316-00036#1 add(s)
              IF l_xrca.xrca001 MATCHES '2*' OR l_xrca.xrca001 MATCHES '0[246]' THEN
                 LET l_sum = l_sum * -1
              END IF
              #210316-00036#1 add(e)
              #子系统本币与底稿本币相等 or 子系统为0不显示
              IF l_sum=l_tmp.glgb003 OR l_sum=0 THEN
                 CONTINUE FOREACH
              END IF
   ...
   IF tm.chk_ap='Y' THEN
      ...
      LET l_sql = " SELECT '2',apcb029,'',apca100,apca004,'',apca038,'','','','','',apcbdocno,0,'','','','','','', ",
                  ...
                  "                        AND xrceent=",g_enterprise ,
                  #"                        AND xrdastus = 'Y' )",  #180716-00006#2 mark 
                  "                        AND xrdastus = 'Y' GROUP BY xrce003,xrce004,xrce005 ) t51 ",  #180716-00006#2 add #PGS(D)-10043-add t51
                  "               ON apccdocno=xrce003  AND apccseq=xrce004 AND apcc001=xrce005 ",
                  "  LEFT OUTER JOIN( ",
                  #"                     SELECT apcf008,apcf009,apcf010,apcf103,apcf113 ",  #180716-00006#2 mark
                  "                     SELECT apcf008,apcf009,apcf010,SUM(apcf103) apcf103,SUM(apcf113) apcf113",  #180716-00006#2 add
                  ...
                  "                        AND apcfent=",g_enterprise ,
                  #"                        AND apcfdocno = apcadocno )", #180716-00006#2 mark
                  "                        AND apcfdocno = apcadocno group by apcf008,apcf009,apcf010 ) t52 ", #180716-00006#2 add #PGS(D)-10043-add t52
                  "               ON  apccdocno=apcf008  AND apccseq=apcf009 AND apcc001=apcf010 ",
                  " WHERE apcaent=apcbent AND apcald=apcbld AND apcadocno=apcbdocno ",
                  "   AND apcbent=",g_enterprise," AND apcbld='",tm.glaald,"'",
                  "   AND (apcadocdt BETWEEN '",l_date_s,"' AND '",l_date_e,"' OR ",
      ...
      IF g_glaa121 = 'Y'   THEN  #启用分录
         ...
         ELSE  #冲账参数为1、2时
            ...
            FOREACH aglq510_cur_chk7_5 INTO l_tmp.*,l_xrcc.*
                                           ,l_xrca.xrca001  #210318-00005#1 add
               IF SQLCA.sqlcode THEN
                  INITIALIZE g_errparam TO NULL
                  LET g_errparam.code = SQLCA.sqlcode
              ...
              #210318-00005#1 add(s)
              IF l_xrca.xrca001 MATCHES '2*' OR l_xrca.xrca001 MATCHES '0[246]' THEN
                 LET l_sum = l_sum * -1
              END IF
              #210318-00005#1 add(e)
...
#  #180716-00006#3 mark-e  
   ...
   LET l_sql=" SELECT sys,glaq002,'',glaq005,glaq022,'',glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003),0),'','','','',0,'','','','','','',SUM(glaq004) ",
             ...
             "          (CASE glac008
              ...
              "            AND glap007 <>'GL'  AND glapstus<>'X' ",
              #"            AND EXISTS (SELECT 1 FROM aglq510_tmp WHERE glar001=glaq002 AND glar009=glaq005 AND diff NOT IN('0','5') ))",  #180716-00006#2 mark
              "            AND EXISTS (SELECT 1 FROM aglq510_tmp WHERE glar001=glaq002 AND glar009=glaq005 ",l_wc3," AND diff NOT IN('0','5') )) t63 ",  #180716-00006#2 add #PGS(D)-10043-add t63
              "  where glap007<>sys ",
              " GROUP BY sys,glaq002,glaq005,glaq022,glaqdocno ",
              " ORDER BY glaq002,glaqdocno "
...
#             " ORDER BY sys5,glaq002,glaq005,flag "
             ...
             #190221-00009#1 add_S             
             "        xrcbdocno,xrcbseq,gzza001,xrcb105,xrcb115,message_code,advise,flag,message1 FROM ",
             ...
             " FROM aglq510_tmp01) t78 "  #PGS(D)-10043-add t78               
             IF tm.chk_show = 'N' THEN 
               LET l_sql = l_sql CLIPPED," WHERE message1 = '2' " 
             END IF
             LET l_sql = l_sql CLIPPED," ORDER BY sys5,glaq002,glaq005,flag "
             #190221-00009#1 add_E           
...
#                               g_glaq6_d[l_ac].xrcb115,g_glaq6_d[l_ac].message_code,g_glaq6_d[l_ac].advise  #190221-00009#1 mark
                               g_glaq6_d[l_ac].xrcb115,g_glaq6_d[l_ac].message_code,g_glaq6_d[l_ac].advise,l_flag,g_glaq6_d[l_ac].message1  #190221-00009#1 add
      IF SQLCA.SQLCODE THEN
         INITIALIZE g_errparam TO NULL 
...
#190516-00019#1 add -e
   ...
   LET l_sql=" SELECT sys,glaq002,'',glaq005,glaq022,'',glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003),0),'','','','',0,'','','','','',1,SUM(glaq004) ",
             ...
             #190408-00030#1---mark---end--
              ...
              "            AND glapent=",g_enterprise," AND glapld='",tm.glaald,"' AND glap002='",tm.glar002,"' AND glap004='",tm.glar003,"'",
              "            AND glap007='GL' AND glapstus<>'X' ",
              "            AND EXISTS (SELECT 1 FROM aglq510_tmp WHERE glar001=glaq002 AND glar009=glaq005 AND sys='4' AND diff NOT IN('0','5') )) t64 ", #PGS(D)-10043-add t64
              " GROUP BY sys,glaq002,glaq005,glaq022,glaqdocno ",
              " ORDER BY glaq002,glaqdocno "
   ...
   LET l_sql=" SELECT sys,glaq002,'',glaq005,glaq022,'',glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003),0),'','','','',0,'','','','','',2,SUM(glaq004),glap007 ",
             ...
             "          (CASE glac008
              ...
              "            AND glapent=",g_enterprise," AND glapld='",tm.glaald,"' AND glap002='",tm.glar002,"' AND glap004='",tm.glar003,"'",
              "            AND glapstus NOT IN ('X','S') ",l_wc,
              "            AND EXISTS (SELECT 1 FROM aglq510_tmp WHERE glar001=glaq002 AND glar009=glaq005 AND sys='4' AND diff NOT IN('0','5') )) t65 ", #PGS(D)-10043-add t65
   ...
   LET l_sql=" SELECT sys,nmbt029,'',nmbt100,'','','','','','','','',nmbtdocno,0,'',SUM(nmbt103),SUM(nmbt113),'','',3 ",
            ...
            #"             AND nmbtent=",g_enterprise," AND nmbtld='",tm.glaald,"' AND nmbt012 BETWEEN '",l_date_s,"' AND '",l_date_e,"'",    #181204-00026#9  mark
             ...
             "             AND EXISTS (SELECT 1 FROM aglq510_tmp WHERE glar001=nmbt029 AND glar009=nmbt100 AND sys='4' AND diff NOT IN('0','5') )) t66 ",  #PGS(D)-10043-add t66
   ...
   LET l_sql=" SELECT glgbdocno,NVL(SUM(glgb010),0),NVL(SUM(glgb003-glgb004),0) glgb003 ",
             ...
             "              glgb003,glgb004",
             "         FROM glgb_t WHERE glgbent=",g_enterprise," AND glgbld='",tm.glaald,"' ",
             "          AND glgbdocno=? AND glgb002=? ) t67 ", #PGS(D)-10043-add t67
             " GROUP BY glgbdocno,glgb002  "
...
#      PREPARE aglq510_4pre_chk5 FROM l_sql 
   ...
   LET l_sql=" SELECT glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003-glaq004),0) glgb003 ",
             ...
             "              glaq003,glaq004",
             "         FROM glaq_t WHERE glaqent=",g_enterprise," AND glaqld='",tm.glaald,"' ",
             "          AND glaqdocno=? AND glaq002=? ) t68 ", #PGS(D)-10043-add t68
             " GROUP BY glaqdocno,glaq002  "
   ...
   LET l_sql=" SELECT sys,glab005,'',nmbb004,'','','','','','','','',nmbbdocno,nmbbseq,'',",
            ...
            #"    AND nmbastus <>'X' and nmba003 IN ('anmt540','anmt541','anmt310','adet402','anmp802','anmt440','aapt352','anmt710') ", #181219-00040#28 add #181219-00040#28 mark
             ...
             "    AND nmck043 <> 'N' ",  #200725-00001#1 add             
   ...
   LET l_sql=" SELECT sys,glaq002,'',glaq005,glaq022,'',glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003),0),'','','','',0,'','','','','',12,SUM(glaq004) ",
             ...
             "          (CASE glac008
              ...
              "            AND glapent=",g_enterprise," AND glapld='",tm.glaald,"' AND glap002='",tm.glar002,"' AND glap004='",tm.glar003,"'",
              "            AND glap007 <>'GL'  AND glapstus<>'X' ",
              "            AND EXISTS (SELECT 1 FROM aglq510_tmp WHERE glar001=glaq002 AND glar009=glaq005 AND sys='4' AND diff NOT IN('0','5') )) t69 ", #PGS(D)-10043-add t69
              "  where glap007<>sys ",
              " GROUP BY sys,glaq002,glaq005,glaq022,glaqdocno ",
              " ORDER BY glaq002,glaqdocno "
   ...
   LET l_sql= " SELECT sys,glaq002,'',glaq005,glaq022,'',glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003),0),'','','','',0,'','','','','',1,SUM(glaq004) ",
              ...
              "            AND EXISTS (SELECT 1 FROM aglq510_tmp WHERE glar001=glaq002 AND glar009=glaq005 AND sys='4' AND diff NOT IN('0','5') ))) t70 ", #PGS(D)-10043-add t70
              " GROUP BY sys,glaq002,glaq005,glaq022,glaqdocno ",
              " ORDER BY glaq002,glaqdocno "
...
DEFINE l_faaj024  LIKE faaj_t.faaj024   #折旧科目
   ...
   LET l_sql=" SELECT sys,glaq002,'',glaq005,glaq022,'',glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003),0),'','','','',0,'','','','','',1,SUM(glaq004) ",
             ...
             #190408-00030#1---mark---end--
              ...
              "            AND glapent=",g_enterprise," AND glapld='",tm.glaald,"' AND glap002='",tm.glar002,"' AND glap004='",tm.glar003,"'",
              "            AND glap007='GL' AND glapstus<>'X' ",
              "            AND EXISTS (SELECT 1 FROM aglq510_tmp WHERE glar001=glaq002 AND glar009=glaq005 AND sys='3' AND diff NOT IN('0','5') )) t71 ", #PGS(D)-10043-add t71
              " GROUP BY sys,glaq002,glaq005,glaq022,glaqdocno ",
              " ORDER BY glaq002,glaqdocno "
   ...
   LET l_sql=" SELECT sys,glaq002,'',glaq005,glaq022,'',glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003),0),'','','','',0,'','','','','',2,SUM(glaq004),glap007 ",
             ...
             "          (CASE glac008
              ...
              "            AND glapent=",g_enterprise," AND glapld='",tm.glaald,"' AND glap002='",tm.glar002,"' AND glap004='",tm.glar003,"'",
              "            AND glapstus NOT IN ('X','S') ",l_wc,
              "            AND EXISTS (SELECT 1 FROM aglq510_tmp WHERE glar001=glaq002 AND glar009=glaq005 AND sys='3' AND diff NOT IN('0','5') )) t72 ", #PGS(D)-10043-add t72
              " GROUP BY glaq002,glaq005,glaq022,glaqdocno,glap007,sys ",
              " ORDER BY glaq002,glaqdocno "   
   ...
   LET l_sql=" SELECT sys,glaq002,'',glaq005,glaq022,'',glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003),0),'','','','',0,'','','','','',12,SUM(glaq004) ",
             ...
             "          (CASE glac008
              ...
              "            AND glapent=",g_enterprise," AND glapld='",tm.glaald,"' AND glap002='",tm.glar002,"' AND glap004='",tm.glar003,"'",
              "            AND glap007 <>'GL'  AND glapstus<>'X' ",
              "            AND EXISTS (SELECT 1 FROM aglq510_tmp WHERE glar001=glaq002 AND glar009=glaq005 AND sys='3' AND diff NOT IN('0','5') )) t73 ", #PGS(D)-10043-add t73
              "  where glap007<>sys ",
              " GROUP BY sys,glaq002,glaq005,glaq022,glaqdocno ",
              " ORDER BY glaq002,glaqdocno "
...
DEFINE l_sfin1003   LIKE type_t.chr1      #200323-00010#1----add
   ...
   LET l_sql=" SELECT sys,glaq002,'',glaq005,glaq022,'',glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003),0),'','','','',0,'','','','','',1,SUM(glaq004) ",
             ...
             #190408-00030#1---mark---end--
              ...
              "       ) t74 ",                                #190604-00035#1 add #PGS(D)-10043-add t74
              " GROUP BY sys,glaq002,glaq005,glaq022,glaqdocno ",
              " ORDER BY glaq002,glaqdocno "
   ...
   LET l_sql=" SELECT sys,glaq002,'',glaq005,glaq022,'',glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003),0),'','','','',0,'','','','','',2,SUM(glaq004),glap007 ",
             ...
             "          (CASE glac008
              ...
              "            AND tmp.sys = '5' ",               #190604-00035#1 add
              "            AND tmp.diff NOT IN ('0','5') ",   #190604-00035#1 add
              "       ) t75 ",                                #190604-00035#1 add #PGS(D)-10043-add t75
              " GROUP BY glaq002,glaq005,glaq022,glaqdocno,glap007,sys ",
              " ORDER BY glaq002,glaqdocno "   
   ...
   LET l_sql=" SELECT sys,glaq002,'',glaq005,glaq022,'',glaqdocno,NVL(SUM(glaq010),0),NVL(SUM(glaq003),0),'','','','',0,'','','','','',12,SUM(glaq004) ",
             ...
             "          (CASE glac008
              ...
              "            AND tmp.sys = '5' ",               #190604-00035#1 add
              "            AND tmp.diff NOT IN ('0','5') ",   #190604-00035#1 add
              "       ) t76 ",                                #190604-00035#1 add #PGS(D)-10043-add t76
              "  where glap007<>sys ",
              " GROUP BY sys,glaq002,glaq005,glaq022,glaqdocno ",
              " ORDER BY glaq002,glaqdocno "
...
#             #200323-00010#1----mark----end
            ...
            #"   AND xcco010<>'4' AND xcco001='1' ",                #210217-00014#1 mark
             "   AND xcco010 NOT IN ('3','4') AND xcco001='1' ",    #210217-00014#1 add            
            ...
            #"   AND xccd304<>'4' AND xccd001='1' ",    #190930-00038#1 mark
             "   AND xccd304<>'0' AND xccd001='1' ",    #190930-00038#1 add   
            ...
            #"   AND xcch304<>'4' AND xcch001='1' ",     #190930-00038#1 mark
             "   AND xcch304<>'0' AND xcch001='1' ",     #190930-00038#1 add
   ...
   #200323-00010#1----add----str
   CALL cl_get_para(g_enterprise,tm.glaacomp,'S-FIN-1003') RETURNING l_sfin1003
   IF cl_null(l_sfin1003) THEN
      LET l_sfin1003 = '1'
   END IF
...
#             "          INNER JOIN xcck_t ON apcbent=xcckent AND apcbld=xcckld AND apcbdocno=xcck006 AND apcbseq=xcck007 ",   #190806-00030#1---mark
             ...
             #"                       WHERE pmdsent=pmdtent AND pmdsdocno=pmdtdocno AND pmdsent = ",g_enterprise," AND pmdssite = '",tm.glaacomp,"')", #190416-00021#1 add             "                          ON apcb002=pmdtdocno AND apcb003=pmdtseq ", #PGS(D)-10043-mark
             #PGS(D)-10043-S-
             "                       WHERE pmdsent=pmdtent AND pmdsdocno=pmdtdocno AND pmdsent = ",g_enterprise," AND pmdssite = '",tm.glaacomp,"') t51 ",
             "                          ON apcb002=pmdtdocno AND apcb003=pmdtseq ",           
             #PGS(D)-10043-E-
   ...
   LET l_sql="SELECT '5','','','','','','','','','','','',xcck006,xcck007,'','','','','','5' ",
             "  FROM xcck_t ",
             " WHERE xcckent=",g_enterprise," AND xcckld='",tm.glaald,"' AND xcck004='",tm.glar002,"' AND xcck005='",tm.glar003,"'",
#             "   AND xcck055 IN ('2011','203','2030') AND xcck001='1' ",  #210118-00024#1---mark
              "   AND xcck055 IN ('201','203') AND xcck001='1' ",   #210118-00024#1---add
