...
# Modify.........: No.TQC-D50029 10/07/28 By fengrui 補上缺失的DECLARE
# Modify.........: No:FUN-D70075 13/07/15 By Nina  EBO 5.3追版
# Modify.........: No:FUN-D70077 13/08/07 By wangrr aapt180取消審核時判斷aapt121中是否已存在沖暫估資料,若已存在不可取消審核
# Modify.........: No:FUN-D80031 13/08/13 By yuhuabao aapt140取消審核判斷如果有生成待抵扣費單且 已沖帳則不可取消審核
# Modify.........: No:FUN-D80064 13/08/16 By Nina  EBO 5.3追版調整
# Modify.........: No:MOD-DA0183 13/10/31 By yinhy aapt140取消审核时，增加oob04为'3'也不可取消審核
# Modify.........: No:MOD-E20016 14/02/07 By apo 變數已在saapt110.global定義,重定義會導致處理後的值回主程式時失效
# Modify.........: No:MOD-E20133 14/02/27 By yihsuan s_t110_unconfirm 訊息 aap-829 修改條件為 IF g_apa.apa79 MATCHES '[123]' THEN 參考 saapt110 
# Modify.........: No:FUN-E20008 13/03/07 By fengmy 還原FUN-D70077
# Modify.........: No:MOD-E30091 14/03/18 By yihsuan s_t110_unconfirm 取消 DISPLAY apa44
# Modify.........: No:MOD-E40131 14/05/02 By yihsuan 取消確認,UPDATE apa_file時排除DIFF2的條件
# Modify.........: No:MOD-E70122 14/07/28 By doris 取消確認段,增加檢核 fak45/fak451 是否存在此筆帳款 apb01/apb02
# Modify.........: No:FUN-EA0011 14/10/09 By Abby [EBO整合]還原FUN-D80064單號，避免與EBO整合時，無法取消確認
# Modify.........: No:MOD-EB0052 14/11/13 By doris 將aap-228的訊息移到FUNCTION s_t110_unconfirm段處理,因FUNCTION s_t110_aph_upd並無針對aph03=1處理
# Modify.........: No:MOD-EB0088 14/11/18 By doris PREPARE t110_z_nme_p2後應DECLARE
# Modify.........: No:MOD-F30075 15/03/13 By doris aap-666(發票已產生折讓,不可取消確認!) 的檢查應加上作廢碼的判斷
# Modify.........: No:MOD-FA0140 15/10/29 By doris 修正apc_file回寫問題
# Modify.........: No:MOD-G30110 16/03/21 By doris 修正apc_file回寫問題,刪除apg_file/apf_file移到後面
# Modify.........: No.FUN-I80008 18/02/21 By TSD.Loxie 若作廢折讓已上傳電子發票(ave_file),則不可異動
# Modify.........: No:FUN-I60004 18/06/12 By Mars    EC整合 : 調整單據還原取消流程
# Modify.........: No.FUN-I80008 18/09/18 By TSD.alana調整電子發票取消確認邏輯
# Modify.........: No.MOD-J40002 19/04/02 By Dido 若未安裝汎宇系統則不檢核 ave_file 
# Modify.........: No.MOD-JC0012 19/12/26 By Dido 電子發票進項折讓增加 aapt220 議題
# Modify.........: No.MOD-K70019 20/07/21 By Dido 取單筆資料回沖 

DATABASE ds
 
GLOBALS "../../config/top.global"
GLOBALS "../4gl/saapt110.global"    #FUN-D70075 add

#DEFINE g_apa    RECORD LIKE apa_file.*   #MOD-E20016 mark
...
FUNCTION s_t110_unconfirm(p_aptype,p_apa01,p_apa41,p_apb06,p_apb37,p_plant)
   ...
   DEFINE l_success  LIKE type_file.chr1   #FUN-I60004 add
   ...
   FOREACH sel_apa_cs INTO g_apa.*
      ...
      IF g_aza.aza75 MATCHES '[Yy]' AND (g_prog = 'aapt110' OR g_prog = 'aapt120') THEN
         CALL aws_ebocli(g_dbs,g_apa.apa01,'*','*','*','*','EBO-120','M010','query') RETURNING l_ebocode
         
         #底稿區資料讀取否
         IF l_ebocode = 'Y' THEN
            IF FGL_GETENV("FGLGUI") <> "0" THEN
               #此筆資料已被EBO端讀取，確認要取消確認？
               IF NOT cl_confirm2('aws-637',g_apa.apa01) THEN   
                  CONTINUE FOREACH
               END IF 
            END IF               
         END IF
     ...
     #IF g_apa.apa79  <> '0'  THEN          #MOD-E20133 mark
      IF g_apa.apa79 MATCHES '[123]' THEN   #MOD-E20133 add 
         IF only_one='1' THEN
      ...
      #FUN-I80008 180221 By TSD.Loxie---(Start)---
      #檢查是否存在電子發票(avg_file),若是則不可取消確認
     #IF g_apa.apa00 = '21' THEN                     #MOD-J40002 mark 
     #IF g_apa.apa00 = '21' AND g_chkaiv = 'Y' THEN  #MOD-J40002             #MOD-JC0012 mark
      IF (g_apa.apa00 = '21' OR g_apa.apa00 = '22') AND g_chkaiv = 'Y' THEN  #MOD-JC0012
         CALL t210_chk_avg(g_apa.apa01)
         IF NOT cl_null(g_errno) THEN
            IF only_one = '1' THEN
               CALL cl_err(g_apa.apa01,g_errno,1)
               #---(S)---FUN-I80008  180918 By TSD.alana    #警告就好
               #ROLLBACK WORK
               #CLOSE WINDOW t110_u
               #RETURN
               LET g_errno = NULL
               #---(E)---FUN-I80008  180918 By TSD.alana
            ELSE
               #---(S)---FUN-I80008  180918 By TSD.alana    #警告就好
               #CALL s_errmsg('apa01',g_apa.apa01,'','aiv-100',1)
               #LET g_success = 'N'
               #CONTINUE FOREACH
               CALL s_errmsg('apa01',g_apa.apa01,'','aiv-070',1)
               #---(E)---FUN-I80008  180918 By TSD.alana
            END IF
         END IF
      END IF
      #FUN-I80008 180221 By TSD.Loxie---(End)---
      ...
      SELECT COUNT(*) INTO l_cnt FROM api_file
       ...
       IF l_cnt > 0 AND p_aptype ='16' THEN #FUN-D70077 mark  #FUN-E20008 remark
      #IF l_cnt > 0 AND (p_aptype ='16' OR p_aptype ='18') THEN #FUN-D70077  #FUN-E20008 mark
          IF only_one='1' THEN
     ...
     #已拋轉票據不可取消確認
      ...
      IF l_n > 0 THEN
         LET g_success = 'N'
         IF only_one='1' THEN
            CALL cl_err(g_apa.apa01,'aap-228',1)
            ROLLBACK WORK
            CLOSE WINDOW t110_u
            RETURN
         ELSE
            CALL s_errmsg('','',g_apa.apa01,'aap-228',1)
            LET g_success='N'
            CONTINUE FOREACH
         END IF
      END IF
     ...
     #MOD-E70122---add---str---
      SELECT COUNT(*) INTO l_cnt
        FROM apb_file,fak_file
       WHERE apb01=fak45 
         AND apb02=fak451
         AND apb01=g_apa.apa01
      IF l_cnt > 0 THEN
         IF only_one='1' THEN
            CALL cl_err(g_apa.apa01,'aap-216',1)
            ROLLBACK WORK
            CLOSE WINDOW t110_u
            RETURN
         ELSE
            CALL s_errmsg('','',g_apa.apa01,'aap-216',1)
            LET g_success='N'
            CONTINUE FOREACH 
         END IF
      END IF
     #MOD-E70122---add---end---
      ...
      IF g_apy.apydmy3 = 'Y' AND g_apy.apyglcr = 'Y' THEN
         IF NOT cl_null(g_apa.apa44) THEN
           ...
           #DISPLAY BY NAME g_apa.apa44   #MOD-E30091 mark
            IF NOT cl_null(g_apa.apa44) THEN
               IF only_one='1' THEN
      ...
      #FUN-I80008 180221 By TSD.Loxie---(Start)---
      #刪除記錄到進項作廢折讓發票檔(ave_file)
     #IF p_aptype = '21' THEN                     #MOD-J40002 mark
     #IF p_aptype = '21' AND g_chkaiv = 'Y' THEN  #MOD-J40002          #MOD-JC0012 mark
      IF (p_aptype = '21' OR p_aptype = '22') AND g_chkaiv = 'Y' THEN  #MOD-JC0012
         CALL t210_del_ave(g_apa.apa01)
      END IF
      #FUN-I80008 180221 By TSD.Loxie---(End)---
      ...
      IF g_aza.aza75 MATCHES '[Yy]' AND (g_prog = 'aapt110' OR g_prog = 'aapt120') THEN
         ...
         IF l_ebocode NOT MATCHES '[Yy]' THEN
            #EBO端未讀取才刪除
            CALL aws_ebocli(g_dbs,g_apa.apa01,'*','*','*','*','EBO-120','M010','del') RETURNING l_success
            IF l_success = 'N' THEN                   
               IF only_one = '1' THEN
                  CALL cl_err(g_apa.apa01,'aws-462',1)  #e-B Online底稿區資料刪除失敗! 
                  ROLLBACK WORK
                  CLOSE WINDOW t110_u
                  RETURN                  
               ELSE
                  CALL s_errmsg('apa01',g_apa.apa01,'del ebo','aws-462',1)  #e-B Online底稿區資料刪除失敗!
                  LET g_success = 'N'
                  CONTINUE FOREACH
               END IF
            END IF    
...
FUNCTION s_t110_del_nme()
   ...
   IF l_aza73 = 'Y' THEN
      ...
      DECLARE t110_z_nme_c2 CURSOR FOR t110_z_nme_p2   #MOD-EB0088 add
     #FOREACH t110_z_nme_p2 INTO l_nme24               #MOD-EB0088 mark
      FOREACH t110_z_nme_c2 INTO l_nme24               #MOD-EB0088 add
         IF l_nme24 <> '9' THEN
            LET g_success='N'
            IF only_one='1' THEN
...
FUNCTION s_t110_aph_upd()  
   ...
   IF l_aza73 = 'Y' THEN
      ...
      FOREACH t110_z_nme_c1 INTO l_nme24               #TQC-D50029
         IF l_nme24 <> '9' THEN
            LET g_success='N'
            IF only_one='1' THEN
               CALL cl_err('','anm-043',1)
