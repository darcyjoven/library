...
#該程式非freestyle程式!
{<section id="aglq710.description" type="s" >}
...
{<section id="aglq710.global" type="m" >}
...
#170601-00046#1  2016/06/01  By 02114  增加重新整理按钮的功能
#170717-00001#1  2017/07/17  By albireo  CE傳票相關glac007=''6'的條件要拿掉,因為CE傳票排除應該是整張的科目都排除 總金額才會平衡
#170531-00014#3  2017/07/26  By 02599  跨年查询时，本年累计栏位影藏
#170719-00041#1  2017/07/27  By albireo XC傳票相關glac010 <> 'N'的條件拿掉,aglq710這邊的角度須整張傳票排除總金額才會正確
#170720-00019#1  2017/07/28  By 09232   将变量定义由num5改为num10
#170523-00020#1  2017/10/19  By 07900   增加进度条显示
#171113-00042#1  2017/11/13  By 05488   切換營運據點後，查詢時帳套要抓新營運據點的帳套
#171215-00044#1  2017/12/15  By 05488   切換帳套後，查詢時的帳套卻是該營運據點的主帳套
...
#180131-00075#1  2018/02/05  By 07900   进度条会清空合计的显示，把合计显示放在 进度条的后面
#180223-00030#1  2018/02/24  By 09773   查询数据出后，双击单身某一笔之后返回时，跳回第一行
#180223-00009#1  2018/03/08  By 09505   因為匯差的關係，可能出現本幣結清但原幣尚有差異，程式原本只判斷本幣有差異才顯示，先调整为畫面上勾選顯示原幣，本幣沒有差異但原幣有差異時也要顯示資料
#180517-00032#1  2018/05/17  By 08729   將顯示原幣&按幣別分頁的值也加入串查到aglq740的FUNCTION中，要不然會串查不出來
#180607-00073#1  2018/06/08  By 00537   切换据点后，按当前新据点，参照程序新开时的预设逻辑重新预设 
#180130-00050#19 2018/07/12  By 09505   账套本位币二三都不开启时，单头多本位币选项隐藏
#181011-00024#1  2018/10/12  By 01531   程序有进度条，勾选原币且按币别分页显示时，换下一页时闪退，后台报错:[客戶環境]Field (proc) not found in form.
#190322-00005#21 2019/03/25  By 12133   匯出excel，改寫為POI方式
#180821-00002#1  2019/08/26  by 08172   凭证原币不为0，本币为0者时，依科目借贷余和原币正负表示原币借贷方向
#190912-00006#1  2019/09/16  By 05795   从menu菜单打开的作业和xshell直接打开的作业 查询的数据有得科目数据不一样。menu打开查得数据不对
...
#191206-00013#1  2019/12/06  By 05795   加一个选框当期有异动资料合计为0且期末为0者是否显示，勾了就显示
#200212-00031#1  2020/02/20  By 05795   添加一个连接到aglq760
#200221-00007#13 2020/03/18  By 13656   整批调整Q類查詢方案
#201109-00009#39 2020/12/29  By 14589   模糊查詢功能，將使用者輸入的字串自動補入*
 
IMPORT os
IMPORT util
#add-point:增加匯入項目
IMPORT JAVA com.fourjs.fgl.lang.FglRecord   #190322-00005#21  2019/03/25 By 12133 add
#end add-point
...
#add-point:增加匯入變數檔
#190322-00005#21  2019/03/25 By 12133 add---(S)
GLOBALS
#單身匯excel POI變數
...
END GLOBALS
#190322-00005#21  2019/03/25 By 12133 add---(E)
#end add-point
...
DEFINE l_ac_d               LIKE type_t.num10              #單身idx  #170720-00019#1 add
...
DEFINE tm              RECORD
       ...
       ,show_zero         LIKE type_t.chr1  #191206-00013#1--add
       END RECORD
DEFINE g_wc1           STRING
...
DEFINE g_site_o        LIKE apca_t.apcasite      #營運據點舊值   #171215-00044#1 add
#180131-00075#1--add--s--xul
   DEFINE g_sum1      LIKE type_t.num20_6 
   DEFINE g_sum2      LIKE type_t.num20_6 
   DEFINE g_sum3      LIKE type_t.num20_6 
   DEFINE g_sum4      LIKE type_t.num20_6 
   DEFINE g_sum5      LIKE type_t.num20_6 
   DEFINE g_sum6      LIKE type_t.num20_6 
   DEFINE g_sum7      LIKE type_t.num20_6 
   DEFINE g_sum8      LIKE type_t.num20_6 
   DEFINE g_sum9      LIKE type_t.num20_6 
   DEFINE g_sum10     LIKE type_t.num20_6 
   DEFINE g_sum11     LIKE type_t.num20_6 
   DEFINE g_sum12     LIKE type_t.num20_6 
   DEFINE g_sum13     LIKE type_t.num20_6 
   DEFINE g_sum14     LIKE type_t.num20_6 
   DEFINE g_sum15     LIKE type_t.num20_6 
   DEFINE g_sum16     LIKE type_t.num20_6 
   DEFINE g_sum17     LIKE type_t.num20_6 
   DEFINE g_sum18     LIKE type_t.num20_6 
   DEFINE g_sum19     LIKE type_t.num20_6 
   DEFINE g_sum20     LIKE type_t.num20_6 
   DEFINE g_sum21     LIKE type_t.num20_6 
   DEFINE g_sum22     LIKE type_t.num20_6 
   DEFINE g_sum23     LIKE type_t.num20_6 
   DEFINE g_sum24     LIKE type_t.num20_6 
   DEFINE g_sum25     LIKE type_t.num20_6 
   DEFINE g_sum26     LIKE type_t.num20_6 
   DEFINE g_sum27     LIKE type_t.num20_6 
   DEFINE g_sum28     LIKE type_t.num20_6 
   DEFINE g_sum29     LIKE type_t.num20_6 
   DEFINE g_sum30     LIKE type_t.num20_6 
#180131-00075#1--add--e--xul
#190322-00005#21  2019/03/25 By 12133 add---(S)
DEFINE g_browser    DYNAMIC ARRAY OF RECORD    #資料瀏覽之欄位
       b_statepic     LIKE type_t.chr50,
       b_glarld LIKE glar_t.glarld,
       b_glar001 LIKE glar_t.glar001,
       b_glar002 LIKE glar_t.glar002
       END RECORD
#190322-00005#21  2019/03/25 By 12133 add---(E)
#end add-point
...
{<section id="aglq710.main" type="m" >}
#+ 此段落由子樣板a26產生
...
MAIN
   ...
   #add-point:作業初始化
   LET g_site_o = g_site      #FOR切換營運據點用 #171215-00044#1 add
   #end add-point
   ...
   #end add-point
   #LET g_forupd_sql = ""   #PGS(D)-10500-mark  #200730-00057#6 mark
   #add-point:SQL_define

   #end add-point
   #PGS(D)-10500 mark-S    #200730-00057#6 -S-
   #LET g_forupd_sql = cl_sql_forupd(g_forupd_sql)                #轉換不同資料庫語法
   #DECLARE aglq710_cl CURSOR FROM g_forupd_sql                 # LOCK CURSOR
   #PGS(D)-10500 mark-E    #200730-00057#6  -E-
   ...
   #end add-point
   #PREPARE aglq710_master_referesh FROM g_sql #PGS(D)-10500 mark  #200730-00057#6 mark
   ...
   #CLOSE aglq710_cl   #PGS(D)-10500-mark  #200730-00057#6  mark
   ...
   DROP TABLE aglq710_tmp #ODI-ORA#
...
{<section id="aglq710.init" type="s" >}
#+ 畫面資料初始化
PRIVATE FUNCTION aglq710_init()
   ...
   #获取账别
  #IF cl_null(g_glaald) THEN        #171113-00042#1 mark
...
{<section id="aglq710.ui_dialog" type="m" >}
#+ 功能選單 
...
#180607-00073#1 --begin 因为这只已经被改过section，所以人工追最新的做法  
      ...
      DIALOG ATTRIBUTES(UNBUFFERED,FIELD ORDER FORM)
         DISPLAY ARRAY g_glar_d TO s_detail1.* ATTRIBUTE(COUNT=g_detail_cnt) 
            ...
            BEFORE ROW
               ...
               #180131-00075#1--add--s--xul
               #进度条会影响 合计的显示，吧合计显示放在 display arry 里
               ...
               DISPLAY g_sum9 TO FORMONLY.sum9
               DISPLAY g_sum10 TO FORMONLY.sum10
               DISPLAY g_sum11 TO FORMONLY.sum11
               DISPLAY g_sum12 TO FORMONLY.sum12
               DISPLAY g_sum13 TO FORMONLY.sum13
               DISPLAY g_sum14 TO FORMONLY.sum14
               DISPLAY g_sum15 TO FORMONLY.sum15
               DISPLAY g_sum16 TO FORMONLY.sum16
               DISPLAY g_sum17 TO FORMONLY.sum17
               DISPLAY g_sum18 TO FORMONLY.sum18
               DISPLAY g_sum19 TO FORMONLY.sum19
               DISPLAY g_sum20 TO FORMONLY.sum20
               DISPLAY g_sum21 TO FORMONLY.sum21
               DISPLAY g_sum22 TO FORMONLY.sum22
               DISPLAY g_sum23 TO FORMONLY.sum23
               DISPLAY g_sum24 TO FORMONLY.sum24
               DISPLAY g_sum25 TO FORMONLY.sum25
               DISPLAY g_sum26 TO FORMONLY.sum26
               DISPLAY g_sum27 TO FORMONLY.sum27
               DISPLAY g_sum28 TO FORMONLY.sum28
               DISPLAY g_sum29 TO FORMONLY.sum29
               DISPLAY g_sum30 TO FORMONLY.sum30
               #180131-00075#1--add--e--xul
...
#                  CALL aglq710_cmdrun() #串查aglq740總分類帳資料 #200212-00031#1--mark 
                  CALL aglq710_cmdrun_760() #串查aglq760     #200212-00031#1--add                  
               END IF
               #EXIT DIALOG   #180223-00030#1  mark
            END IF
            #end add-point
         ...
         ON ACTION datarefresh   # 重新整理
            LET g_error_show = 1
            #CALL aglq710_b_fill()
            #CALL aglq710_fetch()   #170601-00046#1 mark lujh
         ...
         ON ACTION exporttoexcel   #匯出excel
            ...
            IF cl_auth_chk_act("exporttoexcel") THEN
               ...
               END IF
               #190322-00005#21  2019/03/25 By 12133 add---(E)
                  #190322-00005#21  2019/04/11 By 12133 mark---(S)
                        #LET g_export_node[1] = base.typeInfo.create(g_glar_d)
                        #LET g_export_id[1]   = "s_detail1"  
                        #CALL cl_export_to_excel_getpage()
                        #CALL cl_export_to_excel()
                  #190322-00005#21  2019/04/11 By 12133 mark---(E)    
...
{<section id="aglq710.query" type="m" >}
#+ QBE資料查詢
PRIVATE FUNCTION aglq710_query()
   ...
   DIALOG ATTRIBUTES(UNBUFFERED,FIELD ORDER FORM)
      ...
      CONSTRUCT g_wc_table ON glar001
       ...
       #---------------------<  Detail: page1  >---------------------
         ...
         AFTER FIELD b_glar001
            ...
            #201109-00009#39 add---(s)
            CALL FGL_DIALOG_GETBUFFER() RETURNING ls_result
            IF NOT cl_null(ls_result) THEN
               LET ls_result = cl_ap_code_fuzzyquery(ls_result)   #模糊查詢功能
               DISPLAY ls_result TO b_glar001
            END IF
            #201109-00009#39 add---(e)
            #END add-point
      ...
      INPUT BY NAME tm.sdate,tm.edate,tm.ctype,tm.curr_o,tm.curr_p,tm.show_y,
                    tm.show_acc,tm.glac005,tm.stus,tm.glac009,tm.show_ad,tm.show_ce,tm.show_ye,tm.show_zero #191206-00013#1--add-->tm.show_zero
         ...
         AFTER FIELD sdate
            IF NOT cl_null(tm.sdate) THEN
               ...
               #170531-00014#3--add--str--
               #跨年查询，影藏本年累计
               IF tm.syear<>tm.eyear THEN
                  IF tm.curr_o='Y' THEN
                     CALL cl_set_comp_visible("osumd,osumc",FALSE)
                  END IF
                  CASE tm.ctype
                     WHEN '0'
                        CALL cl_set_comp_visible("sumd,sumc",FALSE)                        
                     WHEN '1'
                        CALL cl_set_comp_visible("sumd,sumc,sumd2,sumc2",FALSE)
                     WHEN '2'
                        CALL cl_set_comp_visible("sumd,sumc,sumd3,sumc3",FALSE)
                     WHEN '3'
                        CALL cl_set_comp_visible("sumd,sumc,sumd2,sumc2,sumd3,sumc3",FALSE)
                  END CASE
               ELSE
                  IF tm.curr_o='Y' THEN
                     CALL cl_set_comp_visible("osumd,osumc",TRUE)
                  END IF
                  CASE tm.ctype
                     WHEN '0'
                        CALL cl_set_comp_visible("sumd,sumc",TRUE)                        
                     WHEN '1'
                        CALL cl_set_comp_visible("sumd,sumc,sumd2,sumc2",TRUE)
                     WHEN '2'
                        CALL cl_set_comp_visible("sumd,sumc,sumd3,sumc3",TRUE)
                     WHEN '3'
                        CALL cl_set_comp_visible("sumd,sumc,sumd2,sumc2,sumd3,sumc3",TRUE)
                  END CASE
               END IF
               #170531-00014#3--add--end
            END IF
...
#                  CALL aglq710_set_comp_entry('curr_p',FALSE) #160411-00027#4 mark
      ...
      BEFORE DIALOG
        #預設
        IF g_site_o != g_site THEN   #171215-00044#1 add
           CALL aglq710_init()       #171113-00042#1 add   
           LET g_site_o = g_site     #171215-00044#1 add
        END IF                       #171215-00044#1 add           
        CALL aglq710_show()
        LET g_glar_d[1].sel = ""
      ...
      #200221-00007#13 --s add
       ON ACTION qbe_select
          LET ls_wc = ""
          CALL cl_qbe_list("c") RETURNING ls_wc
          LET g_curr_diag = ui.DIALOG.getCurrent()   
          CALL aglq710_get_buffer(g_curr_diag) 
   ...
   IF l_flag=TRUE THEN
      IF cl_null(g_wc) THEN LET g_wc = " 1=1" END IF  
      CALL cl_progress_bar(3) #170523-00020#1 xul add
      CALL cl_progress_ing("GET DATA... ") #170523-00020#1 xul add       
      CALL aglq710_get_data()
      CALL cl_progress_ing("Padding_b_fill1 ") #181011-00024#1 add
      SELECT COUNT(*) INTO l_cnt FROM aglq710_tmp
      IF l_cnt=0 THEN 
         INITIALIZE g_errparam TO NULL
         LET g_errparam.code = -100
         ...
         LET g_errparam.popup = FALSE
         CALL cl_err()
         CALL cl_progress_bar_close()   #170523-00020#1 xul add      
         RETURN
      END IF
      CALL aglq710_set_page()
      CALL aglq710_fetch1('F')
      CALL cl_progress_ing("FINISH... ")  #170523-00020#1 xul add
   ELSE
...
{<section id="aglq710.b_fill" type="s" >}
#+ 單身陣列填充
PRIVATE FUNCTION aglq710_b_fill()
   ...
   LET g_sql = g_sql, " ORDER BY glar_t.glarld,glar_t.glar001,glar_t.glar002,glar_t.glar003,glar_t.glar004" #ODI-ORA#
...
{<section id="aglq710.fetch" type="s" >}
#+ 單身陣列填充2
...
{<section id="aglq710.detail_show" type="s" >}
#+ 顯示相關資料
...
{<section id="aglq710.filter" type="m" >}
#+ filter過濾功能
...
{<section id="aglq710.filter_parser" type="m" >}
#+ filter欄位解析
...
{<section id="aglq710.filter_show" type="s" >}
#+ Browser標題欄位顯示搜尋條件
...
{<section id="aglq710.insert" type="s" >}
#+ insert
...
{<section id="aglq710.modify" type="s" >}
#+ modify
...
{<section id="aglq710.reproduce" type="s" >}
#+ reproduce
...
{<section id="aglq710.delete" type="s" >}
#+ delete
...
{<section id="aglq710.other_function" type="s" >}
################################################################################
# Descriptions...: 建立臨時表
# Memo...........:
...
#   CALL cl_set_comp_visible("ctype",TRUE)
   ...
   CALL cl_set_comp_visible("ctype",TRUE)         #180130-00050#19  add
   CASE
...
#         CALL cl_set_comp_visible("ctype",FALSE)
         ...
         CALL cl_set_comp_visible("ctype",FALSE)         #180130-00050#19  add
#         LET tm.ctype=' ' 
      WHEN g_glaa015='Y' AND g_glaa019<>'Y' 
...
PRIVATE FUNCTION aglq710_get_data()
   ...
   DEFINE l_cnt            LIKE type_t.num5 #191206-00013#1--add
...
#151204-00013#1--mark--end
   ...
   IF tm.curr_o<>'Y' THEN
      ...
      #170531-00014#3--mod--str--
      #跨年查询时，本年累计影藏，只有不跨年查询，才计算本年累计
#                #本年累計
#                #原幣借     /原幣貸    /本幣借   /本幣貸  /本幣二借  /本幣二貸  /本幣三借  /本幣三貸
...
#                " FROM glac_t"      
      ...
      ELSE
         #本年累計
         LET g_sql=g_sql,"       0,0,0,0, 0,0,0,0 "
      END IF          
      LET g_sql=g_sql," FROM glac_t"
      #170531-00014#3--mod--end
      ...
      ELSE
         LET g_sql=g_sql,
                   " LEFT JOIN (",
                   #                    科目    /原幣借-貸
#                   "            SELECT glaq002,SUM(CASE WHEN glaq003=0 THEN glaq010*-1 ELSE glaq010 END) oqcd,",  #180821-00002#1 mark
                   ...
                   "                           WHEN glaq003 = 0 AND glaq004 = 0 THEN ",
                   "                              CASE ",
                   "                                 WHEN (glaq010 > 0 AND (SELECT glac008 FROM glac_t WHERE glacent =  ",g_enterprise," AND glac002 = glaq002 AND glac001 = (select glaa004 FROM glaa_t WHERE glaaent =  ",g_enterprise," AND glaald = glaqld)) = '1') ",
                   "                                   OR (glaq010 < 0 AND (SELECT glac008 FROM glac_t WHERE glacent =  ",g_enterprise," AND glac002 = glaq002 AND glac001 = (SELECT glaa004 FROM glaa_t WHERE glaaent =  ",g_enterprise," AND glaald = glaqld)) = '2') THEN ",
                   "                                    glaq010 ",
                   "                                 ELSE ",
                   "                                    glaq010*-1 ",
                   "                              END ",
...
#                   "                   SUM(CASE WHEN glaq003=0 THEN glaq010 ELSE 0 END ) oqjc,",
                   ...
                   #                   科目    /原幣借方金額 
                   "            SELECT glaq002, ",
                   "                   SUM((CASE WHEN glaq004 = 0 AND glaq003 <> 0 THEN ",
                   "                           glaq010 ",
                   "                        ELSE ",
...
#                         "           OR (glar002=",tm.eyear," AND glar003<=",tm.eperiod," AND glar003<>0)  )" #170308-00007#1 mark
                         "           OR (glar002=",tm.eyear," AND glar003<=",tm.eperiod," AND glar003>0)  )" #170308-00007#1 add
            ELSE
               LET g_sql=g_sql,
                         " AND glar002=",tm.syear,
         ...
         ELSE
            LET g_sql=g_sql,
                      ...
                      #                   原幣借-貸
#                      "                   SUM(CASE WHEN glaq003=0 THEN glaq010*-1 ELSE glaq010 END) oqcd,",  #180821-00002#1 mark
                      #180821-00002#1 add -s
                      "                   SUM((CASE ",
                      "                           WHEN glaq004 = 0 AND glaq003 <> 0 THEN ",
                      "                              glaq010 ",
      ...
      #170531-00014#3--add--end
         ...
         IF tm.edate=l_pdate_e2 AND tm.stus='1' THEN
            LET g_sql=g_sql,
                      ...
                      "             WHERE glarent=",g_enterprise," AND glarld='",g_glaald,"' ",
                      "               AND glar002=",tm.eyear," AND glar003<= ",tm.eperiod,
#                      "               AND glar003<>0 ", #160503-00037#1 add  #170308-00007#1 mark
                      "               AND glar003>0 ",   #170308-00007#1 add
                      "             GROUP BY glar001,glar009 ",
...
#   DECLARE aglq710_sel_glac_cs CURSOR FOR aglq710_sel_glac_pr         #190912-00006#1---mark
   DECLARE aglq710_sel_glac_cs SCROLL CURSOR FOR aglq710_sel_glac_pr  #190912-00006#1---add
   #160503-00025#1--add--end
...
#                    "       SUM(CASE WHEN glaq003=0 THEN glaq010 ELSE 0 END ),",
      #180821-00002#1 mark -e
      #180821-00002#1 add -s
      LET l_sql_pr5="SELECT SUM((CASE WHEN glaq004 = 0 AND glaq003 <> 0 THEN ",
                    "               glaq010 ",
                    "            ELSE ",
                    "               CASE WHEN glaq003 = 0 AND glaq004 = 0 THEN ",
                    "                  CASE WHEN (glaq010 > 0 AND (SELECT glac008 FROM glac_t WHERE glacent =  ",g_enterprise," AND glac002 = glaq002 AND glac001 = (SELECT glaa004 FROM glaa_t WHERE glaaent =  ",g_enterprise," AND glaald = glaqld)) = '1') THEN ",
...
#   LET l_sql_pr2="SELECT SUM(CASE WHEN glaq003=0 THEN glaq010*-1 ELSE glaq010 END),",  #180821-00002#1 mark
   #180821-00002#1 add -s
   ...
   IF tm.show_ye <> 'Y' OR tm.show_ad <> 'Y' THEN 
      #期初：抓取YE、AD傳票金額
#      LET l_sql_pr6="SELECT SUM(CASE WHEN glaq003=0 THEN glaq010*-1 ELSE glaq010 END),",  #180821-00002#1 mark
      #180821-00002#1 add -s
      LET l_sql_pr6="SELECT SUM((CASE ",
                    "               WHEN glaq004 = 0 AND glaq003 <> 0 THEN ",
                    "                  glaq010 ",
                    "               WHEN glaq003 = 0 AND glaq004 <> 0 THEN ",
                    "                  glaq010*-1 ",
...
#                    "       SUM(CASE WHEN glaq003=0 THEN glaq010 ELSE 0 END),",
      #180821-00002#1 mark -e
      #180821-00002#1 add -s
      LET l_sql_pr7="SELECT SUM((CASE WHEN glaq004 = 0 AND glaq003 <> 0 THEN ",
                    "               glaq010 ",
                    "            ELSE ",
                    "               CASE WHEN glaq003 = 0 AND glaq004 = 0 THEN ",
                    "                  CASE WHEN (glaq010 > 0 AND (SELECT glac008 FROM glac_t WHERE glacent =  ",g_enterprise," AND glac002 = glaq002 AND glac001 = (SELECT glaa004 FROM glaa_t WHERE glaaent =  ",g_enterprise," AND glaald = glaqld)) = '1') THEN ",
...
#                    "       SUM(CASE WHEN glaq003=0 THEN glaq010 ELSE 0 END ),",
      #180821-00002#1 mark -e
      #180821-00002#1 add -s
      LET l_sql_pr8="SELECT SUM((CASE WHEN glaq004 = 0 AND glaq003 <> 0 THEN ",
                    "               glaq010 ",
                    "            ELSE ",
                    "               CASE WHEN glaq003 = 0 AND glaq004 = 0 THEN ",
                    "                  CASE WHEN (glaq010 > 0 AND (SELECT glac008 FROM glac_t WHERE glacent =  ",g_enterprise," AND glac002 = glaq002 AND glac001 = (SELECT glaa004 FROM glaa_t WHERE glaaent =  ",g_enterprise," AND glaald = glaqld)) = '1') THEN ",
...
#                 "       SUM(CASE WHEN glaq003=0 THEN glaq010 ELSE 0 END),",
   #180821-00002#1 mark -e
   #180821-00002#1 add -s
   LET l_sql_pr4="SELECT SUM((CASE WHEN glaq004 = 0 AND glaq003 <> 0 THEN ",
                 "               glaq010 ",
                 "            ELSE ",
                 "               CASE WHEN glaq003 = 0 AND glaq004 = 0 THEN ",
                 "                  CASE WHEN (glaq010 > 0 AND (SELECT glac008 FROM glac_t WHERE glacent =  ",g_enterprise," AND glac002 = glaq002 AND glac001 = (SELECT glaa004 FROM glaa_t WHERE glaaent =  ",g_enterprise," AND glaald = glaqld)) = '1') THEN ",
   ...
   IF tm.show_ce <> 'Y' THEN
      #期初：抓取CE、XC傳票金額
#      LET l_sql_pr6_1="SELECT SUM(CASE WHEN glaq003=0 THEN glaq010*-1 ELSE glaq010 END),",  #180821-00002#1 mark
      #180821-00002#1 add -s
      LET l_sql_pr6_1="SELECT SUM((CASE ",
                      "               WHEN glaq004 = 0 AND glaq003 <> 0 THEN ",
                      "                  glaq010 ",
                      "               WHEN glaq003 = 0 AND glaq004 <> 0 THEN ",
                      "                  glaq010*-1 ",
...
#                      "       SUM(CASE WHEN glaq003=0 THEN glaq010 ELSE 0 END),",
      #180821-00002#1 mark -e
      #180821-00002#1 add -s
      LET l_sql_pr7_1="SELECT SUM((CASE WHEN glaq004 = 0 AND glaq003 <> 0 THEN ",
                      "               glaq010 ",
                      "            ELSE ",
                      "               CASE WHEN glaq003 = 0 AND glaq004 = 0 THEN ",
                      "                  CASE WHEN (glaq010 > 0 AND (SELECT glac008 FROM glac_t WHERE glacent =  ",g_enterprise," AND glac002 = glaq002 AND glac001 = (SELECT glaa004 FROM glaa_t WHERE glaaent =  ",g_enterprise," AND glaald = glaqld)) = '1') THEN ",
...
#                      "       SUM(CASE WHEN glaq003=0 THEN glaq010 ELSE 0 END ),",
      #180821-00002#1 mark -e
      #180821-00002#1 add -s
      LET l_sql_pr8_1="SELECT SUM((CASE WHEN glaq004 = 0 AND glaq003 <> 0 THEN ",
                      "               glaq010 ",
                      "            ELSE ",
                      "               CASE WHEN glaq003 = 0 AND glaq004 = 0 THEN ",
                      "                  CASE WHEN (glaq010 > 0 AND (SELECT glac008 FROM glac_t WHERE glacent =  ",g_enterprise," AND glac002 = glaq002 AND glac001 = (SELECT glaa004 FROM glaa_t WHERE glaaent =  ",g_enterprise," AND glaald = glaqld)) = '1') THEN ",
   ...
   LET l_sql = "SELECT COUNT(*)",
               ...
               "   AND glapdocdt BETWEEN '",tm.sdate,"' AND '",tm.edate,"'",
               "   AND glaq002 = ?"
   PREPARE aglq710_count_glaq FROM l_sql
   #191206-00013#1--add----end
   
   CALL cl_err_collect_init()
   LET l_success = TRUE
   
   #PGS(D)-02160 memo -S-  #200730-00057#6 -S-
   EXECUTE IMMEDIATE "alter session set cursor_sharing = force" #170308-00007#1 add
   #PGS(D)-02160 memo -E-  #200730-00057#6 -E-
...
#160411-00027#4--mark--end
         LET l_sql=l_sql_pr6_1,l_glaq002, #160411-00027#4 add
                   ...
                   #"                                         AND glac007='6' ",l_glac002_str,"))",    #170717-00001#1 mark
                   "                                         AND 1=1 ",l_glac002_str,"))",             #170717-00001#1
                   "         OR ",
                   "        (glap007='XC' AND glaq002 IN (SELECT glac002 FROM glac_t ",
                   "                                       WHERE glacent=",g_enterprise," AND glac001='",g_glaa004,"'",
                   #"                                         AND glac010 <> 'N' ",   #160824-00004#2 排除非费用类科目    #170719-00041#1 mark
...
#            CONTINUE FOREACH
#         END IF
         #180223-00009#1   mark   --(E)--
         IF tm.show_zero = 'N' THEN #191206-00013#1---add
            #180223-00009#1   add    --(S)--
            IF tm.curr_o = 'N' THEN 
               IF l_yeard=0 AND l_yearc=0 AND  #年初數
                  l_qcd=0 AND l_qcc=0 AND      #期初數
                  l_qjd=0 AND l_qjc=0 AND      #期間異動
            ...
            ELSE
               IF l_yeard=0 AND l_yearc=0 AND  #年初數
                  ...
                  l_qjd=0 AND l_qjc=0 AND      #期間異動
                  l_qmd=0 AND l_qmc=0 AND      #期末
                  l_oqmd=0 AND l_oqmc=0 AND    #原币期末
                  l_sumd=0 AND l_sumc=0 THEN   #本年累計，以上全部等於0時該筆科目資料不顯示
                  CONTINUE FOREACH
               END IF
            END IF 
            #180223-00009#1   add    --(E)-- 
          #191206-00013#1---add--str
           ELSE
               LET l_cnt = 0
               EXECUTE aglq710_count_glaq USING l_glac002 INTO l_cnt 
...
#                          "    OR (glar002=",tm.eyear," AND glar003<=",tm.eperiod," AND glar003<>0)  )" #170308-00007#1 mark
      ...
      ELSE
                       #        原幣借-貸
#          LET l_sql_qch=" SELECT SUM(CASE WHEN glaq003=0 THEN glaq010*-1 ELSE glaq010 END),",  #180821-00002#1 mark
          #180821-00002#1 add -s
          LET l_sql_qch="SELECT SUM((CASE ",
                        "               WHEN glaq004 = 0 AND glaq003 <> 0 THEN ",
                        "                  glaq010 ",
                        "               WHEN glaq003 = 0 AND glaq004 <> 0 THEN ",
                        "                  glaq010*-1 ",
...
#                      "        SUM(CASE WHEN glaq003=0 THEN glaq010 ELSE 0 END ),",
         ...
         #180821-00002#1 add -s
                      #        原幣借方金額
         LET l_sql_qj=" SELECT SUM((CASE WHEN glaq004 = 0 AND glaq003 <> 0 THEN ",
                      "                glaq010 ",
                      "             ELSE ",
                      "                CASE WHEN glaq003 = 0 AND glaq004 = 0 THEN ",
                      "                   CASE WHEN (glaq010 > 0 AND (SELECT glac008 FROM glac_t WHERE glacent =  ",g_enterprise," AND glac002 = glaq002 AND glac001 = (SELECT glaa004 FROM glaa_t WHERE glaaent =  ",g_enterprise," AND glaald = glaqld)) = '1') THEN ",
...
#                      "        SUM(CASE WHEN glaq003=0 THEN glaq010 ELSE 0 END),",
          ...
          #180821-00002#1 add -s
                       #        原幣借方金額
          LET l_sql_lj=" SELECT SUM((CASE WHEN glaq004 = 0 AND glaq003 <> 0 THEN ",
                       "                glaq010 ",
                       "             ELSE ",
                       "                CASE WHEN glaq003 = 0 AND glaq004 = 0 THEN ",
                       "                   CASE WHEN (glaq010 > 0 AND (SELECT glac008 FROM glac_t WHERE glacent =  ",g_enterprise," AND glac002 = glaq002 AND glac001 = (SELECT glaa004 FROM glaa_t WHERE glaaent =  ",g_enterprise," AND glaald = glaqld)) = '1') THEN ",
...
#         ELSE  #160824-00004#4 mark
         ...
         ELSE 
            LET g_sql=l_sql_t01,
                      ...
                      " AND glar001 = '",l_glac002,"'",
                      #" )"     #200730-00057#6 mark
                      " ) t51 " #PGS(D)-10500-add t51 200730-00057#6 add
         END IF
      ...
      END IF #170531-00014#3 add
         ...
         IF tm.show_ye <> 'Y' OR tm.show_ad <> 'Y' THEN 
            ...
            IF tm.syear=tm.eyear THEN #170531-00014#3 add
            #用于本年累计YE、AD凭证
...
PRIVATE FUNCTION aglq710_fetch1(p_flag)
   ...
   #CALL cl_progress_ing("Padding_b_fill1 ")  #170523-00020#1 xul add #181011-00024#1 mark
   CALL aglq710_b_fill1() 
...
#   CALL cl_set_comp_visible('oyeard,oyearc,yeard,yearc,yeard2,yearc2,yeard3,yearc3',FALSE) #160503-00025#1 mark
   ...
   LET tm.show_ad='Y'
   #150827-00036#2--add--end
   LET tm.show_zero = 'N'    #191206-00013#1--add-->当期有异动资料合计为0且期末为0者是否显示默认值给N
END FUNCTION
################################################################################
# Descriptions...: 科目说明
# Memo...........:
...
#160118-00012#1--unmark--str--   
   ...
   #是否按照幣別分頁
   IF NOT cl_null(g_glar009) THEN
      IF tm.show_acc = 'Y' THEN #PGS(D)-10500-add  #200730-00057#6 add
         LET g_sql=g_sql," AND glar009='",g_glar009,"'"
      #PGS(D)-10500 -S-  #200730-00057#6 -S-
      ELSE
         LET g_sql=g_sql," WHERE glar009='",g_glar009,"'"
      END IF
      #PGS(D)-10500 -E-  #200730-00057#6 -E-
   END IF 
...
PRIVATE FUNCTION aglq710_cmdrun()
   ...
   LET la_param.param[15] = tm.curr_o  #顯示原幣       #180517-00032#1 add
   LET la_param.param[16] = tm.curr_p  #按幣別分頁     #180517-00032#1 add
...
#                : 回传参数变量2   回传参数变量说明2
# Date & Author..: 200212-00031 By 05795
# Modify.........:
################################################################################
PRIVATE FUNCTION aglq710_cmdrun_760()
DEFINE la_param     RECORD
          prog         STRING,
          param        DYNAMIC ARRAY OF STRING
                       END RECORD
   ...
   LET la_param.param[5] = tm.eperiod   #截止期別
   LET la_param.param[6] = tm.ctype     #多本位幣顯示
   LET la_param.param[7] = tm.curr_o    #顯示原幣
   LET la_param.param[8] = tm.curr_p    #按幣別分頁  
   ...
   LET la_param.param[17] = tm.show_ad  #含審計調整傳票 
   LET la_param.param[18] = 'N'         #當期異動顯示否
...
################################################################################
# Descriptions...: 描述说明
# Date & Author..: #200221-00007#13 add By 13656
# Modify.........:
################################################################################
PRIVATE FUNCTION aglq710_get_buffer(p_dialog)
   ...
   LET tm.show_y        = p_dialog.getFieldBuffer('show_y')
   ...
   LET tm.show_zero     = p_dialog.getFieldBuffer('show_zero')
