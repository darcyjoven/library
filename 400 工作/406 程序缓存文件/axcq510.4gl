...
#該程式非freestyle程式!
{<section id="axcq510.description" type="s" >}
#應用 a00 樣板自動產生(Version:3)
#+ Standard Version.....: SD版次:0030(2018-07-12 16:33:24), PR版次:0030(2021-01-22 15:22:24)
#+ Customerized Version.: SD版次:0000(1900-01-01 00:00:00), PR版次:0000(1900-01-01 00:00:00)
#+ Build......: 000354
#+ Filename...: axcq510
#+ Description: 雜項出庫成本查詢作業
#+ Creator....: 01258(2014-09-03 00:00:00)
#+ Modifier...: 02295 -SD/PR- 14586
 
{</section>}
 
{<section id="axcq510.global" type="s" >}
#應用 i07 樣板自動產生(Version:92)
...
#170310-00068#12  17/04/14    By 09256      增加二次篩選
#170427-00041#6   17/06/09    By 08734      axcq查询作业，取平均值、小计及总计等再计算栏位的金额取位，需按照aooi150处的【成本金额小数字数】栏位的设置取位
#170704-00066#1   17/07/05    By 02111      axcq510_ins_temp() 必須給值 LET sr.inba008 = g_xcck_d[l_cnt1].inba008。
#170814-00012#41  17/09/14    By 10043      開窗q_ooeg001_4與q_ooeg001 內容相同,整批修改
#170915-00014#1   2017/09/26  By 02295      程式里面g_acc写死了aint302对应的ACC码,axcq510也会有报废的部分,所以不能写死aint302
#171201-00043#2   17/12/07    By 10500      还原170427-00041的取位调整
...
#180116-00012#2   2018/03/15  By 10461      查詢欄位的起始年度期別與截止年度期別的栏位检核调整
#180710-00041#1   2018/07/13  By 02295      axcq510单身成本中心栏位改成部门,并可以作为查询条件查询
#190322-00005#65  2019/03/25  By 12136      匯出excel，改寫為POI方式  
#190715-00009#1   2020/02/20  By 13130      修正汇总页签原因码取值不正确的问题
#201109-00009#128 2021/01/22  By 14586      模糊查詢功能，將使用者輸入的字串自動補入*
#end add-point
...
IMPORT JAVA com.fourjs.fgl.lang.FglRecord  #(ver:62) add
...
PRIVATE TYPE type_g_xcck_d        RECORD
   ...
   xcck002_desc LIKE xcbfl_t.xcbfl003, 
   xcck025 LIKE xcck_t.xcck025, 
   xcck025_desc LIKE ooefl_t.ooefl003, 
   ...
   xcck040_desc LIKE ooail_t.ooail003, 
...
#DEFINE g_glaa001             LIKE glaa_t.glaa001  #170427-00041#2 add #171201-00043#2 mark
#end add-point
...
DEFINE g_wc2_extend          STRING   #(ver:57)
...
DEFINE g_wc3_table1  STRING   #180710-00041#1
#end add-point
 
{</section>}
 
{<section id="axcq510.main" type="s" >}
...
MAIN
   ...
   LET g_forupd_sql = " SELECT xcckcomp,'',xcck004,xcck005,xcck003,'',xcckld,'','','',xcck001,''", 
                      " FROM xcck_t",
                      " WHERE xcckent= ? AND xcckld=? AND xcck001=? AND xcck003=? AND xcck004=? AND xcck005=? FOR UPDATE"
   ...
   CALL axcq510_chk_data_exist(g_forupd_sql)   #190806-00019
 
   LET g_sql = " SELECT DISTINCT t0.xcckcomp,t0.xcck004,t0.xcck005,t0.xcck003,t0.xcckld,t0.xcck001,t1.ooefl003 ,t2.xcatl003 ,t3.glaal002 ,t4.ooail003",
               " FROM xcck_t t0",
...
{<section id="axcq510.init" type="s" >}
#+ 瀏覽頁簽資料初始化
PRIVATE FUNCTION axcq510_init()
   ...
   LET g_data_owner = ""   #(ver:50)
   LET g_data_dept  = ""   #(ver:50)
   LET g_data_crtid = ""   #180201-00054
   LET g_data_crtdp = ""   #180201-00054
   LET g_bfill = "Y"
   LET g_detail_idx = 1
   LET g_detail_idx2 = 1
...
{<section id="axcq510.chk_data_exist" type="s" >}
#+ 異動前先確認該筆資料是否存在
PRIVATE FUNCTION axcq510_chk_data_exist(ps_sql)
   ...
   PREPARE axcq510_chk_data_exist_pre FROM ls_sql
   IF SQLCA.SQLCODE THEN
      INITIALIZE g_errparam TO NULL
      LET g_errparam.extend = 'PREPARE axcq510_chk_data_exist_pre ERROR'
...
{<section id="axcq510.ui_dialog" type="s" >}
#+ 功能選單
   ...
   WHILE TRUE
      ...
      DIALOG ATTRIBUTES(UNBUFFERED,FIELD ORDER FORM)
         ...
         BEFORE DIALOG
            ...
            #先填充browser資料
            #190108-00038 ---modify start---
         #  CALL axcq510_browser_fill("")   #mark
...
#wangxina 15/03/26 add   end
            ...
            IF cl_auth_chk_act("exporttoexcel") THEN
               ...
               CASE
                  WHEN lb_get_type = "poi"   #使用poi樣板
                     ...
                     ELSE
                        ...
                        #190322-00005#65 add 2019/03/25 By12136 ---(s)---
                        ...
                        END FOR
                        #190322-00005#65 add 2019/03/25 By12136 ---(e)---
                        #END add-point
               ...
               #(ver:62) --- end ---
                     ...
                     IF g_main_hidden = 1 THEN
                        ...
                        #add-point:ON ACTION exporttoexcel name="menu.exporttoexcel_browser"   #(ver:53)
                        
                        #END add-point
                        CALL cl_export_to_excel()
                        #add-point:ON ACTION exporttoexcel name="menu.exporttoexcel_browser_after"   #(ver:53)
                  ...
                  LET g_export_id[2]   = "s_detail2"
                        ...
                        CALL cl_export_to_excel_getpage()
                        CALL cl_export_to_excel()
                        #add-point:ON ACTION exporttoexcel name="menu.exporttoexcel_after"   #(ver:53)
         ...
         ON ACTION queryplansel
            LET g_action_choice = "queryplan" #ver:55
            ...
            IF NOT cl_null(ls_wc) THEN
               ...
               FOR li_idx = 1 TO la_wc.getLength()
                  CASE
                     WHEN la_wc[li_idx].tableid = "xcck_t"
                        LET g_wc = la_wc[li_idx].wc
                  #  因為假雙檔的單頭與單身是同一張表，所以若是用兩個WHEN去判斷的話，單身的條件會取不到
                  #  WHEN la_wc[li_idx].tableid = "xcck_t"
            ...
            END IF
            #重新顯示,此處不可用EXIT DIALOG, SUBDIALOG重讀會導致部分變數消失   #(ver:57)
         ...
         ON ACTION datarefresh
               LET g_action_choice = 'fetch'
               CALL axcq510_fetch('F')   #180611-00048
...
{<section id="axcq510.browser_search" type="s" >}
#+ 瀏覽頁簽資料搜尋
...
{<section id="axcq510.browser_fill" type="s" >}
#+ 瀏覽頁簽資料填充
PRIVATE FUNCTION axcq510_browser_fill(ps_page_action)
   ...
   #180710-00041#1---add---s
   DEFINE l_wc3_table1     STRING
   DEFINE l_wc3_table2     STRING
   DEFINE l_wc3_table3     STRING
   DEFINE l_wc3            STRING
   #180710-00041#1---add---e
   #end add-point
   ...
   IF g_wc3_table1 <> ' 1=1'THEN
      LET l_wc3_table1 = cl_replace_str(g_wc3_table1,'xcck025','inbf002')
      LET l_wc3_table2 = cl_replace_str(g_wc3_table1,'xcck025','inba004')
      LET l_wc3_table3 = cl_replace_str(g_wc3_table1,'xcck025','inbj017')
      LET l_wc3 = " (",g_wc3_table1,"",
                  "  OR EXISTS(SELECT 1 FROM inbf_t WHERE inbfent=xcckent AND inbfdocno=xcck006 AND ",l_wc3_table1,")",
                  "  OR EXISTS(SELECT 1 FROM inba_t WHERE inbaent=xcckent AND inbadocno=xcck006 AND ",l_wc3_table2,")",
                  "  OR EXISTS(SELECT 1 FROM inbj_t WHERE inbjent=xcckent AND inbjdocno=xcck006 AND inbjseq=xcck007 AND ",l_wc3_table3,"))"
   ELSE
      LET l_wc3 = " 1=1"
   END IF
   LET g_wc = g_wc," AND ",l_wc3
   #180710-00041#1---add---e
   #end add-point    
...
#150805-00001#8add
   ...
   LET g_sql = " SELECT COUNT(1) FROM (",l_sub_sql,") tsed "
#150805-00001#8add  
   #end add-point
   ...
   ELSE
      ...
      FOREACH browse_cur INTO g_browser[g_cnt].b_xcckld,g_browser[g_cnt].b_xcck001,g_browser[g_cnt].b_xcck003,g_browser[g_cnt].b_xcck004,g_browser[g_cnt].b_xcck005 
         IF SQLCA.SQLCODE THEN
            INITIALIZE g_errparam TO NULL 
...
{<section id="axcq510.ui_headershow" type="s" >}
#+ 單頭資料重新顯示
PRIVATE FUNCTION axcq510_ui_headershow()
   ...
   IF g_browser.getLength() > 0 AND g_current_idx > 0 THEN   #200828-00027檢查後進入,避免資料溢位 (代追單)
      ...
      EXECUTE axcq510_master_referesh USING g_xcck_m.xcckld,g_xcck_m.xcck001,g_xcck_m.xcck003,g_xcck_m.xcck004,g_xcck_m.xcck005 INTO g_xcck_m.xcckcomp,g_xcck_m.xcck004,g_xcck_m.xcck005,g_xcck_m.xcck003,g_xcck_m.xcckld,g_xcck_m.xcck001,g_xcck_m.xcckcomp_desc,g_xcck_m.xcck003_desc,g_xcck_m.xcckld_desc, 
          g_xcck_m.xcck001_desc
      CALL axcq510_show()
   END IF   #200828-00027
...
{<section id="axcq510.ui_detailshow" type="s" >}
#+ 單身資料重新顯示
...
{<section id="axcq510.ui_browser_refresh" type="s" >}
#+ 瀏覽頁簽資料重新顯示
...
{<section id="axcq510.construct" type="s" >}
#+ QBE資料查詢
PRIVATE FUNCTION axcq510_construct()
   ...
   DIALOG ATTRIBUTES(UNBUFFERED,FIELD ORDER FORM)
      ...
      CONSTRUCT BY NAME g_wc ON xcckcomp,xcck003,xcckld,xcck001
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck001
      ...
      CONSTRUCT g_wc2_table1 ON xcck002,xcck021,xcck021_desc,xcck006,xcck007,xcck008,xcck009,xcck013,xcck010,xcck011,xcck015,xcck015_desc,xcck017,xcck044,xcck040,xcck042,xcck201,xcck282,xcck202a,xcck202b,xcck202c,xcck202d,xcck202e,xcck202f,xcck202g,xcck202h,xcck202
           FROM s_detail1[1].xcck002,s_detail1[1].xcck021,s_detail1[1].xcck021_desc,s_detail1[1].xcck006,s_detail1[1].xcck007,s_detail1[1].xcck008,s_detail1[1].xcck009,s_detail1[1].xcck013,s_detail1[1].xcck010,s_detail1[1].xcck011,s_detail1[1].xcck015,s_detail1[1].xcck015_desc,s_detail1[1].xcck017,s_detail1[1].xcck044, 
               s_detail1[1].xcck040,s_detail1[1].xcck042,s_detail1[1].xcck201,s_detail1[1].xcck282,s_detail1[1].xcck202a,s_detail1[1].xcck202b,s_detail1[1].xcck202c,s_detail1[1].xcck202d,s_detail1[1].xcck202e,s_detail1[1].xcck202f,s_detail1[1].xcck202g,s_detail1[1].xcck202h,s_detail1[1].xcck202
         ...
         AFTER FIELD xcck021_desc
            ...
            #201109-00009#128---add---(S)
            CALL FGL_DIALOG_GETBUFFER() RETURNING lp_str
            IF NOT cl_null(lp_str) THEN
               LET lp_str = cl_ap_code_fuzzyquery(lp_str)   #模糊查詢功能
               CALL FGL_DIALOG_SETBUFFER(lp_str)
            END IF
            #201109-00009#128---add---(E)
            #END add-point
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck021_desc
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck007
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck008
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck009
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck013
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck015_desc
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck017
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck042
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck201
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck282
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck202a
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck202b
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck202c
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck202d
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck202e
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck202f
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck202g
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck202h
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck202
      ...
      INPUT g_yy1,g_mm1,g_yy2,g_mm2 FROM xcck004,xcck005,b_xcck004,b_xcck005
         AFTER FIELD xcck004 
            IF NOT cl_null(g_yy1) AND NOT cl_null(g_yy2) THEN
                IF g_yy1 > g_yy2 THEN
                   INITIALIZE g_errparam TO NULL
                 # LET g_errparam.code = 'acr-00064'  #180116-00012#2 mark
                   LET g_errparam.code = 'axc-00959'  #180116-00012#2 add
                   LET g_errparam.extend = ''
                   LET g_errparam.popup = TRUE
                   CALL cl_err()
                   LET  g_yy2 = g_yy1                  #180116-00012#2 add
		             LET  g_mm2 = g_mm1                  #180116-00012#2 add
               #   NEXT FIELD xcck004                  #180116-00012#2 mark
                END IF
             END IF
               #180116-00012#2  add(s)
		         IF NOT cl_null(g_yy1) AND NOT cl_null(g_yy2) AND NOT cl_null(g_mm1) AND NOT cl_null(g_mm2)  THEN
                   IF (g_yy1 = g_yy2) AND (g_mm1 > g_mm2) THEN
                      INITIALIZE g_errparam TO NULL
                      LET g_errparam.code = 'axc-00959'  
                      LET g_errparam.extend = ''
                      LET g_errparam.popup = TRUE
                      CALL cl_err()
                      LET  g_yy2 = g_yy1                  
                   ...
                   END IF
                END IF
               #180116-00012#2  add(e)
         AFTER FIELD xcck005
            IF NOT cl_null(g_mm1) AND NOT cl_null(g_mm2) THEN
               IF g_yy1 = g_yy2 AND g_mm1 > g_mm2 THEN
                  INITIALIZE g_errparam TO NULL
           #      LET g_errparam.code = 'acr-00067'  #180116-00012#2 mark
                  LET g_errparam.code = 'axc-00959'  #180116-00012#2 add
                  LET g_errparam.extend = ''
                  LET g_errparam.popup = TRUE
                  CALL cl_err()
                  LET  g_yy2 = g_yy1                  #180116-00012#2 add
		            LET  g_mm2 = g_mm1                  #180116-00012#2 add
           #      NEXT FIELD xcck005                  #180116-00012#2 mark
               END IF
            END IF
         AFTER FIELD b_xcck004
            IF NOT cl_null(g_yy1) AND NOT cl_null(g_yy2) THEN
                IF g_yy1 > g_yy2 THEN
                   INITIALIZE g_errparam TO NULL
                 # LET g_errparam.code = 'acr-00064' #180116-00012#2 mark
                   LET g_errparam.code = 'axc-00960'    #180116-00012#2 add
                   LET g_errparam.extend = ''
                   LET g_errparam.popup = TRUE
                   CALL cl_err()
                   LET  g_yy1 = g_yy2                  #180116-00012#2 add
		             LET  g_mm1 = g_mm2                  #180116-00012#2 add
                #  NEXT FIELD b_xcck004                #180116-00012#2 mark
                END IF
             END IF
               #180116-00012#2 add(s)
		         IF NOT cl_null(g_yy1) AND NOT cl_null(g_yy2) AND NOT cl_null(g_mm1) AND NOT cl_null(g_mm2)  THEN
         ...
         AFTER FIELD b_xcck005   
            IF NOT cl_null(g_mm1) AND NOT cl_null(g_mm2) THEN
              ...
              #   LET g_errparam.code = 'acr-00067'   #180116-00012#2 mark
                  ...
                  LET  g_yy1 = g_yy2                  #180116-00012#2 add
		            LET  g_mm1 = g_mm2                  #180116-00012#2 add
             #    NEXT FIELD b_xcck005                #180116-00012#2 mark
               END IF
            END IF     
      END INPUT
      #150805-00001#8add
      ...
      #180710-00041#1---add---s
      CONSTRUCT g_wc3_table1 ON xcck025 FROM s_detail1[1].xcck025
...
{<section id="axcq510.query" type="s" >}
#+ 資料查詢QBE功能準備
...
{<section id="axcq510.fetch" type="s" >}
#+ 指定PK後抓取單頭其他資料
PRIVATE FUNCTION axcq510_fetch(p_flag)
   ...
   #重讀DB,因TEMP有不被更新特性
   EXECUTE axcq510_master_referesh USING g_xcck_m.xcckld,g_xcck_m.xcck001,g_xcck_m.xcck003,g_xcck_m.xcck004,g_xcck_m.xcck005 INTO g_xcck_m.xcckcomp,g_xcck_m.xcck004,g_xcck_m.xcck005,g_xcck_m.xcck003,g_xcck_m.xcckld,g_xcck_m.xcck001,g_xcck_m.xcckcomp_desc,g_xcck_m.xcck003_desc,g_xcck_m.xcckld_desc,g_xcck_m.xcck001_desc 
...
{<section id="axcq510.insert" type="s" >}
#+ 資料新增
PRIVATE FUNCTION axcq510_insert()
   ...
   LET g_wc2_extend = " 1=1" #清空查詢方案條件 #191129-00016#1 add
   ...
   #將資料顯示到畫面上
   DISPLAY BY NAME g_xcck_m.xcckcomp,g_xcck_m.xcckcomp_desc,g_xcck_m.xcck004,g_xcck_m.xcck005,g_xcck_m.xcck003,g_xcck_m.xcck003_desc,g_xcck_m.xcckld,g_xcck_m.xcckld_desc,g_xcck_m.b_xcck004,g_xcck_m.b_xcck005,g_xcck_m.xcck001,g_xcck_m.xcck001_desc
...
{<section id="axcq510.modify" type="s" >}
#+ 資料修改
...
#  ERROR ""   #(ver:63) mark
   ...
   #200804-00006 ---start---
   #add-point:fetch前置處理  name="modify.pre_fetch"
   
   #end add-point
   FETCH axcq510_cl
      IF SQLCA.SQLCODE THEN
         INITIALIZE g_errparam TO NULL
         LET g_errparam.extend = "FETCH axcq510_cl:"
         LET g_errparam.code = SQLCA.SQLCODE   #(ver:49)
         LET g_errparam.popup = TRUE
         CLOSE axcq510_cl
         CALL s_transaction_end('N','0')
         ...
         RETURN
      END IF
   #add-point:fetch後置處理  name="modify.post_fetch"
   ...
   IF g_action_choice = "modify" OR g_action_choice = "modify_detail" THEN   #191114-00036
      LET lp_cnt = 0
      EXECUTE axcq510_chk_data_exist_pre INTO lp_cnt USING g_enterprise,g_xcck_m.xcckld,g_xcck_m.xcck001,g_xcck_m.xcck003,g_xcck_m.xcck004,g_xcck_m.xcck005
...
{<section id="axcq510.input" type="s" >}
#+ 資料輸入
PRIVATE FUNCTION axcq510_input(p_cmd)
   ...
   LET l_insert = FALSE
#  LET g_action_choice = ""   #(ver:50) mark
 
   #add-point:input段define_sql name="input.define_sql"
   
   #end add-point 
   LET g_forupd_sql = "SELECT xcck002,xcck025,xcck021,xcck006,xcck007,xcck008,xcck009,xcck013,xcck010,xcck011,xcck015,xcck017,xcck044,xcck040,xcck042,xcck201,xcck282,xcck202a,xcck202b,xcck202c,xcck202d,xcck202e,xcck202f,xcck202g,xcck202h,xcck202 FROM xcck_t WHERE xcckent=? AND xcckld=? AND xcck001=?  
       AND xcck003=? AND xcck004=? AND xcck005=? AND xcck002=? AND xcck006=? AND xcck007=? AND xcck008=? AND xcck009=? FOR UPDATE"
   ...
   DISPLAY BY NAME g_xcck_m.xcckcomp,g_xcck_m.xcck004,g_xcck_m.xcck005,g_xcck_m.xcck003,g_xcck_m.xcckld,g_xcck_m.b_xcck004,g_xcck_m.b_xcck005,g_xcck_m.xcck001
...
{<section id="axcq510.input.head" type="s" >}
   
      #單頭段
      INPUT BY NAME g_xcck_m.xcckcomp,g_xcck_m.xcck004,g_xcck_m.xcck005,g_xcck_m.xcck003,g_xcck_m.xcckld,g_xcck_m.b_xcck004,g_xcck_m.b_xcck005,g_xcck_m.xcck001 
...
#此段落由子樣板a19產生
 ...
 #欄位檢查
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcckcomp
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck004
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck005
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD b_xcck004
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD b_xcck005
 ...
 #欄位開窗
         ...
         AFTER INPUT
            ...
            IF p_cmd = 'u' THEN
               ...
               UPDATE xcck_t SET (xcckcomp,xcck004,xcck005,xcck003,xcckld,xcck001) = (g_xcck_m.xcckcomp,g_xcck_m.xcck004,g_xcck_m.xcck005,g_xcck_m.xcck003,g_xcck_m.xcckld,g_xcck_m.xcck001)
               ...
               CASE
                  ...
                  #  CALL s_transaction_end('N','0')   #(ver:52)
                     INITIALIZE g_errparam TO NULL 
                     LET g_errparam.extend = "xcck_t" 
...
{<section id="axcq510.input.body" type="s" >}
      #Page1 預設值產生於此處
         ...
         BEFORE ROW
            ...
            THEN
               ...
               IF SQLCA.SQLCODE THEN   #(ver:49)
                  ...
                  LET l_lock_sw='Y'
               ELSE
                  FETCH axcq510_bcl INTO g_xcck_d[l_ac].xcck002,g_xcck_d[l_ac].xcck025,g_xcck_d[l_ac].xcck021,g_xcck_d[l_ac].xcck006,g_xcck_d[l_ac].xcck007,g_xcck_d[l_ac].xcck008,g_xcck_d[l_ac].xcck009,g_xcck_d[l_ac].xcck013,g_xcck_d[l_ac].xcck010,g_xcck_d[l_ac].xcck011,g_xcck_d[l_ac].xcck015,g_xcck_d[l_ac].xcck017, 
                      g_xcck_d[l_ac].xcck044,g_xcck_d[l_ac].xcck040,g_xcck_d[l_ac].xcck042,g_xcck_d[l_ac].xcck201,g_xcck_d[l_ac].xcck282,g_xcck_d[l_ac].xcck202a,g_xcck_d[l_ac].xcck202b,g_xcck_d[l_ac].xcck202c,g_xcck_d[l_ac].xcck202d,g_xcck_d[l_ac].xcck202e,g_xcck_d[l_ac].xcck202f,g_xcck_d[l_ac].xcck202g, 
      ...
      LET g_xcck_d[l_ac].xcck202h = "0"
         ...
         AFTER INSERT
            ...
            IF l_count = 0 THEN 
               ...
               LET lp_js = util.JSON.stringify(g_xcck_d[l_ac])
               #190412-00022 --- end ---
               ...
               INSERT INTO xcck_t
                     ...
                     VALUES(g_enterprise,
                            ...
                            g_xcck_d[l_ac].xcck002,g_xcck_d[l_ac].xcck006,g_xcck_d[l_ac].xcck007,g_xcck_d[l_ac].xcck008,g_xcck_d[l_ac].xcck009
                            ,g_xcck_d[l_ac].xcck025,g_xcck_d[l_ac].xcck021,g_xcck_d[l_ac].xcck013,g_xcck_d[l_ac].xcck010,g_xcck_d[l_ac].xcck011,g_xcck_d[l_ac].xcck015,g_xcck_d[l_ac].xcck017,g_xcck_d[l_ac].xcck044,g_xcck_d[l_ac].xcck040,g_xcck_d[l_ac].xcck042,g_xcck_d[l_ac].xcck201,g_xcck_d[l_ac].xcck282, 
                                g_xcck_d[l_ac].xcck202a,g_xcck_d[l_ac].xcck202b,g_xcck_d[l_ac].xcck202c,g_xcck_d[l_ac].xcck202d,g_xcck_d[l_ac].xcck202e,g_xcck_d[l_ac].xcck202f,g_xcck_d[l_ac].xcck202g,g_xcck_d[l_ac].xcck202h,g_xcck_d[l_ac].xcck202)
   ...
   #161130-00056
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck002
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck025
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck021
...
#此ctrlp無內容         ON ACTION controlp INFIELD xcck021_desc
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck006
...
#此ctrlp無內容         ON ACTION controlp INFIELD xcck013
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck010
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck011
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck015
...
#此ctrlp無內容         ON ACTION controlp INFIELD xcck017
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck044
         ...
         #應用 a03 樣板自動產生(Version:3)
#此ctrlp無內容         ON ACTION controlp INFIELD xcck040
...
#此ctrlp無內容         ON ACTION controlp INFIELD xcck202
         ...
         ON ROW CHANGE
            ...
            ELSE
               ...
               UPDATE xcck_t SET (xcckld,xcck001,xcck003,xcck004,xcck005,xcck002,xcck025,xcck021,xcck006,xcck007,xcck008,xcck009,xcck013,xcck010,xcck011,xcck015,xcck017,xcck044,xcck040,xcck042,xcck201,xcck282,xcck202a,xcck202b,xcck202c,xcck202d,xcck202e,xcck202f,xcck202g,xcck202h,xcck202) = (g_xcck_m.xcckld, 
                   g_xcck_m.xcck001,g_xcck_m.xcck003,g_xcck_m.xcck004,g_xcck_m.xcck005,g_xcck_d[l_ac].xcck002,g_xcck_d[l_ac].xcck025,g_xcck_d[l_ac].xcck021,g_xcck_d[l_ac].xcck006,g_xcck_d[l_ac].xcck007,g_xcck_d[l_ac].xcck008,g_xcck_d[l_ac].xcck009,g_xcck_d[l_ac].xcck013,g_xcck_d[l_ac].xcck010,g_xcck_d[l_ac].xcck011, 
                   g_xcck_d[l_ac].xcck015,g_xcck_d[l_ac].xcck017,g_xcck_d[l_ac].xcck044,g_xcck_d[l_ac].xcck040,g_xcck_d[l_ac].xcck042,g_xcck_d[l_ac].xcck201,g_xcck_d[l_ac].xcck282,g_xcck_d[l_ac].xcck202a,g_xcck_d[l_ac].xcck202b,g_xcck_d[l_ac].xcck202c,g_xcck_d[l_ac].xcck202d,g_xcck_d[l_ac].xcck202e, 
               ...
               CALL axcq510_update_b('xcck_t',gs_keys,gs_keys_bak,"'1'")
                     #資料多語言用-增/改
                        ...
                        #161130-00056
                     #修改歷程記錄(單身修改)
                     CALL axcq510_set_all_pk_array()   #(ver:50)
                     ...
                     IF NOT cl_log_modified_record(g_log1,g_log2) THEN 
                        CALL s_transaction_end('N','0')
                     END IF
                     CALL axcq510_set_pk_array()   #(ver:50)
      ...
      BEFORE DIALOG
         ...
         #新增時強制從單頭開始填
         IF p_cmd = 'a' THEN
            NEXT FIELD xcckld #170906-00011#1
         ELSE
            CASE g_aw
               WHEN "s_detail1"
                  NEXT FIELD xcck002 #170906-00011#1
...
{<section id="axcq510.show" type="s" >}
#+ 單頭資料重新顯示及單身資料重抓
PRIVATE FUNCTION axcq510_show()
   ...
   #170427-00041#2 add(S) #171201-00043#2 mark-S
#   LET g_glaa001=''  
#   SELECT glaa001 INTO g_glaa001
#     FROM glaa_t
#    WHERE glaaent = g_enterprise
#      AND glaald  = g_xcck_m.xcckld
   #170427-00041#2 add(E) #171201-00043#2 mark-E
   #end add-point
...
{<section id="axcq510.ref_show" type="s" >}
#+ 帶出reference資料
...
{<section id="axcq510.reproduce" type="s" >}
#+ 資料複製
...
{<section id="axcq510.detail_reproduce" type="s" >}
#+ 單身自動複製
...
#  DROP TABLE axcq510_detail   #180110-00034 mark
   
END FUNCTION
 
{</section>}
 
{<section id="axcq510.delete" type="s" >}
#+ 資料刪除
PRIVATE FUNCTION axcq510_delete()
   ...
   DEFINE  l_fields        DYNAMIC ARRAY OF STRING
   DEFINE  l_var_keys_bak  DYNAMIC ARRAY OF STRING
   DEFINE li_idx           LIKE type_t.num10   #171006-00020
   ...
   #先確定該table有ownid
   IF cl_getField("xcck_t","") THEN
   #  LET g_sql = "SELECT , ",   #180201-00054 mark
      LET g_sql = "SELECT ,,, ",   #180201-00054 modify
                   " FROM xcck_t ",
                  " WHERE xcckent = " ||g_enterprise|| " AND xcckld = '",g_xcck_m.xcckld,"' "
                                                ," AND xcck001 = '",g_xcck_m.xcck001,"'"
                                                ," AND xcck003 = '",g_xcck_m.xcck003,"'"
      ...
      #200804-00006 ---start---
      #add-point:ownid檢查前 name="delete.body.b_ownid"
      
      #end add-point
      #200804-00006 --- end ---
 
      PREPARE axcq510_chk_own_pre FROM g_sql
      DECLARE axcq510_chk_own_cus CURSOR FOR axcq510_chk_own_pre
      
   #  FOREACH axcq510_chk_own_cus INTO g_data_owner,g_data_dept   #180201-00054 mark
      FOREACH axcq510_chk_own_cus INTO g_data_owner,g_data_dept,g_data_crtid,g_data_crtdp   #180201-00054 modify
         IF SQLCA.SQLCODE THEN
            INITIALIZE g_errparam TO NULL 
            LET g_errparam.extend = "DELETE FUNCTION CHECK OWNER:" 
            LET g_errparam.code = SQLCA.SQLCODE
            LET g_errparam.popup = TRUE 
            CALL s_transaction_end('N','0')
            CALL cl_err()
   ...
   IF cl_ask_del_master() THEN              #確認一下
      ...
      #(ver:50) 將mark拿掉 ---start---
      #頭先不紀錄
      LET g_log1 = util.JSON.stringify(g_xcck_m_t)   #(ver:50)
      #執行刪除時，不會有新值，所以新值欄位傳送空白             #(ver:50)
   #  LET g_log2 = util.JSON.stringify(g_xcck_m)     #(ver:50)
      IF NOT cl_log_modified_record(g_log1,'') THEN             #(ver:50)
         CLOSE axcq510_cl
         CALL s_transaction_end('N','0')
         RETURN 
      END IF
      ...
      #清空單頭資料
      INITIALIZE g_xcck_m.* TO NULL   #180420-00043
...
{<section id="axcq510.b_fill" type="s" >}
#+ 單身陣列填充
PRIVATE FUNCTION axcq510_b_fill(p_wc2)
   ...
   #end add-point
   DEFINE p_wc2      STRING
   DEFINE ls_owndept_list  STRING  #(ver:50) add
   DEFINE ls_ownuser_list  STRING  #(ver:50) add
...
#150805-00001#8add
   ...
   LET g_sql = "SELECT  DISTINCT xcck002,xcck025,xcck021,xcck006,xcck007,xcck008,xcck009,xcck013,xcck010,xcck011,xcck015,xcck017,xcck044,xcck040,xcck042,xcck201,xcck282,xcck202a,xcck202b,xcck202c,xcck202d,xcck202e,xcck202f,xcck202g,xcck202h,xcck202,t1.xcbfl003 ,t2.ooefl003 ,t4.imaal003 ,t5.oocal003 , 
   ...
   IF axcq510_fill_chk(1) THEN
      #若有觸發條件(g_wc2)異動的任何行為都需添加在下方條件中,避免條件無正常刷新 ver:55
      IF (g_action_choice = "query" OR cl_null(g_action_choice) OR g_action_choice = "queryplan" ) #ver:55
         OR ((g_action_choice = "modify" OR g_action_choice = "modify_detail")     #(ver:50)
             AND (NOT cl_null(ls_owndept_list) OR NOT cl_null(ls_ownuser_list)))   #(ver:50)
         OR (g_action_choice = "insert")                                           #(ver:54)
      ...
      LET g_sql = l_sql, " ORDER BY xcck_t.xcck002,xcck_t.xcck025,xcck_t.xcck021,xcck_t.xcck006,xcck_t.xcck007,xcck_t.xcck008,xcck_t.xcck009" #ODI-ORA#
      LET l_xcck202_sum = 0
   ...
   #  OPEN b_fill_cs USING g_enterprise,g_xcck_m.xcckld,g_xcck_m.xcck001,g_xcck_m.xcck003,g_xcck_m.xcck004,g_xcck_m.xcck005   #(ver:49)
      ...
      FOREACH b_fill_cs USING g_enterprise,g_xcck_m.xcckld,g_xcck_m.xcck001,g_xcck_m.xcck003,g_xcck_m.xcck004,g_xcck_m.xcck005 INTO g_xcck_d[l_ac].xcck002,g_xcck_d[l_ac].xcck025,g_xcck_d[l_ac].xcck021,g_xcck_d[l_ac].xcck006,g_xcck_d[l_ac].xcck007,g_xcck_d[l_ac].xcck008,g_xcck_d[l_ac].xcck009,g_xcck_d[l_ac].xcck013, 
          g_xcck_d[l_ac].xcck010,g_xcck_d[l_ac].xcck011,g_xcck_d[l_ac].xcck015,g_xcck_d[l_ac].xcck017,g_xcck_d[l_ac].xcck044,g_xcck_d[l_ac].xcck040,g_xcck_d[l_ac].xcck042,g_xcck_d[l_ac].xcck201,g_xcck_d[l_ac].xcck282,g_xcck_d[l_ac].xcck202a,g_xcck_d[l_ac].xcck202b,g_xcck_d[l_ac].xcck202c,g_xcck_d[l_ac].xcck202d, 
          g_xcck_d[l_ac].xcck202e,g_xcck_d[l_ac].xcck202f,g_xcck_d[l_ac].xcck202g,g_xcck_d[l_ac].xcck202h,g_xcck_d[l_ac].xcck202,g_xcck_d[l_ac].xcck002_desc,g_xcck_d[l_ac].xcck025_desc,g_xcck_d[l_ac].xcck010_desc,g_xcck_d[l_ac].xcck044_desc,g_xcck_d[l_ac].xcck040_desc   #(ver:49)
...
{<section id="axcq510.idx_chk" type="s" >}
#+ 顯示正確的單身資料筆數
PRIVATE FUNCTION axcq510_idx_chk()
   ...
   LET g_curr_diag = ui.DIALOG.getCurrent() #(ver:56)
...
{<section id="axcq510.b_fill2" type="s" >}
#+ 單身陣列填充2
PRIVATE FUNCTION axcq510_b_fill2(pi_idx)
   ...
   DEFINE l_oocql004_sql3   STRING       #190715-00009#1 add 13130
   #160414-00018#45--e
   DEFINE l_xcck012         LIKE xcck_t.xcck012  #190715-00009#1 add 13130
   #end add-point
   ...
   #LET l_xcbfl003_sql = "(SELECT xcbfl003 FROM xcbfl_t WHERE xcbflent=xcckent AND xcbfl001=xcck002 AND xcbfl002='",g_dlang,"')" #170310-00068#12 mark
   LET l_xcbfl003_sql = "(SELECT xcbfl003 FROM xcbfl_t WHERE xcbflent=xcckent AND xcbfl001=xcck002 AND xcbflcomp=xcckcomp AND xcbfl002='",g_dlang,"')" #170310-00068#12 add 
   ...
   LET l_oocql004_sql3= "(SELECT oocql004 FROM oocql_t WHERE oocqlent='",g_enterprise,"' AND oocql002=xcck021 AND oocql003='",g_dlang,"' and oocql001 in (SELECT distinct gzcb004   FROM gzcb_t WHERE gzcb001 = '24' AND gzcb002 = xcck012))"  #190715-00009#1 add 13130   
   
   #160414-00018#45--e
   
   
    #LET g_sql = "SELECT  UNIQUE xcck002,'',xcck025,'',xcck021,'','','',xcck010,'','',xcck011,xcck044,'',SUM(xcck009 * xcck201),SUM(xcck202 * xcck009) FROM xcck_t",    #160414-00018#45 mark   
    #190715-00009#1 mark-s 13130
#   #160414-00018#45 add-s
#   LET g_sql = "SELECT  UNIQUE xcck002,",l_xcbfl003_sql,",xcck025,",l_ooefl003_sql,",xcck021,",
#               "        ",l_oocql004_sql2,",",l_imag011_sql,",",l_oocql004_sql2,",xcck010,",l_imaal003_sql,",",
#               "        ",l_imaal004_sql,",xcck011,xcck044,",l_oocal003_sql,",",
#               "        SUM(xcck009 * xcck201),SUM(xcck202 * xcck009) FROM xcck_t",  
#  #160414-00018#45 add-e
#               #" WHERE xcckent= ? AND xcckld=? AND xcck001=? AND xcck003=? AND xcck004=? AND xcck005=? AND xcck055 IN ('309','3091','3092')"  #   #160414-00018#45 mark
#               " WHERE xcckent= ? AND xcckld=? AND xcck001=? AND xcck003=? AND xcck055 IN ('309','3091','3092') AND (xcck004*12+xcck005 BETWEEN ",g_yy1,"*12+",g_mm1," AND ",g_yy2,"*12+",g_mm2,") AND ", g_wc  #160414-00018#45 add
   #190715-00009#1 mark-e 13130
 
   #190715-00009#1 add-s 13130
    LET g_sql = "SELECT  UNIQUE xcck002,",l_xcbfl003_sql,",xcck025,",l_ooefl003_sql,",xcck021,",
                "        ",l_oocql004_sql3,",",l_imag011_sql,",",l_oocql004_sql2,",xcck010,",l_imaal003_sql,",",
                "        ",l_imaal004_sql,",xcck011,xcck044,",l_oocal003_sql,",",
                "        SUM(xcck009 * xcck201),SUM(xcck202 * xcck009),xcck012 FROM xcck_t",  
  ...
  #LET g_sql = g_sql, " GROUP BY xcck002,xcck025,xcck021,xcck010,xcck011,xcck044,xcckent ", #170310-00068#12 mark
  #LET g_sql = g_sql, " GROUP BY xcck002,xcck025,xcck021,xcck010,xcck011,xcck044,xcckent,xcckcomp ", #170310-00068#12 add      #190715-00009#1 mark-e 13130
  LET g_sql = g_sql, " GROUP BY xcck002,xcck012,xcck025,xcck021,xcck010,xcck011,xcck044,xcckent,xcckcomp ",#190715-00009#1 add-e 13130
  ...
  #160414-00018#45 mod --e
   ...
   FOREACH b_fill_cs1 INTO g_xcck2_d[l_ac].*,l_xcck012 
      IF SQLCA.sqlcode THEN
         INITIALIZE g_errparam TO NULL 
...
#            CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,g_xcck2_d[l_ac].xcck202_1,'5') RETURNING g_xcck2_d[l_ac].xcck202_1
            #170427-00041#2 add(E) #171201-00043#2 mark-E
            LET l_ac = l_ac + 1
...
#   LET g_xcck2_d[l_ac].xcck002_1 = cl_getmsg("axc-00205",g_lang)
  #CALL cl_get_para(g_enterprise,g_xcck_m.xcckcomp,'S-FIN-6001') RETURNING g_para_data   #采用成本域否  #160414-00018#45 mark
   ...
   END IF
   #170427-00041#2 add(S) #171201-00043#2 mark-S
#   IF cl_null(l_xcck202_sum) THEN LET l_xcck202_sum=0 END IF
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202_sum,'5') RETURNING l_xcck202_sum
   #170427-00041#2 add(E) #171201-00043#2 mark-E
   #fengmy 150123 modify--begin
...
#   LET g_xcck2_d[l_ac].xcck002_1 = cl_getmsg("axc-00205",g_lang)
  #CALL cl_get_para(g_enterprise,g_xcck_m.xcckcomp,'S-FIN-6001') RETURNING g_para_data   #采用成本域否   #160414-00018#45 mark
   ...
   END IF
   #170427-00041#2 add(S) #171201-00043#2 mark-S
#   IF cl_null(l_xcck202_total) THEN LET l_xcck202_total=0 END IF
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202_total,'5') RETURNING l_xcck202_total
   #170427-00041#2 add(E) #171201-00043#2 mark-E
   #fengmy 150123 modify--begin
...
{<section id="axcq510.before_delete" type="s" >}
#+ 單身db資料刪除
...
{<section id="axcq510.delete_b" type="s" >}
#+ 刪除單身後其他table連動
...
{<section id="axcq510.insert_b" type="s" >}
#+ 新增單身後其他table連動
...
{<section id="axcq510.update_b" type="s" >}
#+ 修改單身後其他table連動
...
{<section id="axcq510.key_update_b" type="s" >}
...
{<section id="axcq510.key_delete_b" type="s" >}
...
{<section id="axcq510.lock_b" type="s" >}
#+ 連動lock其他單身table資料
...
{<section id="axcq510.unlock_b" type="s" >}
#+ 連動unlock其他單身table資料
...
{<section id="axcq510.set_entry" type="s" >}
#+ 單頭欄位開啟設定
...
{<section id="axcq510.set_no_entry" type="s" >}
#+ 單頭欄位關閉設定
...
{<section id="axcq510.set_entry_b" type="s" >}
#+ 單身欄位開啟設定
...
{<section id="axcq510.set_no_entry_b" type="s" >}
#+ 單身欄位關閉設定
...
{<section id="axcq510.set_act_visible" type="s" >}
#+ 單頭權限開啟
...
{<section id="axcq510.set_act_no_visible" type="s" >}
#+ 單頭權限關閉
...
{<section id="axcq510.set_act_visible_b" type="s" >}
#+ 單身權限開啟
...
{<section id="axcq510.set_act_no_visible_b" type="s" >}
#+ 單身權限關閉
...
{<section id="axcq510.default_search" type="s" >}
#+ 外部參數搜尋
...
{<section id="axcq510.fill_chk" type="s" >}
#+ 單身填充確認
...
{<section id="axcq510.modify_detail_chk" type="s" >}
#+ 單身輸入判定
...
{<section id="axcq510.mask_functions" type="s" >}
&include "erp/axc/axcq510_mask.4gl"
 
{</section>}
 
{<section id="axcq510.state_change" type="s" >}
    
 
{</section>}
 
{<section id="axcq510.set_pk_array" type="s" >}
...
{<section id="axcq510.msgcentre_notify" type="s" >}
...
{<section id="axcq510.show_ownid_msg" type="s" >}
##+ 判斷是否顯示只能修改自己權限資料的訊息
PRIVATE FUNCTION axcq510_show_ownid_msg()
   #add-point:show_ownid_msg段define(客製用) name="show_ownid_msg.define_customerization"
   
   #end add-point
   #add-point:show_ownid_msg段define(請盡量不要在客製環境修改此段落內容, 否則將後續patch的調整需人工處理) na
...
{<section id="axcq510.set_all_pk_array" type="s" >}
#+ 因為是假雙檔，所以需將單頭與第一單身的key組合起來，才是該筆資料完整的key欄位
PRIVATE FUNCTION axcq510_set_all_pk_array()
   #add-point:set_all_pk_array段define name="set_all_pk_array.define_customerization"
   
   #end add-point
   DEFINE ls_length    LIKE type_t.num5   #(ver:50)
   #add-point:set_all_pk_array段define(請盡量不要在客製環境修改此段落內容, 否則將後續patch的調整需人工處理)
   
   #end add-point
 
   #add-point:Function前置處理 name="set_all_pk_array.before"
   ...
   LET g_pk_array[1].values = g_xcck_m.xcckld
   LET g_pk_array[1].column = 'xcckld'
   LET g_pk_array[02].values = g_xcck_m.xcck001
   LET g_pk_array[02].column = 'xcck001'
   LET g_pk_array[03].values = g_xcck_m.xcck003
   LET g_pk_array[03].column = 'xcck003'
   LET g_pk_array[04].values = g_xcck_m.xcck004
   LET g_pk_array[04].column = 'xcck004'
   LET g_pk_array[05].values = g_xcck_m.xcck005
   LET g_pk_array[05].column = 'xcck005'
 
 
   #設定單身pk
   LET ls_length = g_pk_array.getLength()+1
   LET g_pk_array[ls_length].values = g_xcck_d[g_detail_idx].xcck002
   LET g_pk_array[ls_length].column = 'xcck002'
   LET ls_length = g_pk_array.getLength()+1
   LET g_pk_array[ls_length].values = g_xcck_d[g_detail_idx].xcck006
   LET g_pk_array[ls_length].column = 'xcck006'
   LET ls_length = g_pk_array.getLength()+1
   LET g_pk_array[ls_length].values = g_xcck_d[g_detail_idx].xcck007
   LET g_pk_array[ls_length].column = 'xcck007'
   LET ls_length = g_pk_array.getLength()+1
   LET g_pk_array[ls_length].values = g_xcck_d[g_detail_idx].xcck008
   LET g_pk_array[ls_length].column = 'xcck008'
   LET ls_length = g_pk_array.getLength()+1
   LET g_pk_array[ls_length].values = g_xcck_d[g_detail_idx].xcck009
   LET g_pk_array[ls_length].column = 'xcck009'
 
 
   #add-point:set_all_pk_array段之後 name="set_all_pk_array.after"
...
{<section id="axcq510.other_function" readonly="Y" type="s" >}
...
DROP TABLE axcq510_tmp; #ODI-ORA#
...
#add--2015/09/30 By shiun--(S)
  #將畫面資料擷取傳至temptable
   FOR l_cnt1 = 1 TO g_rec_b
      ...
      LET sr.inba008 = g_xcck_d[l_cnt1].inba008   #170704-00066#1 add
...
PRIVATE FUNCTION axcq510_b_fill_1()
   ...
   #LET l_xcbfl003_sql = "(SELECT xcbfl003 FROM xcbfl_t WHERE xcbflent='",g_enterprise,"' AND xcbfl001=xcck002 AND xcbfl002='",g_dlang,"')" #170310-00068#12 mark
   LET l_xcbfl003_sql = "(SELECT xcbfl003 FROM xcbfl_t WHERE xcbflent='",g_enterprise,"' AND xcbfl001=xcck002 AND xcbflcomp=xcckcomp AND xcbfl002='",g_dlang,"')" #170310-00068#12 add
   LET l_ooefl003_sql = s_fin_get_department_sql('a','xcckent','xcck025')
   LET l_imaal003_sql = "(SELECT imaal003 FROM imaal_t WHERE imaalent=xcckent AND imaal001=xcck010 AND imaal002='",g_dlang,"')"
   LET l_imaal004_sql = "(SELECT imaal004 FROM imaal_t WHERE imaalent=xcckent AND imaal001=xcck010 AND imaal002='",g_dlang,"')"
   LET l_oocal003_sql = "(SELECT oocal003 FROM oocal_t WHERE oocalent=xcckent AND oocal001=xcck044 AND oocal002='",g_dlang,"')"
   ...
   #LET l_oocql004_sql2= s_fin_get_acc_sql('c','xcckent',g_acc,'xcck021')  #170915-00014#1 mark
   LET l_oocql004_sql2= "(SELECT oocql004 FROM oocql_t WHERE oocqlent='",g_enterprise,"' AND oocql002=xcck021 AND oocql003='",g_dlang,"' and oocql001 in (SELECT distinct gzcb004   FROM gzcb_t WHERE gzcb001 = '24' AND gzcb002 = xcck012))"  #170915-00014#1   
   ...
   #170310-00068#12-s add
   #加入二次篩選  
   IF NOT cl_null(g_wc_filter) THEN
      LET g_sql = g_sql CLIPPED," AND ",g_wc_filter 
   END IF
   #170310-00068#12-e add
...
#      IF g_action_choice <> 'fetch' OR cl_null(g_action_choice) THEN #160202-00016#-mark 寫這個，換頁時，合計會跑不出來
         ...
         LET g_sql = g_sql, " ORDER BY xcck_t.xcck002,xcck_t.xcck006,xcck_t.xcck007,xcck_t.xcck008,xcck_t.xcck009" #ODI-ORA#

         LET g_sql_tmp = l_sql, " AND xcck004=? AND xcck005=? ORDER BY xcck_t.xcck002,xcck_t.xcck025,xcck_t.xcck021,xcck_t.xcck006,xcck_t.xcck007,xcck_t.xcck008,xcck_t.xcck009" #add--2015/09/30 By shiun #ODI-ORA#
...
#               CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,g_xcck_d[l_ac].xcck202,'5') RETURNING g_xcck_d[l_ac].xcck202 
#               CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,g_xcck_d[l_ac].xcck202a,'5') RETURNING g_xcck_d[l_ac].xcck202a
#               CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,g_xcck_d[l_ac].xcck202b,'5') RETURNING g_xcck_d[l_ac].xcck202b
#               CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,g_xcck_d[l_ac].xcck202c,'5') RETURNING g_xcck_d[l_ac].xcck202c 
#               CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,g_xcck_d[l_ac].xcck202d,'5') RETURNING g_xcck_d[l_ac].xcck202d
#               CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,g_xcck_d[l_ac].xcck202e,'5') RETURNING g_xcck_d[l_ac].xcck202e
#               CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,g_xcck_d[l_ac].xcck202f,'5') RETURNING g_xcck_d[l_ac].xcck202f 
#               CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,g_xcck_d[l_ac].xcck202g,'5') RETURNING g_xcck_d[l_ac].xcck202g
#               CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,g_xcck_d[l_ac].xcck202h,'5') RETURNING g_xcck_d[l_ac].xcck202h
              #170427-00041#2 add(E) #171201-00043#2 mark-E
               LET l_ac = l_ac + 1
               LET l_xcck202_sum = g_xcck_d[l_ac].xcck202
               #add--2015/09/30 By shiun--(S)
  ...
  #CALL cl_get_para(g_enterprise,g_xcck_m.xcckcomp,'S-FIN-6001') RETURNING g_para_data   #采用成本域否   #160414-00018#45 mark
   ...
   #170427-00041#2 add(S) #171201-00043#2 mark-S
#   IF cl_null(l_xcck202_sum) THEN LET l_xcck202_sum=0 END IF
#   IF cl_null(l_xcck202a_sum) THEN LET l_xcck202a_sum=0 END IF
#   IF cl_null(l_xcck202b_sum) THEN LET l_xcck202b_sum=0 END IF
#   IF cl_null(l_xcck202c_sum) THEN LET l_xcck202c_sum=0 END IF
#   IF cl_null(l_xcck202d_sum) THEN LET l_xcck202d_sum=0 END IF
#   IF cl_null(l_xcck202e_sum) THEN LET l_xcck202e_sum=0 END IF
#   IF cl_null(l_xcck202f_sum) THEN LET l_xcck202f_sum=0 END IF
#   IF cl_null(l_xcck202g_sum) THEN LET l_xcck202g_sum=0 END IF
#   IF cl_null(l_xcck202h_sum) THEN LET l_xcck202h_sum=0 END IF
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202_sum,'5') RETURNING l_xcck202_sum 
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202a_sum,'5') RETURNING l_xcck202a_sum
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202b_sum,'5') RETURNING l_xcck202b_sum
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202c_sum,'5') RETURNING l_xcck202c_sum 
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202d_sum,'5') RETURNING l_xcck202d_sum
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202e_sum,'5') RETURNING l_xcck202e_sum
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202f_sum,'5') RETURNING l_xcck202f_sum 
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202g_sum,'5') RETURNING l_xcck202g_sum
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202h_sum,'5') RETURNING l_xcck202h_sum
   #170427-00041#2 add(E) #171201-00043#2 mark-E
  ...
  #CALL cl_get_para(g_enterprise,g_xcck_m.xcckcomp,'S-FIN-6001') RETURNING g_para_data   #采用成本域否   #160414-00018#45 mark
   ...
   #170427-00041#2 add(S) #171201-00043#2 mark-S
#   IF cl_null(l_xcck202_total) THEN LET l_xcck202_total=0 END IF
#   IF cl_null(l_xcck202a_total) THEN LET l_xcck202a_total=0 END IF
#   IF cl_null(l_xcck202b_total) THEN LET l_xcck202b_total=0 END IF
#   IF cl_null(l_xcck202c_total) THEN LET l_xcck202c_total=0 END IF
#   IF cl_null(l_xcck202d_total) THEN LET l_xcck202d_total=0 END IF
#   IF cl_null(l_xcck202e_total) THEN LET l_xcck202e_total=0 END IF
#   IF cl_null(l_xcck202f_total) THEN LET l_xcck202f_total=0 END IF
#   IF cl_null(l_xcck202g_total) THEN LET l_xcck202g_total=0 END IF
#   IF cl_null(l_xcck202h_total) THEN LET l_xcck202h_total=0 END IF
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202_total,'5') RETURNING l_xcck202_total
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202a_total,'5') RETURNING l_xcck202a_total
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202b_total,'5') RETURNING l_xcck202b_total
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202c_total,'5') RETURNING l_xcck202c_total 
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202d_total,'5') RETURNING l_xcck202d_total
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202e_total,'5') RETURNING l_xcck202e_total
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202f_total,'5') RETURNING l_xcck202f_total 
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202g_total,'5') RETURNING l_xcck202g_total
#   CALL s_curr_round(g_xcck_m.xcckcomp,g_glaa001,l_xcck202h_total,'5') RETURNING l_xcck202h_total
   #170427-00041#2 add(E) #171201-00043#2 mark-E
...
PRIVATE FUNCTION axcq510_filter()
   ...
   DEFINE ls_result   STRING  #201109-00009#128 add
...
#saki       CALL cl_qbe_init()                       
   ...
   CALL axcq510_browser_fill("") #170310-00068#12 add
   #170310-00068#12 mark -- (s)
   #CALL axcq510_b_fill_1() 
   #CALL axcq510_show() 
   #170310-00068#12 mark -- (e)
   #170310-00068#12 add  -- (s)
   #170310-00068#12 add-s 
   IF g_detail_cnt = 0 THEN
   INITIALIZE g_errparam TO NULL
