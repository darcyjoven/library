...
#該程式非freestyle程式!
{<section id="anmp810.description" type="s" >}
#應用 a00 樣板自動產生(Version:3)
#+ Standard Version.....: SD版次:0026(2017-09-15 15:48:59), PR版次:0026(2019-04-30 18:07:52)
#+ Customerized Version.: SD版次:0000(1900-01-01 00:00:00), PR版次:0000(1900-01-01 00:00:00)
#+ Build......: 000382
#+ Filename...: anmp810
#+ Description: 銀行自動調匯作業
#+ Creator....: 02114(2014-08-13 00:00:00)
#+ Modifier...: 04152 -SD/PR- 05634
 
{</section>}
 
{<section id="anmp810.global" type="s" >}
#應用 p02 樣板自動產生(Version:24)
...
#                                        期初回轉，金額反向處理
#160711-00040#20  2017/02/23 By 09042    单别开窗，加上控制组条件
#170614-00021#1   2017/06/14 By 02114    原币为零本币不为零部分,调汇差把本币调成0  如原币0  本币100  调汇差-100
#170914-00036#1   2017/09/15 By 04152    "重評價憑證號碼"欄位隱藏
#180103-00022#1   2018/01/03 By 05016     原幣及本幣 二個都0 時才不調匯  (因為可能本幣為負數)
#180111-00043#21  2018/02/27 By 09232    设计器产生方法时说明默认给值是CALL s_aooi150_ins (传入参数)，本次批量调整将方法明改为此方法的实际用途说明。
#180402-00026#1   2018/04/09 By 10043    1.執行完 anmp810 再 call anmp830 重計銀行存款月結 2. anmp810 --> anmt820  沒有產生分錄底稿
#180319-00019#1   2018/04/23 By 09232    如果agli174有設定時，符合重評條件的範圍為有設定的帳款科目，
#                                           若重評科目= 1.等同帳款科目 則同目前做法
#                                           若重評科目  <> 帳款科目時 不寫 nmbc , 請 依設定"重評價帳款科目" 寫入nmde015    
#                                        否则 依原程式處理邏輯
#180724-00036#1   2018/07/27 By 05488    產生完anmt820後要產生現金變動碼到glbc_t，不然產生分錄底稿時會抓不到現金變動碼
#180925-00014#1   2018/09/26 By 05634    PREPARE nmde_pre取消排序,讓glbc_t寫入順序同nmde_t的順序
#190219-00008#1   2019/02/20 By 07804    修正 nmbc014應給予cl_get_today()取得日期
#190222-00023#1   2019/02/22 By 07804    修正190219-00008#1,nmbc014 給予nmbc005銀行日:期末最後一天
#190328-00033#1   2019/03/28 By 05795    在没有产生nmde_t得时候就不需要调用产生凭证得方法，因为namdeld没有值调用会报错
#180507-00035#3   2019/04/25 By 10043    取法人層級的S-FIN-0003(現金流量表料來源),若為2:總帳科目設定現金變動碼,現金變動碼為空時不寫入glbc
#190423-00027#1   2019/04/30 By 05634    還原180925-00014#1邏輯
#end add-point
...
{<section id="anmp810.main" type="s" >}
#+ 作業開始 
MAIN
...
{<section id="anmp810.init" type="s" >}
#+ 畫面資料初始化
...
{<section id="anmp810.ui_dialog" type="s" >}
#+ 選單功能實際執行處
...
{<section id="anmp810.query" type="s" >}
#+ QBE資料查詢
...
{<section id="anmp810.b_fill" type="s" >}
#+ 單身陣列填充
...
{<section id="anmp810.fetch" type="s" >}
#+ 單身陣列填充2
...
{<section id="anmp810.detail_show" type="s" >}
#+ 顯示相關資料
...
{<section id="anmp810.filter" type="s" >}
#+ filter過濾功能
...
{<section id="anmp810.filter_parser" type="s" >}
#+ filter欄位解析
...
{<section id="anmp810.filter_show" type="s" >}
#+ Browser標題欄位顯示搜尋條件
...
{<section id="anmp810.other_function" readonly="Y" type="s" >}
...
PRIVATE FUNCTION anmp810_nm()
  ...
  #161128-00061#2----modify------begin-----------
   ...
   DEFINE l_success  LIKE type_t.num5     
   DEFINE l_nmdedocno LIKE nmde_t.nmdedocno
   #180319-00019#1 add-s
   DEFINE l_cnt1              LIKE type_t.num5
   DEFINE l_glcj003           LIKE glcj_t.glcj003
   DEFINE l_glcj005           LIKE glcj_t.glcj005
   #180319-00019#1 add-e


   LET g_sql = "SELECT * FROM anmp810_tmp ",
               " WHERE sel = 'Y' "
   PREPARE anmp810_del_p FROM g_sql
   DECLARE anmp810_del_c CURSOR FOR anmp810_del_p
   FOREACH anmp810_del_c INTO l_tmp.*
      ...
      LET lc_param.wc = ' 1=1'
      LET lc_param.nmbxcomp = l_tmp.glaacomp
      LET lc_param.nmbx001 = g_input.yy
      LET lc_param.nmbx002 = g_input.mm
      LET la_param.param[1] = util.JSON.stringify( lc_param )
      LET ls_js = util.JSON.stringify(la_param)
      CALL cl_cmdrun_wait(ls_js)
   END FOREACH
   #180402-00026#1 add --(e)
   ...
   FOR l_i = 1 TO g_detail_d.getLength()
      IF g_detail_d[l_i].sel = "Y" THEN  
         SELECT DISTINCT nmdedocno INTO l_nmdedocno
           FROM nmde_t
          WHERE nmdeent = g_enterprise
            AND nmdeld  = g_detail_d[l_i].glaald
            AND nmde001 = g_input.yy
            AND nmde002 = g_input.mm
            AND nmde003 = 'NM'
                  
         IF NOT cl_null(l_nmdedocno) THEN
            CALL s_pre_voucher_del('NM','N40',g_detail_d[l_i].glaald,l_nmdedocno) RETURNING l_success
            IF l_success = FALSE THEN
               INITIALIZE g_errparam TO NULL
               LET g_errparam.code = SQLCA.SQLCODE
               LET g_errparam.extend = "del glga"
               LET g_errparam.popup = TRUE
               CALL cl_err()
               LET g_success = 'N'       
            END IF
   ...
   #160513-00008#2--add--end
   #LET l_nmbc.nmbc014 = cl_get_today()   #190219-00008#1 add   #190222-00023#1 mark
...
#      #albireo 151002 mark-----e
      ...
      #180319-00019#1 add-s
      LET l_cnt1=0
      SELECT count(1) INTO l_cnt1 FROM glcj_t 
       WHERE glcjent=g_enterprise AND glcjld=l_tmp.glaald AND glcj001='NM'
      #180319-00019#1 add-e
      ...
      FOREACH sel_nmasc1 INTO l_nmas002,l_nmas003
         ...
         #無重評金額
         #IF l_amt1 <= 0 AND l_amt2<=0 THEN   #170614-00021#1 add AND l_amt2<=0 lujh #180103-00022#1 mark
          IF l_amt1 = 0 AND l_amt2 =0 THEN    #180103-00022#1 add         
            CONTINUE FOREACH
         END IF
         ...
         IF l_cnt1>0 THEN   
            LET l_glcj003=''
            #取agli174参数
            SELECT glcj003,glcj005 INTO l_glcj003,l_glcj005 FROM glcj_t 
             WHERE glcjent=g_enterprise AND glcjld=l_tmp.glaald AND glcj001='NM' AND glcj002='1' AND glcj004=l_glab005
            IF NOT cl_null(l_glcj003) THEN
               IF l_glcj003='2' THEN
                  LET l_glab005=l_glcj005 
               END IF
            ELSE 
                CONTINUE FOREACH   #若l_glab005不在agli174需做重评价科目的范围内，则不处理
            END IF
         END IF 
         #180319-00019#1 add-e
         ...
         IF l_tmp.glca002 = '3' THEN   #3:重評價傳票，次月不迴轉。
            ...
            IF (l_cnt1>0 AND l_glcj003='1') OR l_cnt1=0 THEN  #180319-00019#1 add
               LET l_nmde.nmde106 = l_nmde.nmde105 + l_nmde105  #161027-00004#1 add
            END IF  #180319-00019#1 add
         END IF
         IF cl_null(l_nmde.nmde106) THEN LET l_nmde.nmde106 = 0 END IF #150825
         ...
         END IF
         IF cl_null(l_nmde.nmde114) THEN LET l_nmde.nmde114 = 0 END IF  #150825
         #CALL s_curr_round_ld('1',l_tmp.glaald,g_glaa016,l_nmde.nmde114,2) RETURNING g_sub_success,g_errno,l_nmde.nmde114  #150825 #180402-00026#1 mark
         #180402-00026#1 add --(S)
         IF g_glaa015 = 'Y' THEN
         ...
         END IF
         IF cl_null(l_nmde.nmde124) THEN LET l_nmde.nmde124 = 0 END IF  #150825
         #CALL s_curr_round_ld('1',l_tmp.glaald,g_glaa020,l_nmde.nmde124,2) RETURNING g_sub_success,g_errno,l_nmde.nmde124  #180402-00026#1 mark
         #180402-00026#1 add --(S)
         IF g_glaa019 = 'Y' THEN
            CALL s_curr_round_ld('1',l_tmp.glaald,g_glaa020,l_nmde.nmde124,2) RETURNING g_sub_success,g_errno,l_nmde.nmde124
         END IF
         ...
         CALL s_curr_round_ld('1',l_tmp.glaald,g_glaa001,l_nmde.nmde104,2) RETURNING g_sub_success,g_errno,l_nmde.nmde104 
         #CALL s_curr_round_ld('1',l_tmp.glaald,g_glaa016,l_nmde.nmde114,2) RETURNING g_sub_success,g_errno,l_nmde.nmde114   #180402-00026#1 mark
         #CALL s_curr_round_ld('1',l_tmp.glaald,g_glaa020,l_nmde.nmde124,2) RETURNING g_sub_success,g_errno,l_nmde.nmde124   #180402-00026#1 mark
         
         CALL s_curr_round_ld('1',l_tmp.glaald,g_glaa001,l_nmde.nmde105,2) RETURNING g_sub_success,g_errno,l_nmde.nmde105 
         #CALL s_curr_round_ld('1',l_tmp.glaald,g_glaa016,l_nmde.nmde115,2) RETURNING g_sub_success,g_errno,l_nmde.nmde115   #180402-00026#1 mark
         #CALL s_curr_round_ld('1',l_tmp.glaald,g_glaa020,l_nmde.nmde125,2) RETURNING g_sub_success,g_errno,l_nmde.nmde125   #180402-00026#1 mark
         
         CALL s_curr_round_ld('1',l_tmp.glaald,g_glaa001,l_nmde.nmde106,2) RETURNING g_sub_success,g_errno,l_nmde.nmde106 
         #CALL s_curr_round_ld('1',l_tmp.glaald,g_glaa016,l_nmde.nmde116,2) RETURNING g_sub_success,g_errno,l_nmde.nmde116   #180402-00026#1 mark
         #CALL s_curr_round_ld('1',l_tmp.glaald,g_glaa020,l_nmde.nmde126,2) RETURNING g_sub_success,g_errno,l_nmde.nmde126   #180402-00026#1 mark
   ...
   #            LET l_nmbc.nmbc133=l_nmde.nmde126   #本期本位幣三匯差傳票提列提列金額 #160531-00014#1 #161013-00052#1 mark
      ...
      IF SQLCA.sqlcode THEN
         ...
         LET g_success = 'N'       
      END IF
      IF NOT cl_null(l_nmde.nmdeld) AND NOT cl_null(l_nmde.nmdedocno) THEN  #190328-00033#1---add
         #180724-00036#1---add---start---
         CALL anmp810_ins_glbc(l_nmde.nmdeld,l_nmde.nmdedocno,l_nmde.nmdecomp,l_nmde.nmde001,l_nmde.nmde002) 
            RETURNING l_success                    
         IF l_success THEN
            LET g_success = 'Y'
         ELSE
         ...
         LET l_success = TRUE
         LET g_prog = 'anmt820'
         CALL s_pre_voucher_ins('NM','N40',l_nmde.nmdeld,l_nmde.nmdedocno,l_nmde.nmdedocdt,'1')  
                     RETURNING l_success
         LET g_prog = 'anmp810'               
         IF l_success THEN
            LET g_success = 'Y'
   ...
   #END IF     
   ##180402-00026#1 add --(E)
   #180724-00036#1---mark---end---   
   
   CALL cl_err_collect_show()
   IF g_success = 'Y' THEN   
      CALL s_transaction_end('Y',1)
      #180402-00026#1 add --(S) 
      #背景直接呼叫 anmp830 重計銀行存款月結      
      FOR l_i = 1 TO g_detail_d.getLength()
         IF g_detail_d[l_i].sel = "Y" THEN
            INITIALIZE la_param.* TO NULL
            LET la_param.prog = "anmp830"
            ...
            INITIALIZE lc_param.* TO NULL
            LET lc_param.wc = ' 1=1'
            LET lc_param.nmbxcomp = g_detail_d[l_i].glaacomp
            LET lc_param.nmbx001 = g_detail_d[l_i].b_yy
            LET lc_param.nmbx002 = g_detail_d[l_i].b_mm
            LET la_param.param[1] = util.JSON.stringify( lc_param )
            LET ls_js = util.JSON.stringify(la_param)
            CALL cl_cmdrun_wait(ls_js) 
         END IF
...
# Descriptions...: 组合查询条件
# Memo...........:
# Usage..........: anmp810_get_ooef001_wc(p_wc)
#                  RETURNING 回传参数
# Input parameter: p_wc   帳務組織底下所屬的法人範圍
# Return code....: r_wc   查询条件
#                : 回传参数变量2   回传参数变量说明2
# Date & Author..: 日期 By 作者
# Modify.........: #180111-00043#21
################################################################################
...
################################################################################
# Descriptions...: 产生凭证前对资料检核
# Memo...........:
# Usage..........: anmp810__immediately_gen(p_nmdeld,p_nmde001,p_nmde002,p_nmdecomp,p_nmdedocno)
#                  RETURNING 回传参数
# Input parameter: p_nmdeld       账套
#                : p_nmde001      年度
#                : p_nmde002      期别
#                : p_nmdecomp     法人
#                : p_nmdedocno    单号
...
# Descriptions...: 产生凭证
# Memo...........:
# Usage..........: anmp810_for_gen(p_nmdeld,p_nmde001,p_nmde002)
#                  RETURNING 回传参数
# Input parameter: p_nmdeld       账套
#                : p_nmde001      年度
#                : p_nmde002      期别
# Return code....: l_success      布尔值
# Date & Author..: 日期 By 作者
# Modify.........: #180111-00043#21 
################################################################################
...
# Descriptions...: 产生分录底稿
# Memo...........:
# Usage..........: anmp810_go_gen(p_nmdeld,p_nmde001,p_nmde002,p_nmdedocno)
#                  RETURNING 回传参数
# Input parameter: p_nmdeld       账套
#                  p_nmde001      年度
#                  p_nmde002      期别
...
PRIVATE FUNCTION anmp810_go_gen(p_nmdeld,p_nmde001,p_nmde002,p_nmdedocno)
   ...
   DEFINE p_nmdecomp        LIKE nmde_t.nmdecomp
   DEFINE p_nmdedocno       LIKE nmde_t.nmdedocno
   DEFINE l_sql1            STRING     #160711-00040#20
      ...
      DIALOG ATTRIBUTES(UNBUFFERED,FIELD ORDER FORM)
         ...
         INPUT BY NAME sr.glapdocno,sr.glapdocdt ATTRIBUTE(WITHOUT DEFAULTS)
            ...
            ON ACTION controlp INFIELD glapdocno
               ...
               END IF
               #160711-00040#20  2017/02/23  By 09042  add(S)
               ...
               END IF
               #160711-00040#20  2017/02/23  By 09042  add(E)
               CALL q_ooba002_1()
               #151013-00016#4 add end---
...
# Descriptions...: 产生分录底稿
# Memo...........:
# Usage..........: anmp810_go_gen2(p_lip,p_nmdeld,p_nmde001,p_nmde002,p_nmdedocno)
#                  RETURNING 回传参数
# Input parameter: p_lip         单别
#                : p_nmdeld      账套
#                : p_nmde001     年度
#                : p_nmde002     期别
#                : p_nmdedocno   单号
...
# Descriptions...: 描述说明
# Memo...........:
# Usage..........: CALL anmp810_ins_glbc(p_ld,p_docno,p_comp,p_year,p_month)
#                     RETURNING r_success
...
# Date & Author..: #180724-00036#1     2018/07/27   By 05488
# Modify.........:
################################################################################
PRIVATE FUNCTION anmp810_ins_glbc(p_ld,p_docno,p_comp,p_year,p_month)
DEFINE p_ld      LIKE nmde_t.nmdeld
DEFINE p_docno   LIKE nmde_t.nmdedocno
DEFINE p_comp    LIKE nmde_t.nmdecomp
DEFINE p_year    LIKE nmde_t.nmde001
DEFINE p_month   LIKE nmde_t.nmde002
DEFINE r_success LIKE type_t.num5
...
DEFINE l_nmde RECORD  #銀行重評價檔
       ...
       nmdedocno LIKE nmde_t.nmdedocno, #單據編號
       nmdedocdt LIKE nmde_t.nmdedocdt, #單據日期
       nmdeud001 LIKE nmde_t.nmdeud001, #自定義欄位(文字)001
       nmdeud002 LIKE nmde_t.nmdeud002, #自定義欄位(文字)002
       nmdeud003 LIKE nmde_t.nmdeud003, #自定義欄位(文字)003
       nmdeud004 LIKE nmde_t.nmdeud004, #自定義欄位(文字)004
       nmdeud005 LIKE nmde_t.nmdeud005, #自定義欄位(文字)005
       nmdeud006 LIKE nmde_t.nmdeud006, #自定義欄位(文字)006
       nmdeud007 LIKE nmde_t.nmdeud007, #自定義欄位(文字)007
       nmdeud008 LIKE nmde_t.nmdeud008, #自定義欄位(文字)008
       nmdeud009 LIKE nmde_t.nmdeud009, #自定義欄位(文字)009
       nmdeud010 LIKE nmde_t.nmdeud010, #自定義欄位(文字)010
       nmdeud011 LIKE nmde_t.nmdeud011, #自定義欄位(數字)011
       nmdeud012 LIKE nmde_t.nmdeud012, #自定義欄位(數字)012
       nmdeud013 LIKE nmde_t.nmdeud013, #自定義欄位(數字)013
       nmdeud014 LIKE nmde_t.nmdeud014, #自定義欄位(數字)014
       nmdeud015 LIKE nmde_t.nmdeud015, #自定義欄位(數字)015
       nmdeud016 LIKE nmde_t.nmdeud016, #自定義欄位(數字)016
       nmdeud017 LIKE nmde_t.nmdeud017, #自定義欄位(數字)017
       nmdeud018 LIKE nmde_t.nmdeud018, #自定義欄位(數字)018
       nmdeud019 LIKE nmde_t.nmdeud019, #自定義欄位(數字)019
       nmdeud020 LIKE nmde_t.nmdeud020, #自定義欄位(數字)020
       nmdeud021 LIKE nmde_t.nmdeud021, #自定義欄位(日期時間)021
       nmdeud022 LIKE nmde_t.nmdeud022, #自定義欄位(日期時間)022
       nmdeud023 LIKE nmde_t.nmdeud023, #自定義欄位(日期時間)023
       nmdeud024 LIKE nmde_t.nmdeud024, #自定義欄位(日期時間)024
       nmdeud025 LIKE nmde_t.nmdeud025, #自定義欄位(日期時間)025
       nmdeud026 LIKE nmde_t.nmdeud026, #自定義欄位(日期時間)026
       nmdeud027 LIKE nmde_t.nmdeud027, #自定義欄位(日期時間)027
       nmdeud028 LIKE nmde_t.nmdeud028, #自定義欄位(日期時間)028
       nmdeud029 LIKE nmde_t.nmdeud029, #自定義欄位(日期時間)029
       nmdeud030 LIKE nmde_t.nmdeud030, #自定義欄位(日期時間)030
       nmde018   LIKE nmde_t.nmde018, #收付款客商
       nmde019   LIKE nmde_t.nmde019, #帳款客商
       nmde020   LIKE nmde_t.nmde020, #經營方式
       nmde021   LIKE nmde_t.nmde021, #通路
...
DEFINE l_sfin0003     LIKE type_t.chr1  #180507-00035#3 add    
   
   LET r_success = TRUE
   INITIALIZE l_glbc.* TO NULL
   INITIALIZE l_nmde.* TO NULL
   
   DELETE FROM glbc_t
    WHERE glbcent = g_enterprise
      AND glbcld = p_ld    AND glbcdocno = p_docno
      AND glbc001 = p_year AND glbc002 = p_month
   
   #180507-00035#3 add ---s
   LET l_sfin0003 = ''
   CALL cl_get_para(g_enterprise,p_comp,'S-FIN-0003') RETURNING l_sfin0003 
   IF cl_null(l_sfin0003) THEN LET l_sfin0003 = 1 END IF 
   #180507-00035#3 add ---e

   LET l_sql = " SELECT nmdeent,nmdecomp,nmdeld,nmdesite,nmde001,nmde002,nmde003,nmde004,nmde005,nmde006,",
               "nmde007,nmde008,nmde009,nmde010,nmde011,nmde012,nmde013,nmde014,nmde015,nmde017,nmde100,",
               "nmde101,nmde102,nmde103,nmde104,nmde105,nmde106,nmde111,nmde113,nmde114,nmde115,nmde116,",
               "nmde121,nmde123,nmde124,nmde125,nmde126,nmdedocno,nmdedocdt,nmdeud001,nmdeud002,nmdeud003,",
               "nmdeud004,nmdeud005,nmdeud006,nmdeud007,nmdeud008,nmdeud009,nmdeud010,nmdeud011,nmdeud012,",
               "nmdeud013,nmdeud014,nmdeud015,nmdeud016,nmdeud017,nmdeud018,nmdeud019,nmdeud020,nmdeud021,",
               "nmdeud022,nmdeud023,nmdeud024,nmdeud025,nmdeud026,nmdeud027,nmdeud028,nmdeud029,nmdeud030,",
               "nmde018,nmde019,nmde020,nmde021,nmde022,nmde023,nmde024,nmde025,nmde026,nmde027,nmde028,",
               "nmde029,nmde030,nmde031,nmde032,nmde033 FROM nmde_t ",
               "  WHERE nmdeent = '",g_enterprise,"'",
               "    AND nmdeld = '",p_ld,"'",
               "    AND nmdedocno = '",p_docno,"'",  #180925-00014#1 mod 移除最後的逗號  #190423-00027#1  mod 補上最後的逗號
               "  ORDER BY nmde004 "   #180925-00014#1 mark   #190423-00027#1 remark  
               
   PREPARE nmde_pre FROM l_sql
   DECLARE nmde_cs CURSOR FOR nmde_pre
    
   FOREACH nmde_cs INTO l_nmde.*
      LET l_glbc.glbcent   = g_enterprise
      LET l_glbc.glbcld    = p_ld
      LET l_glbc.glbccomp  = p_comp
      LET l_glbc.glbcdocno = p_docno
      SELECT MAX(glbcseq) + 1 INTO l_glbc.glbcseq
        FROM glbc_t
       WHERE glbcent = g_enterprise
         ...
         AND glbc001 = p_year
         AND glbc002 = p_month 
      IF cl_null(l_glbc.glbcseq) THEN 
         LET l_glbc.glbcseq = 1
      END IF
      ...
      END IF
      LET l_glbc.glbc001   = p_year
      LET l_glbc.glbc002   = p_month
      LET l_glbc.glbc003   = '1'
      #現金變動碼
      SELECT glaa005 INTO l_glaa005 FROM glaa_t WHERE glaaent = g_enterprise AND glaald = p_ld AND glaa014 = 'Y'  
      
      SELECT nmak001 INTO l_nmak001 FROM nmak_t WHERE nmakent = g_enterprise AND nmakstus = 'Y' AND nmak002 = '4'  

      #抓取agli171預設存提碼
      SELECT glab005 INTO l_glab005 FROM glab_t WHERE glabent = g_enterprise AND glab002 = '8318'  
         AND glab003 = '8318_13' AND glabld = p_ld 
      #抓取對應的現變碼
      SELECT nmad003 INTO l_glbc.glbc004
        FROM nmad_t
       WHERE nmadent = g_enterprise
         AND nmad001 = l_glaa005
         AND nmad002 = l_glab005
      IF cl_null(l_glbc.glbc004) THEN       
         IF l_sfin0003  = '1' THEN #180507-00035#3 add
            LET r_success=FALSE
            INITIALIZE g_errparam TO NULL
            LET g_errparam.code = 'anm-00339'
            LET g_errparam.extend = l_glbc.glbcdocno,"#",l_glbc.glbcseq
            LET g_errparam.popup = TRUE
            CALL cl_err()
         #180507-00035#3 add ---s
         ELSE
      ...
      LET l_glbc.glbc005   = l_nmde.nmde011
      LET l_glbc.glbc006   = l_nmde.nmde100
      LET l_glbc.glbc007   = l_nmde.nmde101
      LET l_glbc.glbc008   = 0
      LET l_glbc.glbc009   = l_nmde.nmde105   
      LET l_glbc.glbc010   ='2' 
      IF g_glaa015 = 'Y' THEN 
         LET l_glbc.glbc011 = l_nmde.nmde111
         LET l_glbc.glbc012 = l_nmde.nmde115  
      ELSE
         LET l_glbc.glbc011 = 0
         LET l_glbc.glbc012 = 0
      END IF
      IF g_glaa019 = 'Y' THEN 
         LET l_glbc.glbc013 = l_nmde.nmde121
         LET l_glbc.glbc014 = l_nmde.nmde125  
      ELSE
         LET l_glbc.glbc013 = 0
         LET l_glbc.glbc014 = 0
      END IF
      
      LET l_glbc.glbc015   = 'Y'     
      INSERT INTO glbc_t (glbcent,glbcld,glbccomp,glbcdocno,glbcseq,glbcseq1,glbc001,glbc002,glbc003,glbc004,
                          glbc005,glbc006,glbc007,glbc008,glbc009,glbc010,glbc011,glbc012,glbc013,glbc014,
                          glbcud001,glbcud002,glbcud003,glbcud004,glbcud005,glbcud006,glbcud007,glbcud008,
                          glbcud009,glbcud010,glbcud011,glbcud012,glbcud013,glbcud014,glbcud015,glbcud016,
                          glbcud017,glbcud018,glbcud019,glbcud020,glbcud021,glbcud022,glbcud023,glbcud024,
       ...
       VALUES(l_glbc.glbcent,l_glbc.glbcld,l_glbc.glbccomp,l_glbc.glbcdocno,l_glbc.glbcseq,l_glbc.glbcseq1,l_glbc.glbc001,l_glbc.glbc002,l_glbc.glbc003,l_glbc.glbc004,
              l_glbc.glbc005,l_glbc.glbc006,l_glbc.glbc007,l_glbc.glbc008,l_glbc.glbc009,l_glbc.glbc010,l_glbc.glbc011,l_glbc.glbc012,l_glbc.glbc013,l_glbc.glbc014,
              l_glbc.glbcud001,l_glbc.glbcud002,l_glbc.glbcud003,l_glbc.glbcud004,l_glbc.glbcud005,l_glbc.glbcud006,l_glbc.glbcud007,l_glbc.glbcud008,
              l_glbc.glbcud009,l_glbc.glbcud010,l_glbc.glbcud011,l_glbc.glbcud012,l_glbc.glbcud013,l_glbc.glbcud014,l_glbc.glbcud015,l_glbc.glbcud016,
              l_glbc.glbcud017,l_glbc.glbcud018,l_glbc.glbcud019,l_glbc.glbcud020,l_glbc.glbcud021,l_glbc.glbcud022,l_glbc.glbcud023,l_glbc.glbcud024,
