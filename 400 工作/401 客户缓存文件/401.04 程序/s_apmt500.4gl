...
#+ Standard Version.....: SD版次:0007(1900-01-01 00:00:00), PR版次:0007(2018-01-02 09:34:35)
#+ Customerized Version.: SD版次:0001(1900-01-01 00:00:00), PR版次:0007(2021-02-25 10:14:10)
#+ Build......: 000510
#+ Filename...: s_apmt500
...
#C20180827-09016#1 2018/08/27 By Wangxt   MES抛委外采购单人员调整不需要抛到采购人员上，调整到委外采购单录单人上，采购单人员抓取供应商默认采购员
#C20181113-09421#1 2018/11/13 By wangll   检查料件在apmi120有无对应料件，如果没有，则报错
#C20181114-09421#1 2018/11/14 By wangll   修改#C20181113-09421#1，改成根据公司料号找供应商料号
#C20181119-09421#2 2018/11/19 By wangll   增加管控，判断供应商料号的时间只有多角性质为2：代采购的时候才判断
#                                         并且公司料号要管控为采购类型
#C20181122-09421#1 2018/11/22 By wangll 将MES回写委外采购单批号的逻辑改到s_apmt500中
#171214-00052#8   2018/01/11  by 09767   新增函数s_apmt500_adjust_amount()
#190711-00045#7   2020/09/15  By hehw    新增apm取匯率共用元件s_apmt500_get_exchange
#200116-00020#8   2020/04/20  By 12133    單據單身是否進行尾差分攤增加判斷，依發票類型的計稅原則做判斷   #add by lifang 210225
#end add-point
#(ver:6) ---start---
...
DEFINE l_count_pmao002 LIKE type_t.num5       #C20181113-09421#1---add
DEFINE l_pmdn027       LIKE pmdn_t.pmdn027    #add by pane 201117
  ...
  #add by pane 201117 begin#
  IF g_prog = 'apmt501' AND l_pmdl006 = '2' THEN
     DECLARE pmdn_pmdn027 CURSOR FOR SELECT pmdnseq,pmdn001,pmdn027 
                                        FROM pmdn_t
                                      WHERE pmdnent = g_enterprise
                                        AND pmdndocno = p_pmdldocno
                                        AND pmdn027 IS NULL
     FOREACH pmdn_pmdn027 INTO l_pmdnseq,l_pmdn001,l_pmdn027
       IF cl_null(l_pmdn027) THEN
          INITIALIZE g_errparam TO NULL
          LET g_errparam.code = 'cpm-00023'
          LET g_errparam.extend = l_pmdn001
          LET g_errparam.popup = TRUE
          CALL cl_err()
          LET r_success = FALSE
       END IF        
  ...
  #add by pane 201117 end#
   ...
   #ming 20150903 modify -----------------------------(E) 
      ...
      #C20181113-09421#1---add---s
      
      #apmt501多角抛转，检查料件在apmi120有无对应料件，如果没有，则报错
      IF g_prog = 'apmt501' AND l_pmdl006 = '2' THEN   #C20181119-09421#2---add l_pmdl006 = '2'
         LET l_count_pmao002 = 0
         SELECT count(*) INTO l_count_pmao002
            FROM pmao_t
            WHERE pmaoent = g_enterprise
         #   AND pmao004 = l_pmdn001  #C20181114-09421#1---mark
         AND pmao002 = l_pmdn001  #C20181114-09421#1---add
         AND pmao000 = '1'        #C20181119-09421#2---add
         IF l_count_pmao002 = 0 THEN
            INITIALIZE g_errparam TO NULL
            LET g_errparam.code = 'cpm-00012'
...
#      END IF
      ...
      IF l_cnt > 0 AND l_pmdn.pmdn019 <> '9' THEN  #171214-00053#1 mod pmdn019不能为9的条件
         UPDATE pmar_t
...
#   DEFINE l_pmab          RECORD LIKE pmab_t.*
   ...
   #C20180827-09016#1---s---
   SELECT pmaa086,pmaa087 INTO l_pmdl.pmdl002,l_pmdl.pmdl003 FROM pmaa_t
    WHERE pmaaent  = g_enterprise
      AND pmaa001  = p_pmdl.pmdl004
   #C20180827-09016#1---e---   
...
DEFINE l_cnt          LIKE type_t.num10
DEFINE l_isac005        LIKE isac_t.isac005  #計稅原則 #200116-00020#9 add
   ...
   IF NOT cl_null(p_pmdldocno) THEN 
      ...
      END IF
   END IF
   #200116-00020#8 add---(S)
   #依發票類型做判斷，計稅原則(isac005)="1:依交易總額重計稅(清單式)"需進行尾差分攤，為"2:依發票細項計稅(明細式)"不需進行尾差分攤
   SELECT isac005 INTO l_isac005
     FROM pmdl_t,ooef_t,isac_t
    WHERE pmdlent = ooefent AND pmdlent = isacent
      AND pmdlsite = ooef001
      AND pmdl033 = isac002 AND ooef019 = isac001 
      AND pmdlent = g_enterprise AND pmdldocno = p_pmdldocno
   IF l_isac005 = 2 THEN
      LET r_success = FALSE
      RETURN r_success
   END IF
   #200116-00020#8 add---(E)
   RETURN r_success
END FUNCTION
...
#                : r_start_no     起始QC单号
#                : r_end_no       截止QC单号
# Date & Author..: 20180607 by cz
################################################################################
PUBLIC FUNCTION s_apmt500_gen_c1(p_key1,p_key2,p_doc_type,p_date,p_combine)
   DEFINE p_key1          LIKE qcba_t.qcba001
   DEFINE p_key2          LIKE qcba_t.qcba002
   DEFINE p_doc_type      LIKE ooba_t.ooba002
   DEFINE p_date          LIKE type_t.dat
   ...
   IF p_combine = 'N' THEN
      LET g_sql = " SELECT UNIQUE sfcbdocno,sfcb013,pmdl011,pmdl015,pmdl017 ",
                  #"   FROM asfp400_tmp_t", #mark by xianggy 180626
                  "   FROM wssp312_tmp_t", #add by xianggy 180626
                  "  ORDER BY sfcbdocno,sfcb013 "
   ELSE
      LET g_sql = " SELECT UNIQUE '',sfcb013,pmdl011,pmdl015,pmdl017 ",
                  #"   FROM asfp400_tmp_t", #mark by xianggy 180626
   ...
   PREPARE s_apmt500_gen_c1_p1 FROM g_sql
   IF SQLCA.sqlcode THEN
      INITIALIZE g_errparam TO NULL
      LET g_errparam.code = SQLCA.sqlcode
      LET g_errparam.extend = 'prepare s_apmt500_gen_4_p1'
   ...
   DECLARE s_apmt500_gen_c1_cs1 CURSOR FOR s_apmt500_gen_c1_p1
   IF SQLCA.sqlcode THEN
      INITIALIZE g_errparam TO NULL
      LET g_errparam.code = SQLCA.sqlcode
      LET g_errparam.extend = 'declare s_apmt500_gen_c1_cs1'
      LET g_errparam.popup = TRUE
      CALL cl_err()
   ...
   FOREACH s_apmt500_gen_c1_cs1 INTO l_pmdl.*
      IF SQLCA.sqlcode THEN
         INITIALIZE g_errparam TO NULL
         LET g_errparam.code = SQLCA.sqlcode
         LET g_errparam.extend = 'foreach s_apmt500_gen_c1_cs1'
         LET g_errparam.popup = TRUE
         CALL cl_err()
      ...
      SELECT COUNT(UNIQUE sfcbdocno) INTO l_n FROM wssp312_tmp_t
       WHERE sfcb013 = l_pmdl.pmdl004 AND pmdl011 = l_pmdl.pmdl011
         AND pmdl015 = l_pmdl.pmdl015 AND pmdl017 = l_pmdl.pmdl017
      IF l_n = 1 THEN
         SELECT sfcbdocno INTO l_pmdl.sfcbdocno FROM wssp312_tmp_t
          WHERE sfcb013 = l_pmdl.pmdl004 AND pmdl011 = l_pmdl.pmdl011
            AND pmdl015 = l_pmdl.pmdl015 AND pmdl017 = l_pmdl.pmdl017
      END IF
      ...
      #插入"pmdn_t:採購明細檔"
      CALL s_apmt500_gen_c1_ins_pmdn(l_pmdldocno,l_pmdl.*)
           RETURNING l_success           
      IF NOT l_success THEN
         RETURN r_success,r_start_no,r_end_no
      END IF
...
################################################################################
# Descriptions...: 从MES产生采购单 -- 产生采购单身pmdn_t
# Memo...........:
# Usage..........: CALL s_apmt500_gen_c1_ins_pmdn(p_pmdndocno,p_pmdl)
#                       RETURNING r_success
# Input parameter: p_pmdndocno    采购单单号
#                : p_pmdl         采购单头信息
# Return code....: r_success      成功否标识符
...
# Modify.........:
################################################################################
PUBLIC FUNCTION s_apmt500_gen_c1_ins_pmdn(p_pmdndocno,p_pmdl)
   DEFINE p_pmdndocno     LIKE pmdn_t.pmdndocno
   DEFINE l_cnt1           LIKE type_t.num10      #161227-00046#1 add   
   DEFINE p_pmdl          RECORD
                          sfcbdocno     LIKE sfcb_t.sfcbdocno,
   ...
   DEFINE r_success       LIKE type_t.num5
   DEFINE l_success       LIKE type_t.num5
   DEFINE l_pmdnud001     LIKE pmdn_t.pmdnud001   #基板供应商
   DEFINE l_pmdnud014     LIKE pmdn_t.pmdnud014   #WEIGHT
   #161124-00048#9 mod-S
#   DEFINE l_pmdn          RECORD LIKE pmdn_t.*
   DEFINE l_pmdn RECORD  #採購單身明細檔
          pmdnent LIKE pmdn_t.pmdnent, #企业编号
   ...
   DEFINE l_sfcb          RECORD
                          ...
                          sfcb045       LIKE sfcb_t.sfcb045,
                          price         LIKE pmdn_t.pmdn015,
                          assignsupplier LIKE pmdn_t.pmdnud001,
                          weight        LIKE pmdn_t.pmdnud014    #add by hehw 200915                      
                          END RECORD
                          
   DEFINE l_sfcb_c        RECORD
                          sfcbdocno     LIKE sfcb_t.sfcbdocno,
                          sfaa010       LIKE sfaa_t.sfaa010,
                          sfac006       LIKE sfac_t.sfac006,
                          sfcb001       LIKE sfcb_t.sfcb001,
...
#   DEFINE l_sfaa          RECORD LIKE sfaa_t.*
   ...
   DEFINE l_sfac          RECORD
                          ...
                          END RECORD
   #160606-00013 by whitney add end
   DEFINE l_lot_no        LIKE type_t.chr200   #C20181122-09421#1---add
...
#   SELECT * INTO l_pmdl.* 
   ...
   LET g_sql = " SELECT sfac001,sfac003,sfac004,sfac006 ",
               ...
               "   AND sfac002 = '5' ",  #副产品/回收料
               " ORDER BY sfacseq "
   PREPARE s_apmt500_gen_c1_ins_pmdn_p2 FROM g_sql
   IF SQLCA.sqlcode THEN
      INITIALIZE g_errparam TO NULL 
      LET g_errparam.extend = "prepare s_apmt500_gen_c1_ins_pmdn_p2" 
      LET g_errparam.code   = SQLCA.sqlcode
      LET g_errparam.popup  = TRUE 
      CALL cl_err()
      RETURN r_success
   END IF      
   DECLARE s_apmt500_gen_c1_ins_pmdn_cs2 CURSOR FOR s_apmt500_gen_c1_ins_pmdn_p2
   IF SQLCA.sqlcode THEN
      INITIALIZE g_errparam TO NULL 
      LET g_errparam.extend = "declare s_apmt500_gen_c1_ins_pmdn_cs2" 
      LET g_errparam.code   = SQLCA.sqlcode
      LET g_errparam.popup  = TRUE
      CALL cl_err()
      RETURN r_success
   ...
   LET g_sql = " SELECT sfcbdocno, sfaa010, sfac006,   sfcb001, sfcb002, sfcb003,",
               "        sfcb004,   sfcb020, SUM(carry_qty), sfcb044, sfcb045, price, assignsupplier ,weight ",   #add WEIGHT by hehw 200915
               "        ,lot_no ",   #C20181122-09421#1---add
               "   FROM wssp312_tmp_t ",
               "  WHERE sfcb013 = '",p_pmdl.pmdl004,"'",        #供应商
               "    AND pmdl017 = '",p_pmdl.pmdl017,"'",        #取价方式
               "    AND pmdl015 = '",p_pmdl.pmdl015,"'",        #币种
   ...
   LET g_sql = g_sql CLIPPED," GROUP BY sfcbdocno,sfaa010,sfac006,sfcb001,sfcb002,sfcb003,",
                             "          sfcb004,sfcb020,sfcb044,sfcb045,price,assignsupplier,weight "    #add WEIGHT by hehw 200915
                             ,"          ,lot_no "  #C20181122-09421#1---add                           
   ...
   PREPARE s_apmt500_gen_c1_ins_pmdn_p1 FROM g_sql
   IF SQLCA.sqlcode THEN
      INITIALIZE g_errparam TO NULL 
      LET g_errparam.extend = "prepare s_apmt500_gen_c1_ins_pmdn_p1" 
      LET g_errparam.code   = SQLCA.sqlcode
      LET g_errparam.popup  = TRUE 
      CALL cl_err()
      RETURN r_success
   ...
   DECLARE s_apmt500_gen_c1_ins_pmdn_cs1 CURSOR FOR s_apmt500_gen_c1_ins_pmdn_p1
   IF SQLCA.sqlcode THEN
      INITIALIZE g_errparam TO NULL 
      LET g_errparam.extend = "declare s_apmt500_gen_c1_ins_pmdn_cs1" 
      LET g_errparam.code   = SQLCA.sqlcode
      LET g_errparam.popup  = TRUE
      CALL cl_err()
      RETURN r_success
   END IF
   LET l_lot_no = ''   #C20181122-09421#1---add
   FOREACH s_apmt500_gen_c1_ins_pmdn_cs1 INTO l_sfcb.*,l_lot_no  #C20181122-09421#1---add l_lot_no
      IF SQLCA.sqlcode THEN
         INITIALIZE g_errparam TO NULL 
         LET g_errparam.extend = "foreach s_apmt500_gen_c1_ins_pmdn_cs1" 
         LET g_errparam.code   = SQLCA.sqlcode
         LET g_errparam.popup  = TRUE
         CALL cl_err()        
         RETURN r_success
...
#      SELECT * INTO l_sfaa.* FROM sfaa_t
      ...
      LET l_pmdn.pmdn006   = l_sfcb.sfcb020    #採購單位
      LET l_pmdn.pmdn007   = l_sfcb.carry_qty  #採購數量
      LET l_pmdnud001 = NULL 
      LET l_pmdnud014 = 0
      LET l_pmdnud001 = l_sfcb.assignsupplier  #基板供应商
      LET l_pmdnud014 = l_sfcb.weight          #WEIGHT
      #參考單位
      SELECT imaf015 INTO l_pmdn.pmdn008 FROM imaf_t
       WHERE imafent  = g_enterprise
         AND imafsite = l_pmdn.pmdnsite
...
#      LET l_pmdn.pmdn011   = l_pmdn.pmdn007    #計價數量    #161229-00014#1-----dujuan----mark
      ...
      LET l_pmdn.pmdn026   = l_pmdl.pmdl026    #帳款地址代碼    
      #供應商料號
      DECLARE s_apmt500_gen_c1_ins_pmdn_sel_cs1 SCROLL CURSOR FOR
       SELECT pmao004 FROM pmao_t
        WHERE pmaoent = g_enterprise 
          AND pmao001 = p_pmdl.pmdl004
          AND pmao002 = l_pmdn.pmdn001
      ...
      OPEN s_apmt500_gen_c1_ins_pmdn_sel_cs1 
      FETCH FIRST s_apmt500_gen_c1_ins_pmdn_sel_cs1 INTO l_pmdn.pmdn027
      ...
      PREPARE get_qcapc1 FROM l_sql
      EXECUTE get_qcapc1 INTO l_pmdn.pmdn052  
      FREE get_qcap1
      ...
      LET l_pmdn.pmdn056   = 0
      LET l_pmdn.pmdn057   = 0
      LET l_pmdn.pmdn050 = l_lot_no   #C20181122-09421#1---add
      #161124-00048#9 mod-S
#      INSERT INTO pmdn_t VALUES(l_pmdn.*)
      INSERT INTO pmdn_t(pmdnent,pmdnsite,pmdnunit,pmdndocno,pmdnseq,
                         pmdn001,pmdn002,pmdn003,pmdn004,pmdn005,
                         ...
                         pmdn222,pmdn223,pmdn224,pmdn900,pmdn999,
                         pmdn225,pmdn054,pmdn055,pmdn056,pmdn057,
                         pmdn226,pmdn227,pmdn058,pmdn228,pmdnud001,pmdnud014)   #add weight by hehw 200915
                  VALUES(l_pmdn.*,l_pmdnud001,l_pmdnud014)   #add weight by hehw 200915
      #161124-00048#9 mod-E
      IF SQLCA.sqlcode THEN
         RETURN r_success
      END IF
      ...
      LET l_sfcb_c.sfcbdocno   = l_sfcb.sfcbdocno  
      LET l_sfcb_c.sfaa010     = l_sfcb.sfaa010    
      LET l_sfcb_c.sfac006     = l_sfcb.sfac006    
      LET l_sfcb_c.sfcb001     = l_sfcb.sfcb001    
      LET l_sfcb_c.sfcb002     = l_sfcb.sfcb002    
      LET l_sfcb_c.sfcb003     = l_sfcb.sfcb003    
      LET l_sfcb_c.sfcb004     = l_sfcb.sfcb004    
      LET l_sfcb_c.sfcb020     = l_sfcb.sfcb020    
      LET l_sfcb_c.carry_qty   = l_sfcb.carry_qty  
      LET l_sfcb_c.sfcb044     = l_sfcb.sfcb044    
      LET l_sfcb_c.sfcb045     = l_sfcb.sfcb045    
      LET l_sfcb_c.price       = l_sfcb.price            

      #插入"pmdo_t:採購交期明細檔"
      CALL s_apmt500_gen_4_ins_pmdo(l_sfcb_c.*,l_pmdn.*)
           RETURNING l_success
      IF NOT l_success THEN
         RETURN r_success
      END IF           
      #插入"pmdp_t:採購關聯單據明細檔"
      CALL s_apmt500_gen_4_ins_pmdp(l_sfcb_c.*,l_pmdn.*)
           RETURNING l_success
      IF NOT l_success THEN
         RETURN r_success
      END IF     
      ...
      #160606-00013 by whitney add start
      IF l_sfcb_c.sfcbdocno <> l_sfaadocno_o OR l_sfaadocno_o IS NULL THEN
         LET l_cnt = 0
         SELECT COUNT(*) INTO l_cnt
           FROM sfac_t
          WHERE sfacent = g_enterprise
            AND sfacdocno = l_sfcb_c.sfcbdocno
            AND sfac002 = '5'  #副产品/回收料
         IF l_cnt > 0 THEN
            INITIALIZE l_sfac.* TO NULL
            FOREACH s_apmt500_gen_c1_ins_pmdn_cs2 USING l_sfcb_c.sfcbdocno
               INTO l_sfac.sfac001,l_sfac.sfac003,l_sfac.sfac004,l_sfac.sfac006
               IF SQLCA.sqlcode THEN
                  INITIALIZE g_errparam TO NULL 
                  LET g_errparam.extend = "foreach s_apmt500_gen_c1_ins_pmdn_cs2" 
                  LET g_errparam.code   = SQLCA.sqlcode
                  LET g_errparam.popup  = TRUE
                  CALL cl_err()        
                  RETURN r_success
...
#               LET l_pmdn.pmdn011   = l_pmdn.pmdn007    #計價數量  #161229-00014#1-----dujuan-----mark
               ...
               LET l_pmdn.pmdn019 = '13'  #子件特性
               #供應商料號
               DECLARE s_apmt500_gen_c1_ins_pmdn_sel_cs2 SCROLL CURSOR FOR
                SELECT pmao004 FROM pmao_t
                 WHERE pmaoent = g_enterprise 
                   AND pmao001 = p_pmdl.pmdl004
                   AND pmao002 = l_pmdn.pmdn001
                   ...
                   AND pmao000 = '1'      #161221-00064#5 add
                 ORDER BY pmao007 DESC
               OPEN s_apmt500_gen_c1_ins_pmdn_sel_cs2
               FETCH FIRST s_apmt500_gen_c1_ins_pmdn_sel_cs2 INTO l_pmdn.pmdn027
               LET l_pmdn.pmdn040 = ''
               LET l_pmdn.pmdn041 = ''
               LET l_pmdn.pmdn042 = ''
               LET l_pmdn.pmdn043 = ''
               ...
               IF (NOT cl_null(l_pmdn.pmdn004)) AND (NOT cl_null(l_pmdn.pmdn005)) THEN
                  LET l_sql = l_sql ," AND qcap003 = '",l_pmdn.pmdn004,"' AND qcap004 = '",l_pmdn.pmdn005,"' "
               END IF
               PREPARE get_qcap_c1 FROM l_sql
               EXECUTE get_qcap_c1 INTO l_pmdn.pmdn052  
               FREE get_qcap_c1
               IF cl_null(l_pmdn.pmdn052) THEN
                  #若沒有維護aqci050,再從aqci040中帶值
                  SELECT imae114 INTO l_pmdn.pmdn052 FROM imae_t 
...
#               INSERT INTO pmdn_t VALUES(l_pmdn.*)
            ...
            END FOREACH
         END IF
         LET l_sfaadocno_o = l_sfcb_c.sfcbdocno
      END IF
      #160606-00013 by whitney add end
...
# Descriptions...: 帶出匯率
# Memo...........:
# Usage..........: CALL s_apmt500_get_exchange(p_pmdl054,p_pmdl015,p_date)
#                  RETURNING r_pmdl016
# Input parameter: p_pmdl054       內外購
#                : p_pmdl015       幣別
#                : p_date          匯率基準日
# Return code....: r_pmdl016       匯率
# Date & Author..: #190711-00045#7 2019/11/15 By 12133
# Modify.........:
################################################################################
PUBLIC FUNCTION s_apmt500_get_exchange(p_pmdl054,p_pmdl015,p_date)
   DEFINE p_pmdl054          LIKE pmdl_t.pmdl054    #內外購
   DEFINE p_pmdl015          LIKE pmdl_t.pmdl015    #幣別
   DEFINE p_date             LIKE pmdl_t.pmdldocdt  #匯率基準日
   DEFINE r_pmdl016          LIKE pmdl_t.pmdl016    #匯率
   ...
   IF cl_null(p_pmdl054) OR cl_null(p_pmdl015) THEN
      RETURN r_pmdl016     #直接讓匯率為空
   END IF
   ...
   ELSE
      ...
      END IF
      #取匯率
      CALL s_aooi160_get_exrate('1',g_site,p_date,p_pmdl015,l_ooef016,0,l_para_data) RETURNING l_exchange
         
      LET r_pmdl016 = l_exchange
   END IF
...
################################################################################
# Descriptions...: 取得未稅金額、税额、含稅金額
# Memo...........: 寫入xrcd檔，匯率為當下輸入而非從資料庫撈取
# Usage..........: CALL s_apmt500_get_amount_3(p_pmdldocno,p_pmdnseq,p_pmdl015,p_pmdl016,p_pmdn007,p_pmdn015,p_pmdn016)
#                  RETURNING r_pmdn046,r_pmdn048,r_pmdn047
# Input parameter: p_pmdldocno    單據編號
#                : p_pmdnseq      項次
#                : p_pmdl015      幣別
...
#                : r_pmdn047      含稅金額
# Return code....: r_pmdn048      稅額
# Date & Author..: #190711-00045#7 2019/11/14 By 12133
# Modify.........:
################################################################################
PUBLIC FUNCTION s_apmt500_get_amount_3(p_pmdldocno,p_pmdnseq,p_pmdl015,p_pmdl016,p_pmdn007,p_pmdn015,p_pmdn016)
DEFINE p_pmdldocno       LIKE pmdl_t.pmdldocno
DEFINE p_pmdnseq         LIKE pmdn_t.pmdnseq
DEFINE p_pmdl015         LIKE pmdl_t.pmdl015
DEFINE p_pmdl016         LIKE pmdl_t.pmdl016 
...
DEFINE l_xrcd135         LIKE xrcd_t.xrcd115 
   ...
   IF cl_null(p_pmdldocno) OR cl_null(p_pmdnseq) OR cl_null(p_pmdn015) 
      OR cl_null(p_pmdl016) OR cl_null(p_pmdn007) OR cl_null(p_pmdn016) THEN
      RETURN r_pmdn046,r_pmdn048,r_pmdn047
   END IF
   ...
   CALL s_tax_ins(p_pmdldocno,p_pmdnseq,0,g_site,l_money,p_pmdn016,
                  p_pmdn007,p_pmdl015,p_pmdl016,' ',1,1)
        RETURNING r_pmdn046,r_pmdn048,r_pmdn047,l_xrcd113,l_xrcd114,l_xrcd115,
                  l_xrcd123,l_xrcd124,l_xrcd125,l_xrcd133,l_xrcd134,l_xrcd135
