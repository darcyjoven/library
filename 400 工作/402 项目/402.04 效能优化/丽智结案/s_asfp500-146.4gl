...
#+ Standard Version.....: SD版次:0049(1900-01-01 00:00:00), PR版次:0049(2021-06-11 15:29:36)
...
#190605-00009#1   2019/06/19 By 00272     同asfp500/asfp502寫法:拆件式工單的未完工比率要用實際產出數量(sfac005)與預計產出數量(sfac003)的比率
#190109-00048#1   2019/08/01 By 10042     修正結案時，如果單頭的工單來源是1.multi，還要增加過濾工單來源檔若有7.MPS計劃的資料，也要將工單來源單號的MPS計畫數量更新
#171129-00035#3   2019/08/20 By 07423     工單增加參數[可報工數只考慮良品數],若參數=Y,則報廢量與下線量不納入完工量計算
#191118-00017#1   2019/11/18 By 05423     修正成本次要素页签资料未取值的问题
#190109-00044#1   2019/04/25 By 10042     修正結案時，如果單頭的工單來源是1.multi，還要增加過濾工單來源檔若有6.獨立需求單的資料，也要將工單來源單號的獨立需求單數量更新
#200226-00043#1   2020/02/27 By 00272     發/退料檢查段,因為有可能在單據上僅KEY發料套數頁籤(sfdb_t),尚未KEY發料明細頁籤(sfdc_t),故SQL需再考慮僅存在sfdb_t的情況
#200320-00033#1   2020/03/23 By 00272     變數l_flag 長度定義不足,導致應顯示asf-00714錯誤,卻顯示成asf-00704
#200219-00026#1   2020/03/31 By 01534     工单结案时针对有倒扣料的工单进行检查，若这张工单倒扣料的实际发料量小于应发量，给出提示、不能结案(自动结案管控，人工结案给出提示询问)
#200911-00030#1   2020/09/11 By 02702     將200219-00026 mark 倒扣料提示部份恢復提示
#200817-00015#1   2020/08/18 By 10983     调整写法，错误需return
#210106-00050#1   2021/01/08 By 02295     xcbn具体到备料的项序,所以在抓取总应发量和总需求数量时,应细分到项次抓取
#210330-00008#1   2021/03/30 By 06978     修正判斷結案日期小於工單相關單據日期的報錯，增加當僅有工單的狀況(asf-01003)
#210518-00010#1   2021/05/18 By 10983     xcnb具体到备料的项序，解决sql报错
#210611-00008#1   2021/06/11 By 01996     量差明细档xcnb_t插入时抓取工单备料存在标准成本的资料时,需要关联sfba001 = xcah040
#end add-point
...
PUBLIC FUNCTION s_asfp500_unfinished_reference(p_sfaadocno,p_close_dd,p_flag)
   ...
   DEFINE l_sql          STRING #200226-00043#1 add
  ...
  #    AND sfdc001 = p_sfaadocno
  #    AND sfdastus <> 'X' AND sfdastus <> 'S' 
  #200226-00043#1--mark---end---   
   #200226-00043#1--add----str---
   LET l_sql =
              "SELECT UNIQUE sfda002,'asft310',sfdadocno,sfdastus,sfdacrtid,sfdacrtdp,sfdadocdt,sfda001,'' FROM sfda_t",
              " WHERE sfdaent = '",g_enterprise,"'",
              "   AND sfdastus <> 'X' ",
              "   AND sfdastus <> 'S' ",
              "   AND (    EXISTS (SELECT 1 FROM sfdc_t ",
              "                     WHERE sfdaent = sfdcent ",
              "                       AND sfdadocno = sfdcdocno ",
              "                       AND sfdc001 = '",p_sfaadocno,"'",
              "                    ) ",
              "         OR EXISTS (SELECT 1 FROM sfdb_t ",
              "                     WHERE sfdaent = sfdbent ",
              "                       AND sfdadocno = sfdbdocno ",
              "                       AND sfdb001 = '",p_sfaadocno,"'",
              "                   ) ",
              "       ) "
   PREPARE s_asfp500_unfinished_p1 FROM l_sql
   DECLARE s_asfp500_unfinished_reference_sfdc_cs1 CURSOR FOR s_asfp500_unfinished_p1
   #200226-00043#1--add----end---         
...
PUBLIC FUNCTION s_asfp500_close_wo(p_sfaadocno,p_close_dd,p_auto_flag,p_prog)
   ...
   DEFINE l_diff_rate3          LIKE type_t.num26_10  #190605-00009#1 add
   DEFINE l_cnt3                LIKE type_t.num10     #190605-00009#1 add
   DEFINE l_ok                  LIKE type_t.chr1      #190605-00009#1 add
#   DEFINE l_sfaa                RECORD LIKE sfaa_t.*   #161109-00085#60 mark 
   ...
   DEFINE l_d_mfg_0105          LIKE type_t.chr1         #171129-00035#3 add
...
#   SELECT * INTO l_sfaa.* FROM sfaa_t   #161109-00085#60 mark
  ...
  #200219-00026#1-S add
   LET l_cnt = 0
   SELECT COUNT(1) INTO l_cnt FROM sfba_t 
    WHERE sfbaent = g_enterprise
      AND sfba009 = 'Y' #倒扣料
      AND (sfba013 - sfba015 - sfba016) > 0
      AND sfbadocno = p_sfaadocno
   
   IF cl_null(l_cnt) THEN LET l_cnt = 0 END IF
   
   IF l_cnt > 0 AND p_auto_flag = 'Y' AND p_prog NOT MATCHES 'asfp500*' THEN   
      INITIALIZE g_errparam TO NULL
      LET g_errparam.code = 'asf-01027'
      LET g_errparam.extend = l_sfaa.sfaastus
      LET g_errparam.popup = TRUE
      CALL cl_err()   
      LET r_success = 'N'      
   ...
   #200911-00030#1 remark ---s---
   IF l_cnt > 0 AND p_prog NOT MATCHES 'asft340*' THEN   
      #IF NOT cl_ask_confirm('asf-01028') THEN
      IF NOT cl_ask_confirm2('asf-01028',p_sfaadocno) THEN
         LET r_success = 'N'      
         RETURN r_success,r_arr
      END IF      
   END IF
  ...
  #200219-00026#1-E add
   ...
   IF p_auto_flag = 'Y' THEN
      ...
      IF NOT s_num_isnum(l_diff_rate1) THEN
         ...
         RETURN r_success,r_arr
      END IF   
      #171129-00035#3 add --(S)--
      LET l_d_mfg_0105 = cl_get_doc_para(g_enterprise,g_site,l_doc_type,'D-MFG-0105')
      IF cl_null(l_d_mfg_0105) THEN LET l_d_mfg_0105 = 'N' END IF
      IF l_d_mfg_0105 = 'Y' THEN
         LET l_diff_qty = l_sfaa.sfaa012 - l_sfaa.sfaa050 - l_sfaa.sfaa051
      END IF
      #171129-00035#3 add --(E)--
      LET l_ok = 'N' #190605-00009#1 add
      LET l_diff_rate2 = l_diff_qty / l_sfaa.sfaa012 * 100
      #190605-00009#1----add----str---
      IF l_diff_rate2 <= l_diff_rate1 THEN
          LET l_ok = 'Y'
      END IF
      IF l_ok = 'N' THEN
          #拆件式工單的未完工比率要用實際產出數量(sfac005)與預計產出數量(sfac003)的比率
          LET l_cnt3=0
          LET l_diff_rate3=NULL
          SELECT COUNT(*),MAX(CASE WHEN sfac003=0 THEN 0 ELSE (sfac003-sfac005)/sfac003*100 END)
            INTO l_cnt3,l_diff_rate3
            FROM sfac_t
           WHERE sfacent = g_enterprise
             AND sfac002 IN ('3','4')
             AND sfacdocno = p_sfaadocno
          IF l_cnt3 > 0 THEN
              IF l_diff_rate3 <= l_diff_rate1 THEN
                  LET l_ok = 'Y'
              ELSE
                  LET l_diff_rate2 = l_diff_rate3
              END IF
          END IF
      END IF
      #190605-00009#1----add----end---
     #IF l_diff_rate2 > l_diff_rate1 THEN  #190605-00009#1 add
      IF l_ok = 'N'                  THEN  #190605-00009#1 mark
   ...
   #180928-00024#1---mark---s
   IF NOT s_asfp500_ins_xcna(p_sfaadocno) THEN
   #200817-00015#1---add---s
      LET r_success = 'N'
      RETURN r_success,r_arr
   #200817-00015#1---add---e   
   END IF
   #180928-00024#1---mark---e
...
PUBLIC FUNCTION s_asfp500_chk_reference_date(p_sfaadocno,p_close_dd)
  ...
  #200226-00043#1---mark---end--- 
   ...
   LET l_sql =
              ...
              " WHERE sfdaent = '",g_enterprise,"'",
              "   AND sfdastus <> 'X' ",
              "   AND (   sfdadocdt > '",p_close_dd,"'",  #單據日期
              "          OR sfda001 > '",p_close_dd,"')", #過帳日期
              "   AND (    EXISTS (SELECT 1 FROM sfdc_t ",
              "                     WHERE sfdaent = sfdcent ",
              "                       AND sfdadocno = sfdcdocno ",
              "                       AND sfdc001 = '",p_sfaadocno,"'",
              ...
              "                   ) ",
              "       ) "
   PREPARE s_asfp500_unfinished_p2 FROM l_sql
   DECLARE s_asfp500_chk_reference_date_sfdc_cs1 CURSOR FOR s_asfp500_unfinished_p2
   #200226-00043#1--add----end---  
   IF SQLCA.sqlcode THEN
      INITIALIZE g_errparam TO NULL
      LET g_errparam.code = SQLCA.sqlcode
...
PUBLIC FUNCTION s_asfp500_get_max_reference_date(p_sfaadocno)
   ...
   DEFINE l_arr               RECORD
                              ...
                              END RECORD
   DEFINE l_flag              LIKE type_t.chr10    #160111-00024#2 by Ann_Huang --- add  #200320-00033#1 mod chr1=>chr10
   DEFINE r_flag              LIKE type_t.chr10    #160111-00024#2 by Ann_Huang --- add  #200320-00033#1 mod chr1=>chr10
...
DEFINE l_success         LIKE type_t.num5
#190109-00048#1 -S add
DEFINE l_sql             STRING
DEFINE l_cnt             LIKE type_t.num5
DEFINE l_cnt1            LIKE type_t.num5
DEFINE l_sfab001         LIKE sfab_t.sfab001
DEFINE l_sfab002         LIKE sfab_t.sfab002
DEFINE l_sfab003         LIKE sfab_t.sfab003
DEFINE l_sfab007         LIKE sfab_t.sfab007
#190109-00048#1 -E add
   ...
   #   RETURN r_success
   #END IF
   #190109-00048#1 -E mark
   
   #190109-00048#1 -S add
   #如果單頭的工單來源是1.multi時，要將工單來源單號的MPS計畫數量更新
   LET l_sql = " SELECT sfaa010,sfaa013,sfaa050,sfab001,sfab002,sfab003,sfab007",
               "   FROM sfab_t",
               "  INNER JOIN sfaa_t ON sfabent = sfaaent AND sfabdocno = sfaadocno",
               "    AND sfabdocno = '",p_sfaadocno,"'",
               "    AND sfabent = ",g_enterprise,
               "    AND sfab001 = '7'",
               "  ORDER BY sfab002,sfabseq"
   PREPARE s_asfp500_psad_pre FROM l_sql
   DECLARE s_asfp500_psad_cur CURSOR FOR s_asfp500_psad_pre
   FOREACH s_asfp500_psad_cur INTO l_sfaa010,l_sfaa013,l_sfaa050,l_sfab001,l_sfab002,l_sfab003,l_sfab007
      IF SQLCA.sqlcode THEN
         INITIALIZE g_errparam TO NULL
         LET g_errparam.extend = 'FOREACH s_asfp500_psad_cur : '
         LET g_errparam.code   = SQLCA.sqlcode
         LET g_errparam.popup  = TRUE
         CALL cl_err()
         LET r_success = FALSE
      ...
      IF l_count > 0 THEN
         #計算
         SELECT psad004,psad006
           INTO l_psad004,l_psad006
           FROM psac_t,psad_t
          WHERE psacent = psadent
            AND psacdocno = psaddocno
            AND psadent = g_enterprise
            AND psaddocno = l_sfab002
            AND psadseq = l_sfab003
            AND psad001 = l_sfaa010
            AND psacstus = 'Y'
            
         #沒抓到對應的已耗需求量   
         IF cl_null(l_psad006) THEN
            LET r_success = FALSE
            RETURN r_success
         END IF
         
         #單位換算                   生產料號,生產單位,需求單位,本次轉開工單量
         CALL s_aooi250_convert_qty(l_sfaa010,l_sfaa013,l_psad004,l_sfab007)
             RETURNING l_success,l_sfab007
         #單位換算                   生產料號,生產單位,需求單位,已入庫量
         CALL s_aooi250_convert_qty(l_sfaa010,l_sfaa013,l_psad004,l_sfaa050)
             RETURNING l_success,l_sfaa050
         IF l_success THEN
            #已入庫量 - 本次開工量 >= 0 ，代表生產的數量都入庫，不需要減少已耗需求
            IF l_sfaa050 - l_sfab007 >= 0 THEN
               LET l_cnt = 0
               LET l_sfaa050 = l_sfaa050 - l_sfab007  #已入庫量扣掉本項次開工量
            #已入庫量 - 本次開工量 < 0 ，代表還有未生產的數量，需要減少已耗需求
            ELSE
               LET l_cnt = l_sfab007 - l_sfaa050
            END IF
            LET l_cnt1 = l_psad006 - l_cnt  #已耗需求 - l_cnt
         ELSE
            LET r_success = FALSE
            RETURN r_success
         END IF
...
#190109-00044#1 -E add
   ...
   #   RETURN r_success
   #END IF
   #190109-00044#1 -E mark
   
   #190109-00044#1 -S add
   #如果工單來源有6.獨立需求單時，結案時要更新獨立需求單的已耗數量
   LET l_sql = " SELECT sfaa010,sfaa013,sfaa050,sfab001,sfab002,sfab003,sfab007",
               "   FROM sfab_t",
               "  INNER JOIN sfaa_t ON sfabent = sfaaent AND sfabdocno = sfaadocno",
               "    AND sfabdocno = '",p_sfaadocno,"'",
               "    AND sfabent = ",g_enterprise,
               "    AND sfab001 = '6'",   #6.獨立需求單
               "  ORDER BY sfab002,sfabseq"
   PREPARE s_asfp500_psab_pre FROM l_sql
   DECLARE s_asfp500_psab_cur CURSOR FOR s_asfp500_psab_pre
   FOREACH s_asfp500_psab_cur INTO l_sfaa010,l_sfaa013,l_sfaa050,l_sfab001,l_sfab002,l_sfab003,l_sfab007
      IF SQLCA.sqlcode THEN
         INITIALIZE g_errparam TO NULL
         LET g_errparam.extend = 'FOREACH s_asfp500_psab_cur : '
         LET g_errparam.code   = SQLCA.sqlcode
         LET g_errparam.popup  = TRUE
         CALL cl_err()
         LET r_success = FALSE
      ...
      IF l_count > 0 THEN
         #計算
         SELECT psab004,psab006
           INTO l_psab004,l_psab006
           FROM psaa_t,psab_t
          WHERE psaaent = psabent
            AND psaadocno = psabdocno
            AND psabent = g_enterprise
            AND psabdocno = l_sfab002
            AND psabseq = l_sfab003
            AND psab001 = l_sfaa010
            AND psaastus = 'Y'
         ...
         #單位換算                   生產料號,生產單位,需求單位,本次轉開工單量
         CALL s_aooi250_convert_qty(l_sfaa010,l_sfaa013,l_psab004,l_sfab007)
             RETURNING l_success,l_sfab007
         #單位換算                   生產料號,生產單位,需求單位,已入庫量
         CALL s_aooi250_convert_qty(l_sfaa010,l_sfaa013,l_psab004,l_sfaa050)
             RETURNING l_success,l_sfaa050
         IF l_success THEN
            ...
            ELSE
               LET l_cnt = l_sfab007 - l_sfaa050
            END IF
            LET l_cnt1 = l_psab006 - l_cnt  #已耗需求 - l_cnt
         ELSE
            LET r_success = FALSE
            RETURN r_success
         END IF
...
DEFINE l_sql      STRING
   ...
   LET l_sql = " INSERT INTO s_asfp500_xcna VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?) "
   DECLARE s_asfp500_xcna_ins_curs CURSOR FROM l_sql
   
   LET l_sql = " INSERT INTO s_asfp500_xcnb VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?) "
   DECLARE s_asfp500_xcnb_ins_curs CURSOR FROM l_sql
   
   LET l_sql = " INSERT INTO s_asfp500_xcnc VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?) "
   ...
   FOREACH s_asfp500_sfaa_curs INTO l_xcna.xcnaent,l_xcna.xcnasite,l_xcna.xcnacomp,l_xcna.xcnadocno,l_xcna.xcnadocdt,
      ...
      END IF
      EXECUTE s_asfp500_xcna_ins_curs USING l_xcna.*  #将单头档资料插入暂存档
   ...
   LET l_sql = " MERGE INTO s_asfp500_xcnb t1 ",
              ...
              #"               C.xcna006 xcna006_sum,C.xcna007 xcna007_sum ",            #180823-00024#1 mark
               ...
               "               (SELECT xcnbent,xcnbdocno,xcnb003,xcnb004,SUM(xcna006) xcna006,SUM(xcna007) xcna007,xcnbseq,xcnbseq1 ", #210106-00050#1 add xcnbseq  #210518-00010#1 add xcnbseq1
               ...
               "                 GROUP BY xcnbent,xcnbdocno,xcnb003,xcnb004,xcnbseq,xcnbseq1 ) C ",  #210106-00050#1 add xcnbseq   #210518-00010#1 add xcnbseq1
               "         WHERE A.xcnadocno = B.xcnbdocno AND A.xcnaent = B.xcnbent AND A.xcna002 = B.xcnb001 AND A.xcna003 = B.xcnb002 ",
               #"           AND A.xcnadocno = C.xcnbdocno AND A.xcnaent = C.xcnbent AND B.xcnb003 = C.xcnb003 AND B.xcnb004 = C.xcnb004 ",  #210106-00050#1 mark
               #"           AND B.xcnbdocno = C.xcnbdocno AND B.xcnbent = C.xcnbent AND B.xcnb003 = C.xcnb003 AND B.xcnb004 = C.xcnb004 AND B.xcnbseq = C.xcnbseq  ",  #210106-00050#1 add  #210518-00010#1---mark
               "           AND B.xcnbdocno = C.xcnbdocno AND B.xcnbent = C.xcnbent AND B.xcnb003 = C.xcnb003 AND B.xcnb004 = C.xcnb004 AND B.xcnbseq = C.xcnbseq AND B.xcnbseq1 = C.xcnbseq1 ", #210518-00010#1---add
...
DEFINE l_sfba004  LIKE sfba_t.sfba004  #作业序   
   ...
   LET l_sql = " SELECT DISTINCT sfaaent,sfaasite,sfaadocno,sfaa010,sfac006, ",
               ...
               "                                  AND xcah040 = sfba001 ",  #210611-00008#1 add 增加关联本阶主件料号,以防止出现不同主件,相同元件的重复问题
   ...
   FOREACH s_asfp500_sfba_curs INTO l_xcnb.xcnbent,l_xcnb.xcnbsite,l_xcnb.xcnbdocno,l_xcnb.xcnb001,l_xcnb.xcnb002,
      ...
      #插入到xcnb中
      EXECUTE s_asfp500_xcnb_ins_curs USING l_xcnb.*
   END FOREACH
...
DEFINE l_xcai112_2  LIKE xcai_t.xcai112  #变动人工工时=变动工时/耗用批量
   ...
   #成本主要素、次要素待补充，先给空值
#   LET l_sql = " SELECT DISTINCT sfaadocno,sfaa011,sfac006,sffb007,sffb009,'','',", #191118-00017#1 mark
   LET l_sql = " SELECT DISTINCT sfaadocno,sfaa011,sfac006,sffb007,sffb009,xcai201,xcai202,", #191118-00017#1 mod
...
#               "               LEFT OUTER JOIN xcai_t B ON B.xcaient = sfaaent AND B.xcaisite = sfaasite AND B.xcai001 = '",l_xcaa001,"' AND sffb007 = B.xcai102 AND sffb009 = B.xcai103 AND B.xcai002 <= sfaadocdt AND B.xcai004 = sfaa010 AND B.xcai100 = sffb029 AND B.xcai110 = '1' ",#机器
               "               LEFT OUTER JOIN xcai_t B ON B.xcaient = sfaaent AND B.xcaisite = sfaasite AND B.xcai001 = '",l_xcaa001,"' AND sffb007 = B.xcai102 AND sffb009 = B.xcai103 AND B.xcai002 <= sfaadocdt AND B.xcai004 = sfaa010 AND B.xcai100 = sffb029  ",  #PGS(D)-00902-add-
...
#   LET l_sql = " SELECT DISTINCT sfaadocno,sfaa011,sfac006,e.xcai102,e.xcai103,e.xcai108,'','',sfaa010,",   #191118-00017#1 mark
   LET l_sql = " SELECT DISTINCT sfaadocno,sfaa011,sfac006,e.xcai102,e.xcai103,e.xcai108,e.xcai201,e.xcai202,sfaa010,",   #191118-00017#1 mod
...
#      LET l_xcnc.xcnc014 = l_xcnc.xcnc011 * l_xcnc.xcnc024  #标准机器工时耗用量=良品数量*标准机器工时单位耗用量
      ...
      #1-2.累计后插入到暂存档
      EXECUTE s_asfp500_xcnc_ins_curs USING l_xcnc.*
   END FOREACH
...
DEFINE r_success  LIKE type_t.num5
   ...
   DROP TABLE s_asfp500_xcna; #ODI-ORA#

   DROP TABLE s_asfp500_xcnb; #ODI-ORA#

   DROP TABLE s_asfp500_xcnc; #ODI-ORA#
...
PUBLIC FUNCTION s_asfp500_close_dd_chk(p_sfaadocno,p_close_dd)
   ...
   DEFINE l_flag          LIKE type_t.chr10 #200320-00033#1 mod chr1=>chr10
   ...
   IF p_close_dd < l_date THEN
      ...
      INITIALIZE g_errparam TO NULL
      CASE l_flag
           WHEN '0'  LET g_errparam.code = 'asf-01003'   #210330-00008#1 add 僅有工單日期
