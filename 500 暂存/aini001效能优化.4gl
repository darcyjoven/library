...
#+ Standard Version.....: SD版次:0019(2017-02-15 15:48:49), PR版次:0019(2021-06-17 10:57:14)
#+ Customerized Version.: SD版次:0000(1900-01-01 00:00:00), PR版次:0000(1900-01-01 00:00:00)
#+ Build......: 000432
#+ Filename...: aini001
#+ Description: 庫位資料維護作業
#+ Creator....: 02294(2013-07-26 11:01:32)
#+ Modifier...: 06978 -SD/PR- TOPSTD
...
PRIVATE FUNCTION aini001_delete()
   ...
   LET l_n = 0
   SELECT COUNT(1) INTO l_n FROM inag_t WHERE inagent = g_enterprise AND inagsite = g_site AND inag004 = g_inaa_d_t.inaa001
   IF l_n > 0 THEN
      INITIALIZE g_errparam TO NULL
   ...
   DIALOG ATTRIBUTES(UNBUFFERED,FIELD ORDER FORM)
      ...
      LET g_inaa_d[l_ac].inaa012 = "N"
         ...
         BEFORE DELETE  #是否取消單身
            ...
            ELSE
               ...
               LET l_n = 0
               SELECT COUNT(1) INTO l_n FROM inag_t WHERE inagent = g_enterprise AND inagsite = g_site AND inag004 = g_inaa_d[l_ac].inaa001
               IF l_n > 0 THEN
                  INITIALIZE g_errparam TO NULL
                  LET g_errparam.code = 'ain-00001'
      ...
      CALL cl_doc_remove()  
         ...
         AFTER FIELD inaa001
            ...
            IF  NOT cl_null(g_inaa_d[l_ac].inaa001) THEN 
               IF l_cmd = 'a' OR ( l_cmd = 'u' AND ( l_cmd = 'u' AND (g_inaa_d[l_ac].inaa001 != g_inaa_d_t.inaa001))) THEN 
                  IF NOT ap_chk_notDup("","SELECT COUNT(1) FROM inaa_t WHERE "||"inaaent = '" ||g_enterprise|| "' AND inaasite = '" ||g_site|| "' AND "||"inaa001 = '"||g_inaa_d[l_ac].inaa001 ||"' AND "|| "inaasite = '"||g_site ||"'",'std-00004',0) THEN 
                     LET g_inaa_d[l_ac].inaa001 = g_inaa_d_t.inaa001
                     CALL aini001_inaa001_desc()
                     NEXT FIELD CURRENT
                  END IF
                  IF NOT ap_chk_isExist(g_inaa_d[l_ac].inaa001,"SELECT COUNT(1) FROM inay_t WHERE inayent = '" ||g_enterprise||"' AND inay001 = ? ","ain-00307",1 ) THEN
                     LET g_inaa_d[l_ac].inaa001 = g_inaa_d_t.inaa001
                     CALL aini001_inaa001_desc()
                     NEXT FIELD CURRENT
                  END IF
         ...
         AFTER FIELD inaa005
            ...
            IF NOT cl_null(g_inaa_d[l_ac].inaa005) THEN 
               IF l_cmd = 'a' OR ( l_cmd = 'u' AND ( l_cmd = 'u' AND (g_inaa_d[l_ac].inaa005 != g_inaa_d_t.inaa005 OR g_inaa_d_t.inaa005 IS NULL))) THEN  #161019-00017#1 add g_inaa_d_t.inaa005 IS NULL
                  #IF NOT ap_chk_isExist(g_inaa_d[l_ac].inaa005,"SELECT COUNT(1) FROM ooea_t WHERE ooeaent = '" ||g_enterprise||"' AND ooea001 = ? ","aoo-00094",1 ) THEN
                  #   LET g_inaa_d[l_ac].inaa005 = g_inaa_d_t.inaa005
                  #   NEXT FIELD CURRENT
                  #END IF
                  #IF NOT ap_chk_isExist(g_inaa_d[l_ac].inaa005,"SELECT COUNT(1) FROM ooea_t WHERE ooeaent = '" ||g_enterprise||"' AND ooea001 = ? AND ooeastus = 'Y' ",'sub-01302','aooi125' ) THEN
                  #   LET g_inaa_d[l_ac].inaa005 = g_inaa_d_t.inaa005
                  #   NEXT FIELD CURRENT
                  #END IF
         ...
         AFTER FIELD inaa018
            ...
            LET l_n = 0
            SELECT COUNT(1) INTO l_n FROM inac_t
         ...
         BEFORE FIELD inaa010
            ...
            CALL aini001_inaa010_entry() #lixwz210617
            #END add-point
         ...
         BEFORE FIELD inaa011
            ...
            CALL aini001_inaa011_inaa012_entry()
            #END add-point
...
#    g_inaa3_d[l_ac].inac003,g_inaa3_d[l_ac].inac003_desc   #(ver:45) #(ver:46)mark
   ...
   #LET g_loc = 'd'
   CALL aini001_detail_show_current(g_detail_idx)
   
   LET l_ac = li_ac
   RETURN
   #add by lixwz210617 e---
...
#   END IF
   ...
   #mark by lixwz210617 s---
#   LET l_n = 0
#   SELECT COUNT(1) INTO l_n FROM xcck_t WHERE xcckent = g_enterprise AND xccksite = g_site AND xcck015 = g_inaa_d[l_ac].inaa001 AND xcckstus <>'X'
#         AND rownum <=1  #add by lixwz210617
#   #该库位已经结算过成本
#   IF l_n > 0 THEN
...
DEFINE r_success     LIKE type_t.num5
      ...
      END IF
      LET l_n = 0
      SELECT COUNT(1) INTO l_n FROM inah_t 
      ...
      END IF
      LET l_n = 0
      SELECT COUNT(1) INTO l_n FROM inai_t 
...
DEFINE r_success      LIKE type_t.num5
      ...
      IF NOT cl_null(p_inac003) THEN 
         IF NOT ap_chk_notDup("","SELECT COUNT(1) FROM inac_t WHERE "||"inacent = '" ||g_enterprise|| "' AND inacsite = '" ||g_site|| "' AND inac001 = '"||g_master.inaa001 || "' AND inac002 = ' ' AND inac003 = '"||p_inac003||"'",'std-00004',0) THEN 
            LET r_success = FALSE
            RETURN r_success
         END IF 
         
         IF NOT ap_chk_isExist(p_inac003,"SELECT COUNT(1) FROM oocq_t WHERE oocqent = '" ||g_enterprise||"' AND oocq001 = '220' AND oocq002 = ? ","sub-01303",'aini003') THEN#160318-00005#21 mod#"ain-00008",1 ) THEN
            LET r_success = FALSE
            RETURN r_success
         END IF
         IF NOT ap_chk_isExist(p_inac003,"SELECT COUNT(1) FROM oocq_t WHERE oocqent = '" ||g_enterprise||"' AND oocq001 = '220' AND oocq002 = ? AND oocqstus = 'Y' ",'sub-01302','aini003') THEN#160318-00005#21 mod #"ain-00009",1 ) THEN
            LET r_success = FALSE
            RETURN r_success
         END IF
      END IF
...
DEFINE l_n           LIKE type_t.num5
   ...
   LET l_n = 0
   SELECT COUNT(1) INTO l_n FROM inaa_t
    WHERE inaaent = g_enterprise
      AND inaasite = g_site
...
# Modify.........:
################################################################################
PRIVATE FUNCTION aini001_inaa010_entry()
   DEFINE l_n  LIKE type_t.num5
...
# Modify.........:
################################################################################
PRIVATE FUNCTION aini001_inaa011_inaa012_entry()
   DEFINE l_n  LIKE type_t.num5
...
# Modify.........:
################################################################################
PRIVATE FUNCTION aini001_detail_show_current(p_ac)
